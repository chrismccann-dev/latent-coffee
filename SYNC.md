...
- **Composite vs. independent terroir lookup.** Today `lib/terroir-registry.ts` exposes 3 independent `CanonicalLookup`s (country / macro / meso). The archive format writes `Country / Macro Terroir / Meso Terroir` positionally, so the sync validates each slot independently. Wrong-combo detection ("Panama / Sidama Highlands") is not enforced. The existing `matchTerroir` + `canonicalizeMacroTerroir` in [lib/brew-import.ts](lib/brew-import.ts) does do cross-country detection against a flat `TERROIR_REGISTRY` — this sync prefers the newer registry files but the older drift detector is a useful cross-check (see Phase 3).
...
| 5 | Terroir meso             | `Terroir` row, position 3           | `TERROIR_MESO_LOOKUP` (same file)                   | no       | warn      |
