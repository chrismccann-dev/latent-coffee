...
**Standing tripwires (not per-sprint, but always-on):** the canonical, loading-profile-aware tripwire table for **all** constantly-loaded core docs (root + CONTEXT-* + claude.ai prompts + clusters) now lives in **[docs/architecture/doc-tripwires.md](docs/architecture/doc-tripwires.md)** — keep its Current-size column current; a tripwire firing schedules a **manual post-tripwire pruning exercise** ([protocol](docs/features/doc-pruning-mechanism-brainstorm-2026-06-03.md)), not an autonomous prune. The root-doc numbers below are the every-session summary:
- **120KB on any root-level living doc** — when crossed, plan a split sprint. Currently (measured 2026-06-03): **CLAUDE.md ~38KB** (well under — per-page IA / data-model column histories / canonical-registry impl / design token map all extracted to `docs/architecture/page-ia.md` + `docs/architecture/data-model.md` + `docs/architecture/registries.md` + `docs/design-system.md`, pulled on demand); **PRODUCT.md ~114KB** (back under — § Design System extracted to the shared `docs/design-system.md`, but the Roadmap grows over time — watch); **CONTEXT-roasting.md ~116KB (🔴 live fire, 96% — queued as the next manual pruning exercise, kickoff `docs/sprints/context-roasting-prune-kickoff-2026-06-03.md`)**; CONTEXT-brewing.md ~60KB, CONTEXT-shared.md ~52KB, CONTEXT.md ~7KB redirect; BREWING.md ~3KB + ROASTING.md ~6KB redirect stubs. **claude.ai entry prompts carry a tighter 40KB cap** (full-read every session) — `docs/prompts/log-cupping.md` ~49KB is currently over. PRODUCT.md's `## Scaling Watch-Items` section enumerates the broader set.
