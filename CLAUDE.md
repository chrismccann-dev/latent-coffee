...
- **Canonical-registry factory:** [lib/canonical-registry.ts](lib/canonical-registry.ts) exports `makeCanonicalLookup(names, aliases?)` returning a `CanonicalLookup` bundle (`list` / `isCanonical` / `findClosest` / `isResolvable`) used by flavor / producer / roaster / terroir / cultivar registries. Any new canonical string-registry should route through this factory - do NOT re-implement the 3-tier classifier. Tier-0 alias map supports structural drift mapping (e.g. "Geisha" → "Gesha") — `isCanonical` still returns false on aliases, but `findClosest` surfaces the canonical target and `isResolvable` returns true for canonical OR alias-resolvable OR empty (use to gate Save buttons on strict-registry edit surfaces per `memory/feedback_canonical_registry_picker.md`).
- **Registry-as-picker architecture (post Variety 1b, 2026-04-22):** canonical-registry-backed form fields pick from the **registry**, not from existing DB rows. Missing DB rows are lazy materializations: server-side find-or-create on save populates the row with rich-shape attributes from the registry entry via `getXEntry(canonical)`. Aliases never materialize as DB rows. Shipped for cultivar on `/brews/[id]/edit` + `/add` purchased review. Terroir (sub-sprint 1d.3, queued) will follow the same shape. See `memory/feedback_canonical_registry_picker.md` for the full pattern + code hooks; extract `findOrCreateX(supabase, userId, entry)` helpers in `lib/brew-import.ts` rather than duplicating the find-or-create block per PATCH handler.
...
- **Edit brew:** `/brews/[id]/edit` → [EditBrewForm.tsx](app/(app)/brews/%5Bid%5D/edit/EditBrewForm.tsx), single-page (not stepped) form. PATCH handler [app/api/brews/[id]/route.ts](app/api/brews/%5Bid%5D/route.ts) uses a whitelist of editable fields + `.eq('user_id')` belt-and-suspenders over RLS. **Terroir** is pick-from-existing (select of DB rows; new terroirs go through `/add`). **Cultivar** is pick-from-canonical-registry via `CanonicalTextInput(CULTIVAR_LOOKUP)` (Variety sprint 1b); PATCH resolves `cultivar_name` → `cultivar_id` via find-or-create, auto-populating species/family/lineage from `getCultivarEntry()` on create, so any of the 63 canonicals can be picked even if no DB row exists yet. Aliases resolve to canonical on save ("Geisha" → "Gesha"). Genuinely new cultivars (not in registry, not alias-resolvable) still go through `/add`.
- **Canonical typeahead input:** [components/CanonicalTextInput.tsx](components/CanonicalTextInput.tsx) - single-value `<input>` + `<datalist>` + "did you mean X?" amber warning. Consumes a `CanonicalLookup` bundle from any registry built via `makeCanonicalLookup`. Used on the edit form for roaster + producer + cultivar. FlavorNotesInput keeps its own chip-input shape (multi-value) but routes canonicalization through the same factory. Do NOT re-implement the typeahead+warning pattern; import this component. `CanonicalLookup` exports `isResolvable(input)` for strict-registry save-gate validation — true when input is canonical OR alias-resolvable OR empty.
