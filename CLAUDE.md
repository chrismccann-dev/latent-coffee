...
- **Bulk text imports use `jsonb_to_recordset`, not dollar-quoted INSERTs.** When importing rows with newlines / smart quotes / em-dashes (anything from the roasting or brewing spreadsheets), encode the rows as JSON and use `WITH bean AS (...) INSERT ... SELECT bean.user_id, bean.green_bean_id, rec.* FROM bean, jsonb_to_recordset('[...]') AS rec(col1 text, col2 text, ...)`. JSON encoding handles every escape concern. See [supabase/migrations/019_experiments_import.sql](supabase/migrations/019_experiments_import.sql) as the reference shape.
- **Idempotent migrations need an explicit unique constraint.** When a migration inserts data that should re-run safely (`ON CONFLICT DO NOTHING`), add the constraint as part of the same migration if no natural one exists. Migration 019 added `experiments_user_bean_expid_unique (user_id, green_bean_id, experiment_id)` for this reason.
- **MCP `apply_migration` is for DDL; `execute_sql` is for data.** Each MCP call commits independently — don't try to span `BEGIN/COMMIT` across calls. Keep the on-disk `.sql` file as the single-transaction canonical record for future humans + replay.
