...
- **Tag-link list component:** [components/TagLinkList.tsx](components/TagLinkList.tsx) — `SectionCard` + flex-wrap of `<Link><Tag>` triplet. **Canonical primitive for every cross-link tag block on every aggregation detail page.** As of 2026-04-19 (cross-link backfill sprint, PR #19) all 4 pages route every CULTIVARS / TERROIRS / PROCESSES / ROASTERS EXPLORED block through it — no page has a plain `<SectionCard>` + `<Tag>` cross-link block anywhere. Accepts `{ title, items: { key, label, href }[] }` — empty `items` renders null. When a new cross-link tag block is needed, use this; NEVER re-inline the triplet. Href conventions: `/terroirs/{id}` + `/cultivars/{id}` need the id joined into the brew select (PostgREST embeds — one extra column, no extra query); `/processes/{encodeURIComponent(name)}` + `/roasters/{encodeURIComponent(name)}` use the free-text field as the slug.
