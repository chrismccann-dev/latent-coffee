...
- **Cultivar lineage values carry a literal " lineage" suffix** (e.g. `Gesha lineage`, `Bourbon mutation lineage`, `JARC blend lineage`). This is the canonical form in the `cultivars.lineage` column and in any URL / filter / programmatic access — bare `Gesha` will not match. The only lineage without the suffix is `Bourbon (classic)`. Exact values are derivable from the table; do not hard-code a different shape.
...
### Brews (`app/(app)/brews/`)
- **Index:** Grid of book-cover cards. All card content sits on the cover (variety / process / producer / region stack top-left, extraction-strategy chip top-right, flavor notes bottom). No text below the card — everything is one tile. 5-dimension filter bar above the grid (see below).
- **Filter bar:** [components/BrewsFilterBar.tsx](components/BrewsFilterBar.tsx) — client component. 5 dimensions: strategy / process family / roaster family / cultivar lineage / terroir macro. OR within a dimension, AND across dimensions. URL-driven via `searchParams.{strategy,processes,roasters,lineages,macros}` (comma-delimited CSV per dimension). Back button + shareable links work. Hybrid UX: low-cardinality dimensions (strategy / process / roaster) render as flat family-colored pill rows; high-cardinality dimensions (lineage / macro) render as popover-per-dimension buttons with an active-count badge. Empty state shows `NO BREWS MATCH THESE FILTERS` + clear-all; filtered-count renders `N / 55` in the header. All filter option lists are derived from the *unfiltered* corpus so no dead values surface. Server filtering is in-memory over the 55-brew fetch (clearer than dynamic Supabase `.in()` on joined columns at this scale).
