...
3. **Run `/simplify` before review or commit.** Claude over-engineers — duplicate JSX across pages, inline IIFEs, copy-pasted inline styles. Let the simplify skill catch it before it becomes tech debt. Run once per sprint, after implementation is done but before the commit step. Especially important when a sprint touched 2+ files that now share a rendering pattern. **Scan the full target file (not just the diff) for overlap** — dedup opportunities often cluster 100+ lines away from your edit; the mobile-polish sprint missed a `FilterTrigger` extraction during plan mode because the duplicate site was 170 lines down the same file.
