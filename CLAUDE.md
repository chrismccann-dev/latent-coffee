...
- **Audit gaps flagged, not yet fixed:** `/tmp/sprint-handoff/green-detail-audit.md` (Apr 2026) catalogs 20 gaps across P0-P5. Highlights: P0 display bugs where `moisture` renders `10.0%%` and `density` renders `809 g/L g/L` ([page.tsx:84-85](app/(app)/green/%5Bid%5D/page.tsx#L84), template appends units the schema already stores); P2 render — ROAST LEARNINGS only surfaces 5 of 16 schema fields (`starting_hypothesis` + `cultivar_takeaway` + `rest_behavior` are all load-bearing but hidden); ROAST LOG drops `fc_temp`, `drop_temp`, and `profile_link`. Read the audit file before scoping any `/green/[id]` render work.
- **Experiments block:** renders each row of the `experiments` table (22-field schema, A/B/C/D structured tests). Surfaces `experiment_id` (mono semibold) + `Batches` line + `.label`-prefixed Question / Variable / Winner (highlighted) / Key Insight / What Changes Going Forward. The 4 outcome columns + control_baseline / shared_constants / levels_tested / expected_outcomes / failure_boundary are stored in the DB but not surfaced — the page intentionally stays terse since each experiment can be 2KB+ of prose. Chris confirmed post-audit this section needs a rework (along with CUPPING HISTORY) — deferred to its own sprint.
- **Experiments import:** populated by [supabase/migrations/019_experiments_import.sql](supabase/migrations/019_experiments_import.sql). 18 rows currently land (4 beans × 2-7 experiments). 16 more are in the spreadsheet but blocked on the green_beans backfill for 5 missing beans (CGLE Mandela XO / Sudan Rume Washed / Sudan Rume Natural, Forrest Gesha Clouds, Higuito Anaerobic Bourbon); these land as each lot closes per upload-on-resolution.

### Add flow (`app/(app)/add/page.tsx`)
- **Two paths:** self-roasted is a 9-step wizard (Bean / Roast Log / Experiments / Cuppings / Learnings / Terroir / Cultivar / Brew / Save), purchased is a 4-step wizard (Bean / Terroir / Cultivar / Brew) routing through `/api/brews/parse` for Claude-assisted parsing. Both were audited + hardened in PR #25 after Chris hit parser bugs on first real end-to-end run.
- **Spreadsheet header normalization:** `parseSpreadsheet()` lowercases, replaces any non-alphanumeric char with `_`, collapses consecutive underscores, strips leading/trailing underscores. So `"Roest Batch #"` → `roest_batch` (not `roest_batch__`), `"Dev %"` → `dev`, `"Reference Roast?"` → `reference_roast`. Empty or duplicate headers get a positional `col_N` fallback key (e.g. the unlabeled "Observed Outcome D" column in the canonical experiments export lands at `col_14`). **Consumer parsers must use the normalized key shape** — the first pass of this file got 20+ keys wrong.
- **Parsers (all tested end-to-end against Chris's spreadsheet column layouts):** `parseGreenBeanSpreadsheet`, `parseRoastLogSpreadsheet`, `parseExperimentSpreadsheet`, `parseCuppingSpreadsheet`, `parseLearningsSpreadsheet`, `parseTerroirSpreadsheet`, `parseCultivarSpreadsheet`, `parseBrewSpreadsheet`. Each returns the camelCase shape `handleSaveSelfRoasted` expects. When adding a new field, start by pasting the header into the normalizer and confirming the output key; don't guess.
- **Cupping parser quirks:** `brew_method` is folded into `eval_method` at parse time (`"Pourover, spoon + full cup sip - xbloom (Brian Quan profile)"`) because the `cuppings` schema has no `brew_method` column. `sweetness` is dropped entirely. Both are P1 items in the green-detail audit — when the schema gets those columns, update the parser to stop folding/dropping.
- **Learnings parser quirk:** `best_batch_id` accepts multi-value source strings like `"#133 (confirmed reference), #148 (closest replication)"` and extracts the first integer (`"133"`). The UI's string-equality star-match on ROAST LOG requires this plain-integer form. Multi-winner representation is a P3 gap — deferred.
- **Save-flow find-or-create rule:** the lookup query must filter every column in the unique constraint, not a subset. Terroir's constraint is `(user_id, country, admin_region, macro_terroir)`; the lookup filters all 4. Use `.maybeSingle()` so 0 matches returns null cleanly instead of throwing into the destructure and silently leaving `data` as `null` (which then masquerades as "not found" and triggers an insert that collides on the real constraint). Cultivars' constraint is `(user_id, cultivar_name)` — 2 columns, matched exactly. Green_beans' constraint is `(user_id, lot_id)` — on conflict, surface a friendly "lot_id already exists" error rather than the raw Postgres message.

### Workflow: upload on resolution
- **Beans land in the archive only after the lot is structurally closed.** Purchased coffees: after the best brew is declared. Self-roasted: after the reference roast + replication is confirmed. The iteration itself stays out — Claude projects and spreadsheets hold in-progress work.
- **Why:** in-progress data warps `/brews` counts and aggregation synthesis. The archive is a claim about resolved coffees, not "every bean I've tasted." Keep the boundary clean.
- **Exceptions are deliberate and scoped.** SRH Washed was imported pre-resolution for the `/green/[id]` audit sprint (April 2026) because the audit needed real-shape data. Bean was deleted after the audit closed. The exception shape: scoped justification, explicit reversal on completion, documented in the sprint's PR. Don't extend "just this one exception" informally.
...
- **Exercise `/add`-flow changes end-to-end against real spreadsheet data.** The self-roasted flow had 20+ latent parser bugs that all survived until the first real end-to-end paste (PR #25 audit). UI compiling + save button clicking is not proof that a paste → DB path works. Paste representative rows, click Save, inspect the resulting DB rows. Delete test data after.
- **Spreadsheet source data isn't clean — dedupe on import.** The Coffee Roasting Spreadsheet has 27 copy-paste-duplicate cupping rows (117-145 mirrored at 195-220). Python import generators should dedupe on natural keys before emitting SQL.
