...
- **Process names** resolve to [lib/process-registry.ts](lib/process-registry.ts) (post sprint 1e.1, 2026-04-23). **Composable taxonomy**, not a flat list: every brew has `base_process` (1 of 4: Washed / Honey / Natural / Wet-hulled) + optional `subprocess` (Honey color tiers only: White / Yellow / Red / Black / Purple / Generic / Hydro) + up to 4 stackable modifier axes (fermentation 13 / drying 5 / intervention 7 / experimental 4) + optional `decaf_modifier` (SWP / MWP / EA / CO2) + optional `signature_method` (Moonshadow / TyOxidator / Hybrid Washed — proper-name proprietary). Each axis exports its own `CanonicalLookup` + alias map built via `makeCanonicalLookup`. Authoritative authored content in [docs/taxonomies/processes.md](docs/taxonomies/processes.md); DB schema migration to structured columns lands in 1e.2. `composeProcess(structured)` + `decomposeProcess(legacy)` helpers bridge structured ↔ display string. `LEGACY_DECOMPOSITIONS` maps all 20 current `brews.process` values to structured form for the 1e.2 migration (4 `TODO(1e.2)` cells Chris confirms at apply time). Back-compat: `PROCESS_FAMILIES` + `ProcessFamily` type + `getProcessFamily` + `getFamilyColor` preserved verbatim — /brews filter + /processes index render unchanged through 1e.1.
