...
**39 Tools live as of 2026-07-27** (`query_brews` 2026-07-27 — filtered/projected brew lookups; doc-proposal arbiter pair `list_doc_proposals` + `resolve_doc_proposal` landed 2026-06-25; `list_green_inventory` 2026-06-17; `list_skeleton_entries` Sprint 12 / CR-4; `push_roast_recipe` + `patch_roast_recipe` Sub Pages 6.1). The categorized table below is a human-reference overview and lags the live registry slightly — `lib/mcp/*.ts` is the source of truth.
...
|---|---|---|
| **Brews** | `push_brew`, `patch_brew`, `list_recent_brews`, `query_brews`, `get_brew` | Push/patch a brew; list recent, query filtered, or fetch by id (list/get promoted from Resources in 3.0.5 because claude.ai surfaces Tools, not Resources). `query_brews` (2026-07-27): targeted lookups by roaster / coffee_name / extraction_strategy / base_process / cultivar / since_date / green_bean_id, trimmed default projection + `fields` widener, roaster canonical-resolved via ROASTER_LOOKUP with substring fallback — `list_recent_brews` stays the unfiltered recency feed. |
| **Green beans** | `push_green_bean`, `patch_green_bean`, `get_green_bean`, `get_bean_pipeline` | Bean intake + lookups; `get_bean_pipeline` returns fat JSON `{green_bean, roasts[], cuppings[], experiments[], roast_learnings, brews[]}`. Lot Coordinator dogfood (2026-06-11, migration 080 / ADR-0024): `get_bean_pipeline` accepts optional `since:<ISO ts>` for incremental pulls (child arrays filter to `updated_at >= since`; response echoes `since_applied`); `patch_green_bean` accepts `lot_status` (Coordinator-explicit transitions, primarily `waiting_for_brewing` — most transitions auto-advance inside the push/patch write path, see lib/roast-import.ts § lot_status single write path). |
...
| **Cuppings** | `push_cupping`, `patch_cupping` | Day 7 evaluation push. Composite key includes `recipe_variant` (NULLS NOT DISTINCT). Required-axis fix (2026-07-15): `patch_cupping` also accepts optional `cupping_id` direct-PK lookup — when supplied, the composite-key fields (`cupping_date` / `eval_method` / `recipe_variant`) become ordinary patchable fields (the only MCP path that can change a cupping's own key fields). |
...
| **Doc proposals** | `propose_doc_changes`, `list_doc_proposals`, `resolve_doc_proposal` | Prose-doc-change arbiter queue (mirrors the Taxonomy queue's writer + reader + resolver shape as of 2026-06-25). `propose_doc_changes` writes (claude.ai); `list_doc_proposals` reads the pending queue; `resolve_doc_proposal` flips status applied/rejected/superseded (+ `supersede_ids` for arbiter-walked implicit supersession). Claude Code arbitrates in batch via [ARBITER.md](ARBITER.md) — no raw SQL required. |
...
| **Canonical reads** | `read_canonical`, `list_canonicals` | Canonical-introspection Tools. Same gotcha — Tools are reliably surfaced; Resources serve the same JSON. `read_canonical` has two modes: pass `name`/`names[]` for a single-entry lookup (resolves through canonical + alias + fuzzy, returns just the matched entry(ies) + their aliases — the token-cheap default when the value is known); omit them for the full-registry dump (back-compat, what the `canonicals://{axis}` Resource serves). Added 2026-05-31 to kill the full-registry-per-lookup context-window sink (flavors ~20K tokens, ~5% used per session). |
