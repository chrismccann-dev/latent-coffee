...
export default function ProcessSynthesis({
  process,
  existingSynthesis,
  existingBrewCount,
  currentBrewCount,
}: ProcessSynthesisProps) {
  ...
  async function generateSynthesis() {
    setLoading(true)
    try {
      const res = await fetch('/api/processes/synthesize', {
        method: 'POST',
    ...
    <SectionCard title="WHAT I'VE LEARNED ABOUT THIS PROCESS">
      {loading ? (
        <div className="flex items-center gap-3">
          ...
          <p className="font-mono text-xs text-latent-mid">Synthesizing knowledge from {currentBrewCount} {currentBrewCount === 1 ? 'coffee' : 'coffees'} processed {process.toLowerCase()}...</p>
        </div>
      ) : synthesis ? (
        <div>
