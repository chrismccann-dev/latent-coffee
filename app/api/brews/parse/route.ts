...
- modifiers (Axis 2) is an array of optional, stackable techniques layered on top of any strategy. Most brews have none — only include when the notes explicitly mention them. Shape per modifier:
  • { "type": "output_selection", "form": "early_cut" | "late_cut" | "both" | "dilution", "brew_weight": number | null, "cup_yield": number | null, "dilution_g": number | null, "notes": string | null } — reshaping the cup by discarding portions of the extraction curve OR adding water post-brew. brew_weight and cup_yield are grams (brewed vs. served). When form="dilution", dilution_g is grams of water added after brewing (e.g. 12).
  ...
  • { "type": "role_based_pulse", "roles": string | null } — assigning explicit sensory roles to each pour on a percolation-only brewer (V60 / Orea / Kalita / April / Chemex), e.g. "Pour 1=saturation · Pour 2=body · Pour 3=clarity". Use ONLY when the brew is on a percolation-only brewer; if the recipe involves immersion, classify under extraction_strategy="Hybrid" with hybrid_subform="phase_mapped" instead.
