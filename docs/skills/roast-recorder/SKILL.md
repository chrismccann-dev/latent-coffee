...
4. **`fc_audibility` enum (Sprint 11 RO-CP-3 / migration 061 / 2026-05-20; 5th value `did_not_fire` added Group 3 / Item 31 / migration 066 / 2026-05-24).** Per batch: `audible` / `subtle` / `silent` / `ambiguous` / `did_not_fire`. Four of five (subtle / silent / ambiguous / did_not_fire) trigger the same downstream protocol (bean-temp end condition + drop-ceiling-primary + Agtron proxies). `did_not_fire` is distinguished from `silent` by a bean-temp endpoint check: `did_not_fire` means the event structurally didn't happen (bean topped out below the 200-205°C FC window); `silent` means the event happened without sound (snap physically didn't fire on heavy-anaerobic cell-wall modification). On `did_not_fire`, `fc_start` / `fc_temp` / `dev_time_s` must all be null. See `cluster/protocols/fc-marking.md` in Roest Knowledge for the full protocol stack + disambiguation rule.
