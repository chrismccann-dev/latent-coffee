...
The brewing workflow gained a **Claude-Code-native entry surface**, [`.claude/skills/brew/SKILL.md`](.claude/skills/brew/SKILL.md), so a brew can run end-to-end on a Claude Code (mobile) session (Coffee Brief, recipe, in-thread iteration, optimized-brew push), the same flow the claude.ai project ran. **No change to this cluster's content or role.** The `brew` skill is a thin operator-direct entry (mirroring `roasting-coordinator`'s operator-direct pattern, [ADR-0024](docs/adr/0024-lot-coordinator-claude-code-native.md)) that composes this operational guide via `read_doc`; it does NOT restate it. Two CC-client habits were added at the entry surface (the skill + [`start-brew.md`](docs/prompts/start-brew.md)), not here: the **running tasting-arc state block** (keeps the iteration arc alive across mobile compaction) and a **close-retro append** to the new [`cluster/process-friction-log.md`](docs/skills/brewing-assistant/cluster/process-friction-log.md) (the brewing analog of the roasting process-friction log). `start-brew.md` + `bundled-brewing-completion.md` stay live as the claude.ai entry + the shared completion path (grace-handoff: net-new, retire on Chris's signal once mobile CC brewing is proven). This is a **surface migration**, deliberately NOT a Coordinator/Assistant restructure, because brewing has none of roasting's single-session context-bloat problem (one short thread; only the optimized brew persists).
