...
- Brew session intent (`start-brew.md` prompt; `log-brew.md` prompt continues the same session)
...
- **Called by:** Master Coordinator (via `start-brew.md` / `log-brew.md` brew-session intent), Cupping Specialist Path A (cross-domain handoff Chain 1 — optimized brew dial-in on the reference roast at lot resolution)
...
- **Prompts unchanged.** Per scope decision 2, `start-brew.md` / `log-brew.md` / `bundled-brewing-completion.md` continue as the claude.ai entry surface. Brewing Assistant becomes the canonical fetch target the prompts compose over.
...
- **Migration source:** today's `docs/prompts/start-brew.md` STEPS 1-3 cover Phase 1 (recipe construction); `docs/prompts/log-brew.md` covers Phase 2 (in-thread iteration); `docs/prompts/bundled-brewing-completion.md` covers Phase 3 handoff. Prompts continue to drive the operator surface; Brewing Assistant elevates the logic to a sub-skill spec.
