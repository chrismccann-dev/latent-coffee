...
The full BREW PROMPT operational content (Steps 1-4 + Recipe Output + Iteration Loop + Resolved Brew Output Format) - this doc is the executable how-to the brew session composes over via `read_doc_section`. Entry surfaces: the `/brew` skill (primary) and [`docs/prompts/start-brew.md`](docs/prompts/start-brew.md) (mobile fallback, Phase 1 recipe construction); Phase 2 iteration is in-thread with no per-iteration prompt, and the Phase 1 / Phase 2 / Phase 3 framing summary lives in [SKILL.md](docs/skills/brewing-assistant/SKILL.md).
...
Evaluate the coffee's process and variety against the **Process / Variety Signal Table** in [brewing-historian/cluster/patterns/cross-coffee-insights.md § Process / Variety Signal Table](docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md). Explicitly state whether any flags apply. This determines whether the default Clarity-First strategy is appropriate or whether a different extraction strategy should be considered from the start.
...
- **Prior brews of this coffee / sibling lots (DB)** — `query_brews(roaster: "<Canonical Roaster>")`, adding `coffee_name: "<farm / coffee family substring>"` to narrow. This is the bounded path for "have I brewed this before?" and sibling-variant lookups; rows carry the FK-joined terroir / cultivar / green_bean, so match a sibling lot's canonicals from here instead of minting divergent rows. Do NOT wide-pull `list_recent_brews` and filter by hand — that is the 100-240K-char overflow this Tool graduated from (2026-07-27).
...
The brief framework defaults to ultra-light / light roasters (Sey / Substance / Moonwake / Picky Chemist / Big Sur archetypes). When the roaster roasts the coffee at **medium** or above (verify via roaster product page; some specialty naturals are roasted medium even by clarity-oriented roasters), flag the adjusted over-extraction risk profile in 1c: roast character is the over-extraction risk on a medium roast, not under-development. Pushing harder surfaces roast-derived chocolate/nut/bittering forward of the green's fruit signature; the lever is evaluate cooler + accept a chocolatier register, NOT temperature/agitation push. Pattern PROMOTED 2026-07-27 (operator-ratified as any-process and not roaster-specific): the canonical home is the by-roast-level capsule [by-roast-level/medium-developed.md](docs/skills/brewing-historian/cluster/patterns/by-roast-level/medium-developed.md); this hook stays as the runtime intake guidance.

**Dark-roast override semantics + forceful-confirmation discipline at intake**: the medium-roast hook above is one anchor on a broader spectrum. General principle: **when a coffee is darker than the ultra-light / light default — or potentially darker but unconfirmed — roast level becomes the OVERRIDING signal above variety / altitude / process / everything else.** Analog to the heavy co-ferment / aggressive process tier-zero override (which dominates Process / Variety signal-table reasoning); the roast-level analog applies at the dark end of the spectrum. Be forceful at intake — not shy, not blocking. Three sub-disciplines apply when roast-level uncertainty surfaces at intake:

1. **Ask for Agtron reading** — if the brief reads "potentially darker than default" + no Agtron available, explicitly ask Chris to take a reading before constructing the recipe. Don't gate on it (proceed if unavailable); the ask itself is the discipline. **Practical-constraint awareness** (Chris-flagged 2026-05-24): the Lighttells CM200 is **home-only** (not available at the office); 15g brewing dose is insufficient for the reader (needs ~30g, so two doses pulled). Frame the ask in that constraint context — "if the CM200 is available + a 30g read is feasible at this brewing location" — rather than a blanket demand.

2. **Visual confirmation alternative** — when Agtron isn't feasible (office, limited beans, time constraint), ask for visual confirmation. Chris has roasted enough coffees to broadly distinguish light / light-medium / medium / past-medium by visual inspection (not perfect, but directionally useful). Frame: "can you take a quick visual look at the beans — do they read light / light-medium / medium / darker than that?"

3. **Cup-side signal recognition** — when the first brew surfaces very roasty / bitter / smoke / harsh qualities at lower temperatures (where light roasts wouldn't display these), recognize as a dark-roast tell and design subsequent iterations around the dark-roast override semantics (evaluate cooler, accept chocolatier register, do NOT push temperature / agitation push). This is the third route into the override: even if intake-time Agtron + visual were skipped, the cup-side signal still routes the recipe.

The discipline is **"more forceful but not blocking"**. The brewing expert should NOT default to default Clarity-First on lots that smell or read darker than the archetype suggests.

**Ground-Agtron-over-WB extraction-prior rule** (apply when ground Agtron is available):

When a **ground Agtron reading at dose-out** is available, treat it as the authoritative roast-level prior for over-vs-under extraction, NOT the whole-bean (WB) Agtron from the roaster's spec sheet, the inventory step, or the roast row. Mechanism (per [roest-knowledge/cluster/machine/counterflow-observations.md § WB-to-Ground Agtron Delta as Development Signal](docs/skills/roest-knowledge/cluster/machine/counterflow-observations.md)): WB Agtron tracks **surface** development; ground Agtron tracks the **surface + core composite** that the brew water actually encounters. When the WB-to-Ground delta is wide (>7 points), the WB read can suggest a lighter roast than the brewable composite actually is — hedging finer / warmer on the lighter WB prior over-extracts (lived case: CGLE Sudan Rume Round 13, where a WB-anchored finer/warmer hedge over-extracted Brew 1 and the ground read was the correct prior).

**Measurement-availability asymmetry by source** (Chris-locked decision):
...
When neither WB nor ground Agtron is available, fall back to the roaster's stated roast level + the Roaster roast-level hook above. Do NOT push Chris to start measuring ground Agtron on purchased lots — the no-ground-on-purchased decision is intentional **for routine cases**. **Carve-out**: when the brief reads "potentially darker than the ultra-light / light default", the "do NOT push" guard is **suspended** and the forceful-confirmation discipline above applies (three routes, not blocking).
...
| **Extraction Push**         | 6.3-5.5 (or finer) | 93-98°C                 | **Low — Melodrip, clarity-preserving**                               | 1:15-1:17          | Clean coffees where Clarity-First leaves yield on the table but Full Expression's high agitation would compress aromatic clarity: clean washed Geshas, Esmeralda climate-controlled naturals, washed processed lots with Wölfl/Tran/Giachgia targets. Confirmed brews: [by-strategy/extraction-push.md](docs/skills/brewing-historian/cluster/patterns/by-strategy/extraction-push.md). |
| **Hybrid** (v8.4)           | Varies by sub-form | Varies by sub-form (often staged) | Phase-dependent — different per phase by design                  | 1:14-1:17          | Switch-style brews and SWORKS valve-driven brews where each phase has a different job. Sub-form selected at brief: Sequential / Phase-Mapped / Selective Bloom / Intensity-Clarity Split / Temperature-Staged. Per-sub-form corpus status: [by-strategy/hybrid.md](docs/skills/brewing-historian/cluster/patterns/by-strategy/hybrid.md). |
...
**Going below grind 6.3 is a different philosophy, not a tweak.** For the highest-EY roasters, going below 5.5 may be required — but see [brewing-equipment-expert/cluster/grinder-eg1.md § Structural findings](docs/skills/brewing-equipment-expert/cluster/grinder-eg1.md) for the D50 plateau caveat (compression below 5.5 + floor at ~820-880 µm below 5.0).

**Apex clarify-side default (North Star).** For **apex coffees** (self-roasted, or apex-selected purchased) the strategy **defaults clarify-side (Suppression / Clarity-First / Hybrid)** - an apex default, not a global override: still pick the strategy keyed to the lot+roast at Step 1d. Full rationale + the non-apex training-ground case: [CONTEXT-taste.md § Brewing philosophy](CONTEXT-taste.md).

**Hybrid sub-form classification rule.** If the brewer changes mode mid-brew (immersion ↔ percolation, or pours have explicitly different sensory jobs), it is Hybrid; otherwise pick from the five intensity strategies. Per-strategy detail + per-sub-form examples live in [brewing-historian/cluster/patterns/by-strategy/](docs/skills/brewing-historian/cluster/patterns/by-strategy/). The 5 Hybrid sub-forms (Sequential / Phase-Mapped / Selective Bloom / Intensity-Clarity Split / Temperature-Staged) are documented in [brewing-historian/cluster/patterns/by-strategy/hybrid.md](docs/skills/brewing-historian/cluster/patterns/by-strategy/hybrid.md).
...
- **Thermal Staging** — Has this coffee or process type historically been resistant to both Clarity-First and Balanced lever moves? Thermal Staging is the experimental fallback when the standard temperature primacy rule does not resolve the cup. Default: none.
...
- **At home** (built water available): consult [water.md](docs/skills/brewing-equipment-expert/cluster/water.md) — the anion→phase chart (§ 3) + the recipe library (§ 6) — and **suggest a water recipe for this coffee + confirmed extraction strategy.** If a recipe-library row matches the coffee, use it; otherwise start from the **verified 2-coffee default: straight MgCl₂ @ GH 44 / KH 0** (Phase 2b, 2026-07-17 — won on both a clarity washed and a body-forward natural), then tune per the chart: back-of-cup body/structure wanted → add a **splash-dose sulfate fraction** (a 4:1-type blend; full-GH sulfate solo injects); **floral → no supported mineral route** (the WBC sulfate→florality label is 0/2 in Latent testing — florality rides overall cup quality, don't promise it from a build). Amplitude + sulfate-cation behavior are coffee-dependent, so a first cup on a new coffee is a re-taste, not a lock. Populate **Water Recipe** with the structured suggestion in taxonomy vocabulary (base + GH + KH + cation/anion). Frame it as an **offer, not a mandate** — the standing daily comp (Third Wave ~1:3, or any crushable comp) stays the low-effort default; the built suggestion is for when Chris wants to build deliberately.
...
After each brew, provide tasting notes across the **whole temperature arc** — the governing evaluation frame (arc-claim rationale: [CONTEXT-taste.md § Brewing philosophy](CONTEXT-taste.md)). Record every station, no throwaway: **aroma → hot ~59-60°C → warm ~54-55°C → cool ≤50°C and below**, plus how the cup changes between them. **Never iterate off a single temperature** — judge the *shape* of the evolution (does the cup open into new layers as it cools, or collapse to **one-dimensional** at one or more stations?), not one station in isolation; a cup that's great hot but flat cool (or vice versa) has failed the arc — an iteration signal, not a pass. A single-peak **Cooling-Curve Target** is a named exception *within* the whole-arc default (fires only when a coffee's peak window diverges from the 50-55°C drinking default — see the Named Consideration in Step 1d).
...
- **Whole-arc shape** — does the cup hold its **layered-evolving** shape across the *whole* arc (per the whole-arc frame at the top of this step)? Never resolve a brew on a single station's reading.
...
- **Bittersweet-cliff-at-temperature problem** ("clean at 95°C, bitter at 96°C") → **Hybrid (Temperature-Staged)**. Phase boundary coincides with temperature change. Distinct from a standalone Thermal Staging modifier because the temperature change is bound to a phase boundary.
- **Heavy co-ferment / anaerobic-natural / process-loud problem** ("the fermentation is muddling the cup") → **Suppression** (single-mode hold-back) is the canonical first call per [cross-coffee-insights.md § Anaerobic-Natural Suppression + Temperature-Primacy pattern](docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md). Confirmed 4 origins (Colombia / Ethiopia / Panama / Costa Rica). Step beyond Suppression to Hybrid (Intensity-Clarity Split) only if Suppression goes thin / sweetness retreats.
...
- [brewing-equipment-expert/cluster/operational-reference.md](docs/skills/brewing-equipment-expert/cluster/operational-reference.md) — location constraints + brewer rotation + Filter Flow Gap (valve / filter / grinder detail lives in the sibling sworks.md / filters.md / grinder-eg1.md)
...
- [docs/prompts/start-brew.md](docs/prompts/start-brew.md) — operational entry surface prompt (thin pointer to this doc) for Phase 1; Phase 2 iteration is in-thread with no per-iteration prompt
