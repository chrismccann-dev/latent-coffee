...
Operational entry surface = [`docs/prompts/start-brew.md`](docs/prompts/start-brew.md) (Phase 1 — recipe construction). Phase 2 (in-thread iteration) happens inside the active brew session with no dedicated per-iteration prompt; the prompt is a thin entry-surface invocation, while this doc holds the substantive operational guidance the session composes over via `read_doc`. The Phase 1 / Phase 2 / Phase 3 framing in [SKILL.md](docs/skills/brewing-assistant/SKILL.md) summarizes the role; this doc is the executable how-to. (`log-brew.md` was the Phase 2 entry surface through 2026-05-26 / Writing-path Sub-sprint 3, then deprecated to a redirect stub since Phase 2 iteration is in-thread per ADR-0011 § iteration-depth asymmetry.)
...
**Apex clarify-side default (North Star, 2026-06-14).** For **apex coffees** - self-roasted, or apex-selected purchased (developed-fully-for-expression by the roasting philosophy) - the brew is the clarify stage of the express-then-clarify couple, so the strategy **defaults clarify-side (Suppression / Clarity-First / Hybrid)**, Full Expression / Extraction Push the exception. For **non-apex off-register purchased** coffees the brew is the training ground (current practice + deliberately reach for more WBC technique; these roasts often *force* Full Expression - the wrong-zone trap). This is an **apex default, not a global override** of the strategy-neutral framework: still pick the strategy keyed to the lot+roast at Step 1d, and `Default-Clarity-First-is-conservative` (catalog § Brewing domain principles) still governs the non-apex case. Canon: [CONTEXT-taste.md § Brewing philosophy](CONTEXT-taste.md).
...
- **Filter** — canonical from `read_canonical("filters")` (e.g. `xBloom Premium Paper Filters`, `CONE FAST`, `FLAT FAST`, `UFO FAST`, `WAVE B3`, `CAFEC Abaca+ Cup 4 Cone Paper Filter`). Note: legacy `Espro Bloom Flat` resolves via alias to `xBloom Premium Paper Filters`. Sibarist canonicals do NOT include the `Sibarist` brand prefix (that's manufacturer metadata, not part of the canonical name); use `CONE FAST` not `Sibarist FAST CONE`. Cafec papers: short forms (`Cafec T-92`) resolve via alias to canonical (`CAFEC T-92 - Cup 4 Light Roast Paper Filter`); use either form. The CAFEC Cup-1 papers were collapsed into their Cup-4 twins (2026-06-04) — Chris brews single cups, so Cup-1/Cup-4 is paper SIZE he treats as identical; legacy `… Cup 1 …` strings still resolve via alias.
