...
**Set the BEAN_TEMP target ~3-4°C ABOVE the observed FC temp so the auto-drop extends the post-FC window rather than firing right behind FC.** If the target sits at or just above FC temp, the threshold fires within seconds of FC and forces a manual hold every batch to get any development - defeating the auto-drop. Calibrate the target against the coffee's *observed* FC temp, not a generic ceiling. Evidence: CGLE-GESHA-CLOUDS-2026 (close-out 2026-06-14). At a 250°C peak this lot's FC landed ~208-209°C; a 209°C BEAN_TEMP target (V3) fired ~4:10-4:20, right behind FC, forcing manual holds on all three V3 batches. Raising the target to 211°C (V4A) - ~2-3°C above the observed FC temp - produced the first clean auto-drop on the lot (5:09 / 211.0°C, dev 34s) and became the lot reference roast. The corollary: when the same curve is held but the target is raised too far past where the bean's RoR can carry it (V4C, 213°C target), the auto-drop won't fire before the time ceiling and a stall-prevention manual override fires instead - so the workable target band sits a few degrees above FC temp, not arbitrarily high.
