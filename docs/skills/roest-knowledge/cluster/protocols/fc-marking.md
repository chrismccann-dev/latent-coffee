...
**FC marking is now decoupled from drop control.** As of 2026-05-04, drop is controlled by bean temp end condition on the profile (see [cluster/machine/counterflow-observations.md § Drop Temp as the Primary Drop Signal](docs/skills/roest-knowledge/cluster/machine/counterflow-observations.md#drop-temp-as-the-primary-drop-signal)). FC marking is a data-recording event, not a drop-trigger event.
