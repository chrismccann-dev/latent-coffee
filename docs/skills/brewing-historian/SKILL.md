...
- [`cluster/patterns/cross-coffee-insights.md`](docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md) — the **cross-anchor insight layer**. Holds only learnings that cross ≥2 anchors (roast × process, roaster-override, cultivar × equipment, water × roast, cooling diagnostic, modifier compatibility) plus the Cross-Axis Strategy Router (tactical Coffee Brief lookup), the Modifier Layer (Axis 2, no single-axis capsule), Open Questions (staging queue), sub-threshold (n<3) cultivar candidates, and the End-of-Coffee Workflow. Single-axis detail lives in the capsules below; this file keeps a router pointer. Also holds sub-threshold (n<3) process-family candidates (mirror of the cultivar candidates). Re-architected pruning cases 007a/007b (2026-06-04): per the uniform n≥3 graduation lifecycle, By-Variety detail moved into the by-cultivar capsules (007a); 007b drained the Pending Relocation block — By-Process detail folded into the by-coffee-family capsules, Office Brewing Notes into the Brewing Equipment Expert cluster (sworks.md valve restriction-timing principles + operational-reference.md office tap water), per-coffee cooling notes into their cultivar/roaster capsules, and a `## Sub-Threshold Process-Family Candidates` section added for the n<3 families.
- [`cluster/patterns/by-strategy/<strategy>.md`](docs/skills/brewing-historian/cluster/patterns/by-strategy/) — 6 per-extraction-strategy files (Suppression / Clarity-First / Balanced Intensity / Full Expression / Extraction Push / Hybrid). Each holds the "Coffees That Confirmed X" evidence + cross-links to the Axis 1 canonical strategy definition.
- [`cluster/patterns/by-cultivar/<cultivar>.md`](docs/skills/brewing-historian/cluster/patterns/by-cultivar/) — 7 N≥3 graduated cultivar capsules (Gesha N=34, 74158 N=5, Sidra N=5, Ethiopian landrace population N=4, Mejorado N=3, Pacamara N=3, Sudan Rume N=3). Each now holds its variety-intrinsic + cultivar × process detail in full (graduated from the cross-coffee-insights.md § By Variety rollup in case 007a); cross-anchor interactions keep a router back to cross-coffee-insights.md. Sub-threshold (n<3) cultivars stay in cross-coffee-insights.md until they reach n≥3.
- [`cluster/patterns/by-coffee-family/<family>.md`](docs/skills/brewing-historian/cluster/patterns/by-coffee-family/) — 6 N≥3 base+fermentation-modifier capsules (Anaerobic Natural N=10, Anaerobic Washed N=5, Yeast-Inoculated Washed N=4, Yeast-Inoculated Natural N=3, Double Anaerobic Washed N=3 — also the heavy-co-ferment lane, Thermal Shock Washed N=3). Each now carries its By-Process default-strategy detail in full (folded in from the cross-coffee-insights.md By-Process table in case 007b); per-family deep-dive patterns accrue. Sub-threshold (n<3) families (Washed, White/Light Honey, Honey-medium, Standard Natural, Controlled Natural, Anoxic) stay in cross-coffee-insights.md § Sub-Threshold Process-Family Candidates until they reach n≥3.
...
- **Patterns:** A (substrate-event refresh on push_brew), D (tier-threshold refresh when corpus crosses early→emerging→established→mature), F (bloat-tripwire decomposition) — see [ADR-0013](docs/adr/0013-self-improvement-primitives.md)
...
- **Source migration:** BREWING.md § Cross-Coffee Insight Layer (lines 585-934 of the pre-PR file, ~90KB of content) extracted into 21 cluster files. BREWING.md shrunk from 213,831 bytes → 124,107 bytes (~88KB shrink, within the 60-80KB target band; slightly over). At this PR the h1 + pointer block at `#cross-coffee-insight-layer` stayed in BREWING.md for anchor back-compat. (BREWING.md is now a ~3KB redirect-stub pointer table post Wave 4 PR 4b — it no longer carries a `#cross-coffee-insight-layer` section anchor; follow its "Former section → Now at" map to [cluster/patterns/cross-coffee-insights.md](docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md).)
