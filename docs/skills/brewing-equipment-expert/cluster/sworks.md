...
| **Concentrated Pour-Over (valve form)** — canonical name locked at the 2026-08-31 item-56 grill (see [CONTEXT-brewing.md](CONTEXT-brewing.md) headword) | 0 (bloom, hold 40–45 s) → 5 (mains incl. early final pour to 150 g ~1:10–1:15) → 7 (late open ~1:45–2:00) | 1:10 undiluted concentrate on Picolot Loud Giants (Pacamara natural, Full Expression control) at the control's OWN grind + temp (6.1 / 95°C) — 1.73% TDS, 1.57× control, zero grind spent; reproduced-marginal (Δ 0.06%); the late-open TIME is the lever to pin ([RP8-T3](docs/research-projects/concentrated-pourover-t3-loud-giants-valve.md)). Confirmed on a second coffee first-try (RP8-T4 Chombi NASD, Suppression control: Dial 6 control → Dial 5 concentrate, late open pinned 1:40, 1.49% TDS). |
