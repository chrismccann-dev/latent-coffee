...
**Last adopted:** 2026-07-19 (RP6 final-screen fold - modifiers bounded per-coffee, § 6 row-3 ordering blind-adjudicated; RP6 experimental arc CLOSED).
...
|---|---|---|---|
| **More back-of-cup: body / sweetness / structure** | **Sulfate** - CaSO₄ (MgSO₄ unreliable) | **Splash-dose fraction of GH** (full GH-44 sulfate solo tips to inject - Phase 2b) | Cation behavior is **coffee-dependent** (PB: agnostic; Gesha: **CaSO₄ = structure-that-holds-sweetness-into-the-finish, confirmed 2/2 blind-adjacent tests**; MgSO₄'s Phase 2b "sweetness-in-body" read did NOT replicate - final screen re-test read savory-not-sweet, ranked last; **anecdote-until-reproduced**). Best used as a **complement to a chloride front** (the 4:1 blend pattern), not solo. Must be present **during** extraction (§ 5). |
...
| **Amplify the whole cup (subtle lift + finish)** | **Silica** (1 Eidon drop/L) | ~12 ppm; ceiling **<24 ppm** | **PB-BOUNDED - does NOT generalize (verified 2nd coffee 2026-07-19).** On the PB: low-dose across-the-board *amplifier* ("salting food") + lingering finish. On the Gesha: subtle sweetness-carry into the finish only, detectable only in repeated A/B. NOT the smooth/creamy texture the WBC field labels it (0/2 on a deliberate hunt). >24 ppm turns harsh/almost-salty. Optional-add status unchanged for PB-family cups. |
| **Savory / salinity seasoning / low-dose amplifier** | **NaCl** (very low) | ~2 ppm where workable | **Coffee-DEPENDENT (2 coffees, near-opposite reads).** PB: NO workable dose - ≥5 ppm injects savoriness amplifying the latent tomato note (base-independent, not chloride stacking). Gesha: **2 ppm = clean whole-cup amplifier, zero inject (2/2 flights)** - but demoted to last in same-day blind ranking; **fragile, low-confidence**. 5 ppm on the Gesha untested. Per-coffee verification required before use. |
...
| **Smoother, balanced amplifier of both axes** | **Blend** chloride-forward + sulfate fraction (**4:1 MgCl₂:CaSO₄** for structure; MgCl₂+MgSO₄ coffee-dependent) | GH ~44 total | Pairs the bright front (MgCl₂) with a back-of-cup scaffold. Coffee-set: on the clarity PB no blend beat MgCl₂ solo; on the body-wanting Mystic the MgSO₄ blend WON (single-session read); on the Gesha the 4:1 CaSO₄ blend **edged straight MgCl₂ blind** (final screen) while the 4:1 MgSO₄ blend FAILED (savory back, ranked last). MgCl₂ also **rescues** CaCl₂'s lactic edge in-blend. |
...
| **Hydrangea Gesha Natural DRD El Burro Lot 15** (Lamastus, Chiriquí, Panama; body-forward floral natural, mango/violet/strawberries) | **Primary: straight MgCl₂** · **Variant: 4:1 MgCl₂:CaSO₄** | ~44 | **zero** (KH 12 killed the back - verified) | zero (primary) / 20% share (variant, as CaSO₄) | **Avoid CaCl₂** (chalky/sour/inject here); Mg the safe cation | **Straight MgCl₂ @ GH 44 / KH 0** - clean, crisp, extends the front, sweet note blooms as it cools (reproduced flights 3+4+6). **Variant 4:1 blend** (finished waters 198:50) - silky, structured, holds the back half. **Ordering blind-adjudicated 2026-07-19 (final screen): the 4:1 blend ranked ABOVE straight pre-reveal, tie-adjacent** ("pretty close... A but a little more amplified") - combined with Phase 2b's pre-reveal tie, blind evidence is blend ≥ straight, never straight > blend; the earlier straight-lean was reveal bias. **Co-equal, slight blind edge to the blend. Operator practice default: straight MgCl₂ as the daily workhorse; blends/modifiers are intentional seasoning on top** (post-reveal call, flagged as practice not verdict). Blend-variant notes: 5:1 CaSO₄ did not beat the anchor unblinded (read slightly overdone) - **ratio ordering vs 4:1 UNRESOLVED** (P6T4-AI-4, the two never met head-to-head); 4:1 MgCl₂:MgSO₄ sweetness-payload **FAILS here** (savory-not-sweet back, ranked last - the Phase 2b flight-5 sweetness read did not replicate; exploratory single-cup reads are **anecdote-until-reproduced**). Untested three-parter idea logged: peak + light gypsum (~6-7:1) + 2 ppm NaCl (P6T4-AI-3). Xbloom V60 fixed recipe: 15 g/248 ml 1:16.5 · EG-1 6.2 · BP no-decline · ~3:36. Rest: markedly better day 19 than day 12 | **VERIFIED (Phase 2b + final screen, 28 brews, 3 sittings, blind-adjudicated)** |

**Optional amplifier (PB-bounded - 2nd-coffee verify FAILED to generalize, 2026-07-19):** +1 drop/L silica (~12 ppm; ceiling <24) lifted the Pink Bourbon's whole cup + finish, but on the Gesha produced only a subtle finish sweetness-carry (§ 3 silica row). An **optional** add for PB-family cups only, NOT baked into any peak, NOT a general amplifier.
