...
- **Brew finding (2026-05-31, brew 24b39678) - 'Fast' label ≠ fast in the cup for a Hybrid open-phase rinse.** On a Hario Switch Intensity-Clarity Split (Untold Janson Pacamara Natural, medium-dark Agtron 47.9), the Meteor 02 was used for the open-percolation 'clarity' phase whose entire job is to drain fast and rinse the roast tail rather than continue extracting. Despite the 'Fast' flow label, its measured ~65s drawdown (Research Project #1, baseline-indistinguishable) meant the open phase kept pulling roast bitterness on the way down and the cup stayed bitter. Swapping to Sibarist CONE FAST (measured 45s) made the open phase drain as a true rinse and resolved it. **Rule: for a Hybrid open-phase rinse, select the paper on MEASURED drawdown, not the marketing flow label - the Meteor 02 is a clarity-cone for full-brew use, not a fast-rinse paper.**
