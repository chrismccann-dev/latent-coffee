...
- [Roasting Historian](docs/skills/roasting-historian/) cluster — closed-lot patterns + open questions + cross-coffee insights + roast-to-brew translation
- [WBC Roasting Archivist](docs/skills/wbc-roasting-archivist/) cluster — WBC corpus + sourcing strategy
- [Roest Knowledge](docs/skills/roest-knowledge/) cluster — machine docs + API quirks + protocols (evaluation / fan-strategy / fc-marking)
- [Peer-Learning Roasting Archivist](docs/skills/peer-learning-roasting-archivist/) cluster — per-peer profiles (Dongzhe today)
- (Pre-2026-05-27 the Inputs list included "Learning Assistant track-aware metadata when the lot is part of a research track." That dependency is removed — Research Coordinator + Research Assistant per [ADR-0017](docs/adr/0017-research-assistant-architecture.md) do NOT write track-aware metadata onto constituent roasts. If a roast is part of a research track, the cross-link is logged in the project protocol doc + handoff brief, not on the `roasts` row.)
...
- **Patterns:** E (workflow-execution refresh — sub-skill invoked → output observed → success/fail captured via downstream cup outcome) — see [ADR-0013](docs/adr/0013-self-improvement-primitives.md)
...
- **POD-1 dependency:** when Cupping Specialist (Wave 3 PR 3) routes V-set Path B, it dispatches Roasting Assistant for V_(n+1) design. Handoff chain pre-authored in [`coordinator/handoff-rules.md`](docs/skills/coordinator/handoff-rules.md) Chain 3.
