...
- **Final screen (modifiers + blend variants on the Gesha) - EXECUTED + FOLDED 2026-07-19** (9 cups, 3 flights, one sitting; archive [water-final-screen.md](docs/research-projects/water-final-screen.md) incl. handoff brief). **The RP6 EXPERIMENTAL ARC IS CLOSED.** Verdicts: **HF5 RESOLVED blind - the 4:1 MgCl₂:CaSO₄ blend edged straight MgCl₂ pre-reveal (tie-adjacent); the prior straight-lean was reveal bias** (§ 6 row-3 caveat removed; operator practice default: straight as daily workhorse, blends as intentional seasoning). **HF1 FAILED - silica does NOT generalize** (PB-bounded; subtle finish-carry only on the Gesha). **HF2 partial contradiction - 2 ppm NaCl clean on the Gesha (zero inject, 2/2) but demoted in same-day blind: fragile.** **HF4 FAILED - MgSO₄ blend reads savory-not-sweet** (Phase 2b flight-5 read did not replicate → exploratory reads now tagged anecdote-until-reproduced). HF3 directional (sulfate axis live, ratio ordering open P6T4-AI-4). **Step 0 executed the food-grade MgCl₂ swap (9.16 mS, reagent retired) - R5 CLOSED.** Instrument finding: scale over-reads small masses ~33%; pipette + EC-endpoint titration adopted (§ 3a). Open items carried past the retro: P6T4-AI-1 (scale bench test, before next sitting) · AI-3 (three-parter micro-flight, ~36 g Gesha remains) · AI-4 (ratio ordering) · KCl remnant arm (standalone errand on arrival). **NEXT: the official RP6 project retro (operator + Coordinator grilling per [process-retro.md](docs/skills/research-coordinator/cluster/process-retro.md)) - the candidate ledger is closed: P6T2 L-c #1-11 · P6T3-N1-N7 · P6T4-N1-N6 + modifier-screen + Track-1 lessons await the cross-project graduation scan.**
