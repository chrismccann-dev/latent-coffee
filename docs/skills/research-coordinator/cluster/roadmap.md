...
- **Phase 2b (2nd-coffee verification) - SCOPED 2026-07-05, awaiting Assistant session.** The VALUES verification (structure no longer gated on it) on the **Hydrangea Gesha Natural DRD, El Burro Lot 15** (in hand; body-forward + floral contrast to the clarity Pink Bourbon). Focused pre-brew anion→phase replication (~15-20 brews / ~300 g) testing HV1 map-holds · HV2 phase-flip-toward-sulfate-on-a-body-coffee · HV3 sulfate→florality · HV4 CaCl₂-lactic-general? Protocol: [water-2nd-coffee-verification.md](docs/research-projects/water-2nd-coffee-verification.md) + [spawn prompt](docs/research-projects/water-2nd-coffee-verification-spawn-prompt.md). **If the map holds → de-provisionalize `water.md` § 6 + add a Gesha recipe row** (Coordinator/execution fold, post-track).
- **Modifier minerals screen (KCl / silica / NaCl) - SCOPED 2026-07-08, awaiting Assistant session.** The three WBC-seeded modifier tracks (potassium-as-finish · silica-as-texture · NaCl-salinity), combined into one session on the **Pink Bourbon** (extra in hand), run during the Phase-2b Gesha rest. Additive modifiers (no GH) dosed low on the MgCl₂ peak base + a distilled isolation cup each (~11 cups). Tests HM1 KCl→finish (disambiguates the disliked-KHCO₃-buffer from potassium-the-cation) · HM2 silica→texture · HM3 NaCl→salinity/sweetness. Protocol: [water-modifier-minerals.md](docs/research-projects/water-modifier-minerals.md) + [spawn prompt](docs/research-projects/water-modifier-minerals-spawn-prompt.md). Post-track fold: new `water.md` chart-rows + two new § 3 stock rows (KCl, NaCl).
