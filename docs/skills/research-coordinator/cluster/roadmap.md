...
- **Codification (operator call 2026-07-03) - the fold, de-gated for STRUCTURE:**
  ...
  - **Phase B (QUEUED):** wire water into the `/brew` flow (the existing free-text `water_recipe` field gets the taxonomy's vocabulary; water becomes a deliberate lever). No schema change expected.
