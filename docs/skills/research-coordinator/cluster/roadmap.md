...
- **Codification (operator call 2026-07-03) - the fold, de-gated for STRUCTURE:**
  ...
  - **Phase B (SCOPED, awaiting execution):** wire water into the `/brew` flow so each brew recipe also **suggests a water recipe** (from `water.md`'s chart), turning water into a deliberate lever in the normal brewing loop. **Home context only** (operator req 2026-07-04) - built water needs the home gear; office brews just record `water_recipe` as-is (no suggestion), an offer-not-mandate (the daily crushable comp stays the low-effort default). Pure skill-doc wiring (brewing-assistant Step 2 + location-constraints + the brew skill); the existing free-text `water_recipe` field + the already-present home/office location split carry it - no schema/MCP change. [Execution plan](docs/research-projects/water-codification-phase-b-plan.md). Future enhancement noted: recipe-library fold-back (home brews enriching `water.md`'s library over time).
