...
### Research Project #6 - Water chemistry & mineral concentrates (Phase 2 CLOSED · codifying)

**Opened:** 2026-06-20. Third project on the Coordinator/Assistant architecture; second taste-shaped project (RP6 retro graduates the RP5-seeded taste-study primitive candidates if they fire again). **Status:** Track 1 (Phase 1 concentrate screen) CLOSED 2026-06-21; **Track 2 (Phase 2 single-mineral isolation) CLOSED 2026-07-03 - all 5 HTs resolved.** Operator called codify-now (2026-07-03): **Codification Phase A scoped** ([execution plan](docs/research-projects/water-codification-phase-a-plan.md)) - the water knowledge substrate (taxonomy + chart + recipe seed), awaiting a fresh execution session.
...
- **Codification (operator call 2026-07-03) - the fold, de-gated for STRUCTURE:**
  - **Phase A (SCOPED, awaiting execution):** the water knowledge substrate as a brewing cluster doc - `docs/skills/brewing-equipment-expert/cluster/water.md` (taxonomy + the anion→phase chart + build method + the MgCl₂-forward Pink-Bourbon recipe seed, values stamped provisional) + MCP registration. [Execution plan](docs/research-projects/water-codification-phase-a-plan.md).
  - **Phase B (QUEUED):** wire water into the `/brew` flow so each brew recipe also **suggests a water recipe** (from `water.md`'s chart), turning water into a deliberate lever in the normal brewing loop. **Home context only** (operator req 2026-07-04) - built water needs the home gear (distiller / balance / stocks); office brews just record `water_recipe` as-is (no suggestion). The existing free-text `water_recipe` field gets the taxonomy's vocabulary. No schema change expected.
- **Phase 2b (2nd-coffee replication)** - the VALUES verification (structure no longer gated on it). Needs a coffee purchase + rest; a **body-wanting coffee** is ideal (tests the coffee-dependence prediction: the desired phase may flip toward sulfate).
- **Future RP6 tracks (2026 WBC-seeded, [handoff](docs/research-projects/wbc-2026-water-handoff.md)):** stage-split mineral profiling (Mg-early/Ca-late - native xBloom fit); potassium-as-finish (KCl - ordered); silica-as-texture (Eidon silica - ordered). Anchor: the two-chloride best cup ≈ Delgado's formula minus his sulfate+silica → test "+ a touch of sulfate + silica on the chloride base."
- End game: per-coffee water recipe library (`water.md` seeds it) - huge WBC payoff.

**Carried items / audit:** **P6T2-AI-1** (the anion→phase / cation-gating map) - pre-brew-CONFIRMED; structure codifies now (Phase A), values verify on the 2nd coffee. P6T1-AI-3 (TONIK roasted-barley confound) - still re-queued (no reference cup fit across 3 sittings); close or drop soon. L-c #1-11 method primitives logged in the Track 2 archive for the PROJECT retro (per-axis-scoring, WATER-vs-CONCENTRATE labeling, build-from-stocks, naive-palate-as-asset, pre-brew-variance-needs-reproduce). Candidate CONTEXT-taste pointer to the water mechanism (light, in Phase A).
