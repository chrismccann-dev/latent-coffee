...
**New lessons captured.** Numbered (continue the project's running numbering — Project #4's lessons start at RP4-N1 because lessons #1-40 inherited from #1-#3). **Assistant-issued numbers are PROVISIONAL:** Assistant sessions are context-firewalled from each other and WILL collide on numbers in multi-track projects (RP8's T3/T4 and T4/T5 briefs both did). The Coordinator canonicalizes the ledger at fold time or in the project end-document; briefs should cite their own numbers confidently and let the Coordinator reconcile. Lessons that graduate to cluster primitives go through the cross-project ratification gate at [`process-retro.md`](docs/skills/research-coordinator/cluster/process-retro.md). Single-project lessons stay logged here.
