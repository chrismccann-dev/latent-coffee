...
- **Presumptive office index (2026-09-01):** [docs/brewing/freezer-stock.md](docs/brewing/freezer-stock.md) now carries a `**Location:** Office` field on office entries + an **Office stock index** block in its header — use it as the scoping starting point instead of reconstructing holdings from old protocol docs. It is PRESUMPTIVE by design (counts stamped "as of <date>", refreshed opportunistically); the Step 0 physical count stays the authority.
