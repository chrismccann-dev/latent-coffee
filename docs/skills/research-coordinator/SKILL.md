...
**Office-lane exception (operator-established 2026-07-29; RATIFIED at the RP8 retro 2026-08-27):** the single-slot rule governs the home/bench lane, where research competes for the operator's scarce free time. Office brewing time is a separate resource pool (daily brewing at work on freezer coffees with already-optimized recipes, no competing setup) - one office-lane project may run concurrently with the home-lane § Now occupant. The roadmap carries at most one entry per lane; lanes are labeled explicitly. First occupant (RP8 concentrated pour-over, 5 tracks) proved the lane end-to-end; the lane's standing charter + protocol conventions (≤15-min-per-cup design budget, capture trims, shared-freezer inventory rules, low-inventory budget rule, brewing-only scope) live in [`cluster/office-lane.md`](docs/skills/research-coordinator/cluster/office-lane.md) — Coordinator bakes them into every office-lane protocol.
