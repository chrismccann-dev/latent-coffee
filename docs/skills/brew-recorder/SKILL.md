...
4. **Validate strategy + modifiers.** `extraction_strategy` z.enum on 6 canonicals (Suppression / Clarity-First / Balanced Intensity / Full Expression / Extraction Push / Hybrid). When Hybrid, `hybrid_subform` is REQUIRED. Modifiers via `cleanModifiers()` — 5 canonical types (output_selection / thermal_staging / aroma_capture / role_based_pulse / equipment) per `lib/extraction-modifiers.ts`. The legacy name `inverted_temperature_staging` is accepted on input and normalized to `thermal_staging` (alias-safe, Sub-sprint 4c); `immersion` was retired in v8.4 (absorbed into the Hybrid strategy via `hybrid_subform`).
