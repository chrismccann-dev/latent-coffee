...
Distinct from the per-lot arc above: the Coordinator also owns the **inventory roast-queue stack-rank** — the soft ordering of in_inventory lots that answers "what should I roast next?" Entry surface is operator-direct ("re-rank my inventory"), the same pattern as "start a new lot," but **inventory-scoped** — it does NOT adopt a single lot's Brief. It reads all in_inventory lots (`list_green_inventory`) + each lot's `intake_hypothesis` + the green inventory doc's Suggested Roasting Order, reasons a ranked-top + banded-tail stack-rank, and writes `roast_priority` + `roast_priority_rationale` per lot via `patch_green_bean`. The intake-time insert (step 1 above) is the incremental half of the same operation. Full doctrine: [`cluster/inventory-rerank.md`](docs/skills/roasting-coordinator/cluster/inventory-rerank.md). The `/green` inventory section renders in this order. **The Claude-Code trigger surface for both halves is the [`green-inventory`](.claude/skills/green-inventory/SKILL.md) skill** ("re-rank my inventory" / "add a green bean to my inventory") — the operator-direct wrapper, mirroring [`brew`](.claude/skills/brew/SKILL.md) on the brewing side. Adding a lot to inventory there is the *light* intake (lands `in_inventory` + ranked); the *full* intake that authors a Brief + designs V1 stays here ("start a new lot").
