...
1. **Mark reference.** `patch_roast`: `is_reference: true`, `worth_repeating: yes`, reference basis. The basis is **apex-defined** — the roast that best reveals **layered-evolving** complexity (develops across the temperature + structural arcs), **ceiling defined by the apex, not an Agtron number** ([SKILL.md § Apex anchor](docs/skills/roasting-coordinator/SKILL.md) / [CONTEXT-taste.md § Roasting philosophy](CONTEXT-taste.md)). (Distinct from the per-V-set *experiment winner* — don't over-patch historical experiment winner fields; the lot-level reference designation lives on the roast + in `roast_learnings`.)
2. **Write `roast_learnings`.** `push_roast_learnings` with `best_roast_id` + the 14 prose fields + the 4 `*_scope_tags` arrays. Capture the lived distinctions the lot taught — e.g. `evaluation_strategy` vs `optimized_drinking_strategy` when the V-set winner was confirmed under one brew strategy but the locked drinking recipe landed on another. Frame `why_this_roast_won` against the apex: which latent layers the winning roast revealed and held across the arc (the develop-fully-for-expression move), and where the **brew** picked up the residual suppression (the express-then-clarify couple).
