...
Declaring `roasts.is_reference = true` is the **load-bearing cross-domain workflow-transition gate** (see [CONTEXT.md § Reference roast](../../../CONTEXT.md)) — it transitions a lot from the roasting-side workflow to the brewing-side workflow, and the operator hands the reference roast into the brewing-side iteration loop to dial in the optimized brew. The declaration is intentionally definite, not ambiguous; the consequences begin downstream as soon as the flag flips. **The Cupping Specialist's job is to make sure the reference call lands on solid ground before the cross-domain transition begins.**
