...
**Post-close espresso validation (operator, 2026-09-09):** Batch 229 served as straight espresso AND short-ratio cortado - both excellent, with the **melon note accentuated** in both formats. The espresso-suitability hunch (originally parked on 228's profile) is confirmed on the reference itself: the 209-210.5°C development zone makes light Gesha espresso viable, and the cup holds under maximum-intensity extraction and milk - the controls-both-sides posture validated at the push extreme (brewing_tolerance upgraded to Moderate-High on the DB row). Final operator calibration: a win and close to the apex, with a hair lighter Agtron (~209.5-210°C trigger on this curve) the predicted filter-cup ideal - baked into the starting hypothesis.
