...
**Peer-variant pickup (Cluster A, 2026-06-01).** Before designing V1, ask: have I already brewed a peer-roasted variant of this green - an external roaster's version of the same lot? Peer variants get a 5-field Peer-Variant Handoff written brewing-side via `peer-variant-completion.md`; when the green lot didn't exist yet at brew time (the common pre-V1 case, e.g. Panama Janson) the handoff is **courier-carried** and I'll paste it here. If I bring one:
...
- `read_doc(uri="docs://skills/wbc-roasting-archivist/cluster/sourcing/strategy.md")` — sourcing buckets (action axis) + 5-lane portfolio frame + sourcing channels. The sourcing bucket shapes how much experiment investment is justified on this lot. For the inventory lane-mapping + gap analysis, `list_green_inventory` is the live per-lot source; the lane-lens summary is `read_doc(uri="docs://skills/wbc-roasting-archivist/cluster/sourcing/inventory-snapshot.md")`.
