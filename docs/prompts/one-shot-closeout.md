...
**If STAGE 4 deferred the call** (`additional_notes` on the experiment is prefixed `PROVISIONAL VERDICT: real-pourover pending` — the one-shot analog of the V-set Simulated Pourover Gate): LOCK the final Outcome A/B HERE, now that the optimized/real-pourover brew has been dialed in. Read the brew-side result and confirm with Chris which outcome holds, then proceed with that locked verdict through the remaining stages (it drives `worth_repeating` in STAGE 2 and `why_this_roast_won` in STAGE 4). Clear the `PROVISIONAL VERDICT` prefix from the experiment's `additional_notes` via `patch_experiment` once locked.

## STAGE 2 - Mark the reference roast (unconditionally for one-shots)

**One-shot exemption from Path A push-back duty** (Phase 1 / Item 34 follow-up / 2026-05-24): the [Cupping Specialist sub-skill's Pre-declaration discipline](docs/skills/cupping-specialist/SKILL.md) — which gates V-set lot Path A on Simulated Pourover Gate completion — does NOT apply to one-shots. Structural exemption: single roast (no V-set iteration), single Day 7 cup read (no V-set lineage means no secondary contender or V_(n-1) winner to compare against in an SPG cup-set). `is_reference: true` is set directly in this STAGE without the push-back gate. See [CONTEXT-roasting.md § Reference roast § Cross-domain workflow-transition gate](CONTEXT-roasting.md) for the cross-domain transition framing — declaring `is_reference = true` still triggers the brewing-side workflow transition on one-shots, just without the V-set-specific pre-declaration checklist.
...
- **`starting_hypothesis`**: most actionable field for the next similar one-shot. Same `"Low confidence"` prefix; encodes "next time I'd try <Z> based on this attempt." This is what future `one-shot.md` STAGE 1 carry-forward search will consume. **Seed it from the experiment's `what_changes_going_forward`** (authored in `one-shot.md` STAGE 4 — read it off the experiment row rather than re-authoring from scratch; that field already names the forward lesson, e.g. a drop-rule change). Refine/extend with anything close-out reflection added, but don't duplicate or re-derive the lesson the experiment already states. Pair with `starting_hypothesis_scope_tags` — the tags define which future similar-lot pattern surfaces this hypothesis on carry-forward search. **Tag aggressively here**: this is where the N=1 hypothesis earns its keep on the next one-shot.
...
- **Cross-Coffee Insight Layer**: ONLY append to `docs://skills/roasting-historian/cluster/patterns/cross-coffee-insights.md` if a generalizable cross-coffee pattern emerged AND `key_insight_confidence` was Medium-High or High on the experiment. One-shots typically don't reach the threshold for CCIL contribution; defer to repeated observation on similar future lots. Single-lot patterns at Low confidence go to `additional_notes` on the experiment row, NOT to CCIL. **N=2 cross-one-shot rule (backlog #47, codifying the operator's prior conservative call):** a single hypothesis replicated across two one-shots (N=2 cross-lot) still stays BELOW the CCIL bar - hold it in `general_takeaway` (with scope_tags so the next match surfaces it); promote to CCIL at N=3, or earlier only when the confidence ladder genuinely reads Medium-High (which N=2 cross-lot reaches only WITH a falsifier tested).
