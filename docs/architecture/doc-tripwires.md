...
| ✅ | docs/skills/brewing-equipment-expert/  [EquipmentReference] | 109.9 KB | 150 KB | claude.ai workflow |
...
1. **docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md (80.0 / 80 KB)** — 🔴 at the Historian single-doc cap; watch / split next time it's touched ([split kickoff](../sprints/cross-coffee-insights-split-kickoff-2026-06-03.md)).
2. **docs/prompts/one-shot.md (33.9 / 40 KB)** · **docs/skills/brewing-assistant/cluster/operational-guide.md (49.8 / 60)** · **wbc-recipes.md (48.8 / 60)** — ⚠️ approaching; watch.

Cleared:
- ~~docs/prompts/log-cupping.md~~ — **pruned 2026-06-04** (49.3 → 24.0 KB), [pruning case 006](../sprints/pruning-cases/006-log-cupping.md). First **prompt-shape (procedure)** case. `consolidate`-to-pointer (duplicated CONTEXT-roasting vocabulary → section pointers) + `archive` (STAGE 0 pre-rewrite migration → `docs/prompts/log-cupping-stage0-migration.md` + thin detection stub; DB check found 0 live pre-rewrite lots). **Live cupping behavior left byte-for-byte intact** — inline V_(n+1) design + doc-proposal kept per operator call (the move-out question went to the roadmap's Lot Coordinator item). Cap held comfortably with headroom (evidence: 40 KB is the right prompt cap).
