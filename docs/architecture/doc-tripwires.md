...
Cleared:
- ~~docs/skills/brewing-historian/cluster/patterns/cross-coffee-insights.md~~ — **re-architected 2026-06-04** (80.0 → 44 KB; cluster 157.2 → 135.5), [pruning case 007a](../sprints/pruning-cases/007-cross-coffee-insights.md) + 007b. **A new shape: `re-home` / `re-scope`** — not the scoped `split`. Tightened the CCIL's *job* (cross-anchor learnings only) and re-homed single-axis detail OUT to capsules behind routers; 007b drained the cross-cluster holding block (dedupe + back-pointer rewire). First re-scope; first cross-cluster re-home.
- ~~docs/prompts/log-cupping.md~~ — **pruned 2026-06-04** (49.3 → 24.0 KB), [pruning case 006](../sprints/pruning-cases/006-log-cupping.md). `consolidate`-to-pointer (duplicated CONTEXT-roasting vocab → pointers, behavior byte-for-byte intact) + `archive` (dead STAGE 0 pre-rewrite migration → sibling break-glass doc; DB-confirmed zero live pre-rewrite lots). First prompt-shape case; confirmed the **40 KB cap holds** (landed at 24 KB with full behavior).
- ~~docs/skills/brewing-equipment-expert/ cluster + filters.md~~ — **pruned 2026-06-03** (cluster 157.8 → 109.9 KB; `filters.md` 70.4 → 22.4 KB), [pruning case 004](../sprints/pruning-cases/004-filters-md.md). `split` on the ownership axis: owned (23) stay in `filters.md`, not-owned (44) → `docs/taxonomies/filters-not-owned-archive.md`; research blocks → appendix pointer. `lib/filter-registry.ts` kept intact as the full 67-entry validator. **Cleared the cluster cap too, so case 005 (cluster decomposition, Pattern F) is MOOT — never needed a session.** First `split` on an ownership axis + first split shaped by a validation-mirror constraint.
