...
- Lot-side residue of the schema bug: Roest inventory 9962 is missing structured moisture/density/elevation/price (values packed into its notes + live in the DB). Fix shipped; run from the next MCP-connected session: `patch_inventory(roest_inventory_id: 9962, moisture: 10.3, density: 786, elevation: 1900, price: 25.65)`. (The fix session couldn't execute it — no Latent MCP connection from Claude Code without a bearer token, and Roest creds live only in Vercel env.)
