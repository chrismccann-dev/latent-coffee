...
Both are real, long-lived DB columns used elsewhere (`lib/roast-import.ts`, `push_green_bean`); the interfaces were just incomplete. Neither is a cross-system substrate change (the columns already exist across all six actors) — this only brings Actor 6's type layer in line. The `GreenBean`/`Cupping` interfaces remain partial against the schema (e.g. 039's `seller` / `exporter` / `elevation_m` / `additional_notes` are still untyped) — left for adoption as consumers need them. **[Superseded same-day, 2026-06-05:** the schema-vs-type drift scan (#396) typed all 18 remaining drifts including those four, and the `check:types-vs-schema` gate (#397) reversed the adopt-as-consumed policy — *every column on a modeled table is now typed in `lib/types.ts`*, enforced on CI (standing rule in [CLAUDE.md](CLAUDE.md) Actor-6 guidance + [docs/architecture/data-model.md](docs/architecture/data-model.md)). The "left for adoption" sentence above records the belief at note-time only.**]
