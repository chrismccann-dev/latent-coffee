...
### 3. Apex coordinator bake-in — cross-domain doc-layer pass (NEW 2026-06-15; standalone, SHIPPABLE NOW)

Split out of the brewing migration (#4) so it can ship independently and serve the **live AN10 dogfood**. Pure doc-layer, no schema. The North Star apex propagated 2026-06-14 (PR #443) into the domain-principle homes ([catalog § Roasting / § Brewing principles](docs/skills/coordinator/catalog.md)) + [brewing-assistant operational-guide](docs/skills/brewing-assistant/cluster/operational-guide.md), but **not** into the `roasting-coordinator` skill's own steps - verified 2026-06-15: zero apex references across its SKILL.md + 8 cluster docs, so the live coordinator is apex-blind.

- **Roasting side (do first - serves AN10).** Insert apex roasting-philosophy references into `roasting-coordinator` design / route / close steps (V-set design, SPG decision, reference declaration): "ceiling defined by the apex, not an Agtron number," "the narrow band the V-sets hunt," develop-fully-for-expression then relocate-suppression-to-the-brew. The doctrine already exists verbatim in [catalog § Roasting domain principles](docs/skills/coordinator/catalog.md) + [CONTEXT-taste.md § Roasting philosophy](CONTEXT-taste.md) - this is a **pointer insertion, not re-authoring**, safe mid-dogfood (touches no lot data).
- **Brewing side.** Confirm/strengthen the apex surfacing in `brewing-assistant` (operational-guide already carries the clarify-side default at Step 1d); ensure **whole-arc station discipline** (aroma → hot ~59-60 → warm ~54-55 → cool ≤50; never iterate off a single temperature) is enforced in the iteration loop, not just named in the structural-wrong check.
- **Ride-along currency fixes (six-actor audit).** Add `layered-evolving` / `one-dimensional` / `reveal-not-inject` to the synthesis humanizer `USER_OVERRIDE` preservation list (Actor 6). Fix the stale 4-modifier list in `brew-recorder` SKILL.md step 4 (still `inverted_temperature_staging` / `immersion` - should be the 5-set `output_selection` / `thermal_staging` / `aroma_capture` / `role_based_pulse` / `equipment`, the same drift as the open `CONTEXT-brewing.md` modifier issue in [issues.md](docs/product/issues.md)).

**Trigger:** none - shippable now, **not gated** on the brewing trigger or the AN10 cycle.

### 4. Brewing-session migration to Claude Code — NEW (promoted from On deck 2026-06-15), DECOUPLED from #2

Move the brewing workflow off claude.ai onto a Claude-Code-native (mobile) session. **Not** a Coordinator/Assistant restructure - brewing lacks roasting's single-session context-bloat problem ([ADR-0024](docs/adr/0024-lot-coordinator-claude-code-native.md)), so this is a *surface migration of one short session*: it reuses `brewing-assistant` (Steps 1-4) + `brew-recorder` + the equipment / historian / WBC knowledge clusters, plus roasting's operator-direct entry pattern and self-improving-loop spine (friction log + retro). Mobile probe (2026-06-09/10) verified read path, write path (`propose_doc_changes` from mobile OAuth, end-to-end), and >24h idle-retention.

**Decoupled from #2:** gated only on **"next real brew,"** not on #2's fresh-lot trigger. Brewing's trigger fires more often and it is the cleaner first test of the operator-direct CC entry surface (no Brief, no Assistant split, no `lot_status` migration), so it de-risks that surface for roasting rather than waiting behind it. It inherits the apex-aware brewing stack from #3.

Sub-tasks:
- **4a. Entry surface.** Add a real invocable `.claude/skills/brew/SKILL.md` (trigger: "brew a coffee" / one-liner) wrapping `start-brew.md`'s framing - fetch operational-guide Steps 1-4, compose the three knowledge clusters, run Coffee Brief → recipe → iterate → complete via `bundled-brewing-completion`. Resolves the "no claude.ai project instructions in CC" gap, which is **shared with roasting** (neither coordinator has a `.claude/skills/` wrapper today); brewing establishes the pattern roasting can inherit (a `.claude/skills/lot/` wrapper).
- **4b. Audio parity.** Verify CC mobile audio dictation handles the long-multi-fact tasting-note turn (`feedback_audio_dictation.md` pattern); codify the **running-state-block habit** (mobile-probe finding - compaction blurs per-iteration tasting detail) into the brewing entry doc so the tasting arc survives the iteration loop. Verify-first - the one item that could block.
- **4c. Live dogfood.** One real brew end-to-end on mobile CC; capture friction; revise.
...
- **Whole-arc / layered-evolving tasting-capture on `brews` (scope-first; coupled to Active queue #4).** The brewing philosophy makes whole-arc evaluation the governing frame, and the migration (#4) starts *producing* arc data the schema can't store (`flavors` jsonb not temperature-stationed; `cooling_curve_target` a single window). Question: does the *optimized-brew archive row* need per-station (hot/warm/cool) capture - a structured `tasting_arc` jsonb, a column-only first pass, or is free-text `flavors` + `cooling_curve_target` enough? Fold in **`layered-evolving` as a structured cup-evaluation axis** (same substrate; can't score it without per-station data) and the relationship to the existing `cuppings.temperature_behavior` prose axis. Could conclude no-change. Canon: [CONTEXT-taste.md § Brewing philosophy](CONTEXT-taste.md).
...
- **Producers as a first-class citizen** (medium-term, needs a dedicated brainstorm). What does Chris fundamentally want — sourcing intelligence? farm-level lineage? a relationship graph (producer → farm → importer → roaster)? Brainstorm precedes any code. **Deferred until:** corpus has signal (currently only Pepe Jijon / Finca Soledad has 3+ brews). **Fold in (2026-06-15):** the North Star **sourcing philosophy** ([CONTEXT-taste.md](CONTEXT-taste.md)) names producer reputation as **acquisition signal #1** (proxy for the engineered process), implying a possible "known-for engineered process" process-signature attribute - distinct from pure aggregation. Same corpus-signal gate; near-term sourcing action is the Oct-2026 Panama push, which is operator/doc-layer, not app-layer.
