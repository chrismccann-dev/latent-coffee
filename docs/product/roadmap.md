...
### 1. Cluster B — system-maintenance / anti-bloat — NOW
...
- ~~**Doc-link remediation sprint**~~ - **SHIPPED 2026-06-05** ([shipped.md](docs/sprints/shipped.md)): migrated the file-relative `../../` minority to root-relative under [ADR-0021](docs/adr/0021-root-relative-doc-links.md), repointed the redirect-stub dead anchors, reconciled `page-ia.md` stale component pointers (Session 04 Candidates 1/3/4), and fixed a `check:doc-links` slug bug so anchor generation matches GitHub. `check:doc-links` is now green (0 live misses).
