...
**Surface note (2026-06-09):** Claude Code mobile shipped (cloud-container sessions, repo clone + MCP + skills, resumable threads). The brainstorm currently pins the Coordinator + V-Set Assistant to claude.ai sessions; re-evaluate claude.ai vs Claude Code mobile as the session surface at build time. See the brewing-surface migration entry in [On deck](#on-deck) for the mobile probe findings.
