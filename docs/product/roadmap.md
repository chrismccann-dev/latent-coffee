...
### 4. Brewing-session migration to Claude Code — 🔄 ENTRY SURFACE BUILT (2026-06-15); live dogfood (4c) pending, DECOUPLED from #2
...
**Status (2026-06-15):** the entry surface (4a) + audio/compaction codification (4b) **shipped** ([shipped.md](docs/sprints/shipped.md)); same posture as #2 (built, dogfood-pending). What ships: a net-new operator-direct [`.claude/skills/brew/SKILL.md`](.claude/skills/brew/SKILL.md) (verified to load in a desktop CC session) + a Claude-Code-client note + running-tasting-arc-state-block discipline added to both the skill and [`start-brew.md`](docs/prompts/start-brew.md) + the brewing [`process-friction-log.md`](docs/skills/brewing-assistant/cluster/process-friction-log.md) spine. **Two mobile-runtime gates are deferred to 4c** because they cannot be verified from a desktop worktree: (1) does a fresh *mobile* CC session surface the `brew` skill (if not, the `start-brew.md` operator-one-liner is the fallback entry, already wired), and (2) audio-dictation parity on the long-multi-fact tasting turn (a hard audio gap is the one finding that can block the migration). `start-brew.md` / `bundled-brewing-completion.md` stay live (grace-handoff; retire on Chris's signal once mobile CC brewing is proven).

Sub-tasks:
- **4a. Entry surface. ✅ BUILT 2026-06-15.** Net-new invocable [`.claude/skills/brew/SKILL.md`](.claude/skills/brew/SKILL.md) (trigger "brew a coffee" / "start a brew") composing `start-brew.md`'s framing: fetch operational-guide Steps 1-4 via `read_doc`, compose the three knowledge clusters, run Coffee Brief → pause at Step 1d → recipe → iterate → complete via `bundled-brewing-completion`, with the self-roasted hard-gate + optimized-brew carve-out carried over. Resolves the "no claude.ai project instructions in CC" gap, which is **shared with roasting** (neither coordinator has a `.claude/skills/` wrapper today); brewing establishes the pattern roasting can inherit (a `.claude/skills/lot/` wrapper).
- **4b. Audio parity. ✅ CODIFIED 2026-06-15; mobile verification deferred to 4c.** The **running-tasting-arc-state-block habit** (mobile-probe finding: compaction blurs per-iteration tasting detail) is now codified in both the `brew` skill and `start-brew.md` so the tasting arc survives the iteration loop. The live mobile audio-dictation parity check (`feedback_audio_dictation.md` pattern) is the one item that could block; it runs in the first dogfood brew.
- **4c. Live dogfood. ⏳ PENDING (next real brew).** One real brew end-to-end on mobile CC; verify skill-loading + audio parity; capture friction in the brewing process-friction-log; revise. The architecture is revisable on first-brew evidence.
