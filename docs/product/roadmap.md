...
- **Producer sourcing-priority canon pass** (queued, Chris-requested 2026-06-19) — a `/grill-with-docs` session to reconcile the `sourcingPriority` bucket model + the fit-vs-buy two-axis framing (from [the prototype](docs/features/producer-sourcing-targets-prototype-2026-06-19.md)) into canon: update [strategy.md § 7](docs/skills/wbc-roasting-archivist/cluster/sourcing/strategy.md) + [CONTEXT-taste § Sourcing philosophy](CONTEXT-taste.md), author the missing apex anchors (Mikava/Santuario, CGLE) as new `ProducerEntry` rows, and extend the seeded buckets to the broader watch/learning/reference roster. **Grilling session, not execution** — ask-don't-ship (the bucket model is prototype-not-canon today). **Trigger:** next session.
