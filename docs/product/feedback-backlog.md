...
*Last updated: 2026-07-27 (route-feedback pass on the Untold brew-retro N=3 graduations: filed the freezer-stock whole-doc-read overflow (prompt/doc-touch, high — mobile hard-block) + the `query_brews` filtered-lookup Tool (mcp, high); both arrive with a pre-authored kickoff brief from the 2026-07-27 retro session. Prior: 2026-07-15 — lifecycle-gate reconciliation grill shipped: #22 + #23 + #58 closed per ADR-0025 — SPG-pending composite ratified, winner-dependence partial-write rule, peer-cup gate demoted to advisory; new freezer-stock peer-roast reminder item filed. Earlier same day: MCP required-axis fix shipped — #57 + the SPG eval_method 2-row data patch flipped to shipped, PR #578; and the plan-feedback pass — claude.ai surface fully deprecated per Chris: Bucket C escalation + #49 + #51 + the Round-1 #5/#6 intermittents RETIRED; fc_audibility data-patch UNBLOCKED; #57 sub-issue (b) mooted by the 2026-07-10 single-connector canon.)*
...
### `query_brews` — brew lookups have no filter or projection (N=3, mobile hard-block shape)
- **Shape:** mcp (new Tool)
- **Recurrence:** 3 (brew sessions 2026-07-01 N=1, 2026-07-10 N=2, 2026-07-27 ×2 in one session)
- **Criticality:** high
- **Status:** planned (claimed by [docs/sprints/freezer-lookup-and-query-brews-kickoff.md](docs/sprints/freezer-lookup-and-query-brews-kickoff.md) item 2; build gated on Chris's (a)-vs-(b) design-fork answer)
- **Source:** brewing friction log ([process-friction-log.md](docs/skills/brewing-assistant/cluster/process-friction-log.md), 2026-07-27 untold-fazenda-um entry, PR #595); kickoff brief authored 2026-07-27 (item 2)
- **Body:** `list_recent_brews` has only `limit` + a fixed ~30-column select, so any targeted question (has this coffee been brewed? what FKs did the sibling lot use?) pulls a wide window: 112K and 239K chars in one session, both routed to file + `jq` — a recovery mobile sessions cannot do. The Tool's own description already forward-references "a future `query_brews` Tool" for filtered fetches. Design fork needs Chris's call: (b) new `query_brews` Tool [recommended, matches documented intent, 38→39] vs (a) filter params on `list_recent_brews`. A `fields` projection alone does NOT fix it — the filter is the load-bearing part. Filter axes: `roaster` + `coffee_name` (the pair that drove both blown calls) + the four already named in the description.
...
> **Shipped 2026-07-27 (freezer-stock lookup read-cap overflow — N=3 graduation, same-day route→plan→execute):** the whole-doc `read_doc` of `docs/brewing/freezer-stock.md` (107 bags / 70.8 KB, truncates ~line 487 — a mobile hard-block once the target bag sat at line 585) replaced with the live-verified enumerate-then-read-section pattern (`list_doc_sections` → `read_doc_section`, ~8.7 KB, cannot truncate) in BOTH entry surfaces: `/brew` SKILL.md Step 1a + `docs/prompts/start-brew.md` (mobile fallback). Friction-log entries 2026-07-01/07-10/07-27 annotated `→ graduated:`. Brief: [docs/sprints/freezer-lookup-and-query-brews-kickoff.md](docs/sprints/freezer-lookup-and-query-brews-kickoff.md). Removed from the open list per the status lifecycle.
