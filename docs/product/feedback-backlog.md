...
> **Shipped 2026-07-27 (`query_brews` MCP Tool):** the N=3 filtered-brew-lookup graduation built as option (b) — a new Tool per the kickoff brief's recommendation (Tool count 38→39; `list_recent_brews` stays the unfiltered recency feed). Filter axes `roaster` (canonical-resolved via ROASTER_LOOKUP, substring fallback) / `coffee_name` / `extraction_strategy` / `base_process` / `cultivar` / `since_date` / `green_bean_id`; trimmed default projection (identity + core recipe + FK-joined terroir/cultivar/green_bean) with `fields` as opt-in widener. Sibling-lot lookups drop from 112-239K-char wide-pulls to a few KB bounded. Completion report: [docs/sprints/query-brews-completion.md](docs/sprints/query-brews-completion.md). Removed from the open list per the status lifecycle.
