...
**Clean anaerobic washed (light, not heavy) → Clarity-First confirmed (one data point).** Ilde Burbano El Pilón Pink Bourbon (Palestina, Huila, 1740m; 24h anaerobic, double-washed, Light roast; green grape / blackberry / bergamot) landed on Clarity-First upper-edge - EG-1 6.5 / 94°C / Kalita Wave 155 + xBloom Premium Paper / 15g:240g (1:16). Distinguishes from the existing Moonwake processed-lot patterns: heavy anaerobic washed (Jeferson Motta) = Full Expression; this clean/light anaerobic *washed* on a transparency-driven variety stays clarity-side. Cup is layered-evolving and peaks integrated at ~45°C (grape + blackberry + bergamot finish); hot/warm reads separated and is not worth chasing. brew_id 8cc88312-3bee-453f-8965-e98d957f8c5e.
...
**Hachi × Terroir Maximus × Brian Quan collab set (Syrina, 3-variant, office 15g) - roast-development ceiling.** Distinct from Dongzhe's small-pouch washed/natural workflow above: this is a 3-process-variant Syrina set (50g each - 繁花 FanHua heavy-ferment natural, 流光 LiuGuang cascade ferment, 映雪 YinXue Enzyflow Honey), very-light roast (LiuGuang WB Agtron 86.9), brewed at the office (15g, SWORKS Bottomless Dripper / xBloom Premium, office tap). All variants need heavy pushing (Extraction Push, EG-1 ~5.6, 96°C held, SWORKS valve opening late on the final pour) - the 6.5 / 93-94°C small-pouch Clarity-First recipe does NOT apply here. Confirmed roast-development ceiling on 2 of the 3 variants (FanHua + YinXue): pushing extraction to develop the cup surfaces a generic brown-tea / roast-derived note rather than more fruit, and going finer than 5.6 pushes further INTO the roast, not the fruit. The brown-tea-at-the-finish that persists across the whole cooling arc + separates / turns acidic very cool is the ceiling tell, NOT under-extraction to chase - do not try to out-grind it. Best expression = the lighter-touch Extraction Push at 5.6 / 96°C; peak cool (≤50°C). When brewing the still-frozen 流光 LiuGuang, expect the same ceiling - start at 5.6 / 96°C and don't chase development with grind. Squares with Dongzhe's documented failure mode (early / underdeveloped brews taste roasty). Brews: FanHua `9ca79dbf`, YinXue `2729194b`.
...
| **Notes**               | **ROAST LEVEL IS NOT RELIABLE FROM FRAMING - CHECK AGTRON AT DOSE-OUT.** Untold's VARIES/clarity-to-balanced framing implies light/light-medium, but two archive lots came in materially darker than the framing suggested. (1) Brazil Fazenda Um Wush Wush Natural (WWNAT, 2026-05-22) - roasted MEDIUM; Balanced Intensity amplified developed-roast solubles into an oversteeped-black-tea wall; resolved via Hybrid Intensity-Clarity Split (first confirmed data point for that sub-form). (2) Panama Janson Pacamara Natural (Hacienda 491, 2026-05-31) - whole-bean Agtron 47.9, MEDIUM-DARK and visibly oily (darkest roast in the archive); the roast overrode both the Pacamara-density and the Dark Room Dried cold-room-dehydration signals; resolved via Hybrid Intensity-Clarity Split on the Switch (EG-1 6.8 / 88°C / Sibarist CONE FAST / tight late_cut ~205g of 250g; brew 24b39678). (3) Brazil Fazenda Um Pink Bourbon Dark Room Natural (TUVADKNAT, 2026-07-27) - whole-bean Agtron 70.1, MEDIUM LIGHT, same farm and producer as the WWNAT lot; the FIRST Untold lot with NO roast-tail wall. Operator read: 'a lot of Untold coffee is very overly roasted, this one is not as bad.' Resolved via Hybrid Intensity-Clarity Split but for the OPPOSITE reason - buried-front-note recovery, not roast-tail relocation - so CONE B3 (60s) was HELD rather than swapped to CONE FAST, and the late cut was tiny (~13g of 240g) rather than the 40-55g cuts the roast-tail lots needed (EG-1 6.6 / 92°C kettle on base / closed immersion to 155g steeping to 1:45; brew 91a014fd). **Working rule for Untold lots: take an Agtron read at dose-out and let roast level lead strategy - but the roast-tail expectation now carries an Agtron bound. At or below ~65 WB, expect to need Hybrid clarity-recovery structure (Intensity-Clarity Split) to relocate roast ashiness beneath the fruit; single-mode Balanced/Clarity mechanics amplify the roast rather than tame it. At Agtron ~48 the ashiness is inherent and not fully eliminable; the win is relocation, not elimination. At ~70 WB and above the wall did not appear - run a single-mode diagnostic brew first rather than pre-committing to ICS, because the ICS fast-rinse open phase starves a light-roast back half that has no heavy fraction to flush.** Three roast-tail-class data points are Untold lots, so the roaster-level skew toward more-developed-than-framed remains real - but it is a gradient with a threshold somewhere between Agtron 65.4 and 70.1, not a blanket property of the roaster. |


**Third developed-roast data point + a HARDWARE correction to the working rule (2026-07-27) - Panama Finca Deborah Geisha Natural "Interstellar" (Jamison Savage; Gesha Green Tip; Natural / Yeast Inoculated 100+hr; WB Agtron 60.6 Medium; brew c818dfcb).** Third consecutive Untold lot where roast level overrode process and cultivar - here beating BOTH the Panama Gesha Clarity-First default and the yeast-inoculated-natural Balanced Intensity default. The card's working rule ("let roast level lead strategy; expect to need Intensity-Clarity Split") was correct and was followed from brief time. **What the card was missing is that ICS has a hardware precondition.** Three brews of ICS on the April Switch failed; the operator identified that the April Switch cannot drain fast enough to make the ICS open phase a true rinse (confirmed bare, no paper). Brew 4 changed only the vehicle to Hario Switch + CONE FAST - holding EG-1 6.5 / 92C / 250g / no cut identical to Brew 1 - and the roast register went from dominant to manageable in one move. Both prior Untold resolutions (WWNAT, Janson Pacamara) had used a cone switch + fast paper; that was the operative variable, not just the sub-form. **Amended working rule for Untold lots: take an Agtron read at dose-out, let roast level lead strategy, and run the Intensity-Clarity Split on a CONE SWITCH + FAST PAPER - not the April Switch, whose integration strength distributes the roast through the cup instead of draining it.**

Two lot-level refinements: (1) **Do not cut the tail at ~Agtron 60.** A 200g-of-250g cut removed balancing depth and made the cup proportionally more roast-dominant; the Pacamara's tight cut is correct at 47.9 but wrong at 60.6, so the cut-tighter guidance applies to the moderately-dark end only. (2) **Post-brew NaCl was worth as much as the vehicle change on this lot** - a pinch of canning salt in the carafe took the roast character from present to "virtually gone." Worth trying as a standing finishing move on any Untold developed-roast lot; see the water cluster entry.

First Gesha in the Untold archive, and the first Untold lot where the roast defeated a delicate high-aromatic variety rather than a dense body-forward one - a useful boundary marker on how far Untold's developed roasting can be recovered.
