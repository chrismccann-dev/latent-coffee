...
- **Chrome vs semantic colors:**
  ...
  - **Lifecycle-tile gradient (v2, 2026-05-29):** `latent-tile-{inventory(grey),next-roast(leaf green #5F7A33),next-cupping(olive-bronze),brewing(steel blue),resolved(roasted-brown #3a2418)}` — green-coffee → roasted-coffee (brewing sits off-axis: water hue, ball in the brewing court). `/green` index binds states to these; unresolved keeps neutral `mid`. **The resync-grill reconciliation (2026-05-31) LANDED in the polish-audit sprint (2026-06-11): every green detail-view hero cover/edge now binds to its `--tile-*` token (waiting-roast / waiting-cupping / brewing / resolved / unresolved / inventory), matching the index card the user tapped from, while the in-content emphasis accents STAY (amber on drop-temp + drop-rules, lavender on predicted-cup + producer-notes, green on the resolved verdict + reference-cup).** Two separated color axes: **hero = lifecycle state** (consistent index→detail), **content accent = task salience** (points the eye at the field that matters at the bench). They never compete because they live in different page zones. `tile-next-roast` was re-derived off `#4A7C59` to leaf green `#5F7A33` (grilling-queue 53) so the lifecycle's "green bean" stop no longer shares a hue with `resolved-emphasis`.
...
- **Primitive components — use, don't reimplement:**
  ...
  - **Index-page family (`@/components/IndexList`, Redesign Sprint 6, 2026-05-29):** `IndexCap` (page caption — every index page) / `GrlRow` (grouped-list row, `count` → `.cnt` + 5-block `barBlocks` bar, OR `right` → `.simple` free-meta variant) / `GrlGroupHeader` (swatch + group name + count) / `LotStage` (/green lifecycle section header) + the `barBlocks(count)` helper — **absolute thresholds** (1→1 · 2→2 · 3-4→3 · 5-7→4 · 8+→5, unified with the confidence scale per grilling-queue 54), NOT normalized to the page max. CSS = the `.cap` / `.grl-*` / `.lot-stage` block in `globals.css` (the v2 "legacy" index family, distinct from `.ssp-*`). The /brews book-cover card is its own primitive (`@/components/BrewCard`). Index pages are fluid reference surfaces — no `.ssp-page` container query (grl/lot rows are inherently fluid; the brew grid uses Tailwind responsive cols).
...
- **Lifecycle tiles (green-coffee → roasted-coffee gradient):** `latent-tile-inventory` (grey `#B4B4AE`) → `latent-tile-next-roast` (leaf green `#5F7A33` — re-derived off `#4A7C59` per grilling-queue 53 so the lifecycle's "green bean" stop and the resolution accent stop sharing a hue) → `latent-tile-next-cupping` (olive-bronze `#6B5E3A`) → `latent-tile-resolved` (roasted brown `#3a2418`), plus the off-axis `latent-tile-brewing` (steel blue `#3F5E6B` — water; ball in the brewing court). Both index cards AND detail-view heroes bind states to these (hero rebind landed in the polish-audit sprint); unresolved keeps neutral `mid` (outside the green-brown axis). `#4A7C59` itself now means exactly two things: `resolved-emphasis` (the data signal) and the focus ring (`accent-light` — interaction chrome, never reads as data).
...
Tailwind defaults, used densely on a narrow set: **1 / 1.5 / 2 / 3 / 4 / 5 / 6 / 8 / 12 / 16**. Page shell is `.ssp-page` (`max-width: 820px`, set in `globals.css` — the lab-document column) for detail pages, `max-w-[1200px]` for the index grids and header. Horizontal padding `px-6`, vertical `py-8`. Header height fixed `h-14` (56px). No arbitrary `p-[17px]` anywhere — if a size isn't on the scale, it's drift. The v2 `--s1..--s8` CSS-var scale (4/8/12/16/22/32/48/64) lives in `:root` for the `.ssp-*` family's internal padding.
...
**Chrome @media exception (grilling-queue 52, ratified 2026-06-11).** Container queries govern the *detail surfaces*; **viewport-level chrome cannot use container queries by construction** and legitimately uses Tailwind `@media`: the `Header` nav collapse, the `BrewsFilterBar` FILTERS disclosure, and the index-page card grids (`sm:`/`lg:` grid columns). Chrome collapse points honor the two-point model — the Header and filter bar collapse at `lg:` (1024), so tablets 768-1023 get the hamburger + collapsed filters ("tablet falls out of fluid behavior" extends to chrome). New chrome @media outside these three surfaces still needs a reason.
...
The ranking is derived from the **most-constrained surface** — the 390px mobile width is the forcing function (per [ADR-0018](docs/adr/0018-per-surface-mobile-pattern.md)): least room forces the question "what is the single most important thing, and in what order?" That same ranking is then expressed at every width.
...
A standing tension the recount surfaced — **synthesis was demoted below the info block on every aggregation page because of its *length*, not its value** — was resolved by the capsule inversion (polish-audit sprint, 2026-06-11): `SynthesisCard` now renders the short-form capsule at every width with the long-form essay behind an inline `Full synthesis` disclosure, so its render weight matches its secondary rank. The roaster-page collapsed-tertiary exception stays as ratified (revisit after living with capsule weight — Q5 deferral).
...
- Unicode glyphs as chrome: `+ ADD`, `← Back to Brews`, `→` for hover-revealed affordance, `·` as metadata separator. Disclosure chevrons are the drawn `.chev` (a rotated 7px border corner — the ONE expand affordance app-wide), not a `▾` glyph.
- **The single sanctioned drawn icon: the Header hamburger/close SVG** (ratified 2026-06-11, Q4) — a `☰` glyph at 14px renders worse and varies by platform. Anything else drawn needs a new ratification.
- Exactly four emoji used as category prefixes only: `🌍` (terroir), `🧬` (cultivar), `☕` (empty brew avatar), `🌱` (empty green-bean avatar). Never in buttons, never in badges, never decorating tags. (The ConfidenceCard's 🟢🟡🔴 status emoji were replaced by the 5-block bar, 2026-06-11.)
...
- **No shadows at rest.** The only shadow in the product is index-card hover (`.brew-card` AND `.green-card`) — `translateY(-1px)` + a soft custom shadow (`0 8px 24px -10px rgba(0,0,0,0.16)`), the book-lifts-off-the-shelf moment. Focus ring is 2px sage `outline` + 2px offset.
- **Restrained motion.** Every transition is `transition-colors` or `transition-all duration-150` (buttons) / `0.12s` (index-card hover). Default CSS ease. No bounces, springs, skeleton shimmer, page-load fades, scroll-linked effects, or spinners (the synthesis loading state is a static mono line).
