...
- Panama Gesha Natural Anaerobic — Comp Edition lot (e.g. Picolot Janson Green-Tip Gesha Natural Anaerobic 1010) — confirmed Balanced Intensity at EG-1 6.3 / 94°C kettle-on-base / SWORKS slow/slow/open (Dial 5 → 5 → 7). 6.2 oversteeps (roast edge on attack, oversteeped finish, loses silky-creamy cooling); 6.3 is the floor. Kettle on base mattered — natural decline left brew finishing closer to 90°C and lost aromatic lift. Cooling-driven evaluation: cup reads quiet/muted hot, full label profile (raspberry → orange → honey → yuzu → rose → silky-creamy texture) only emerges below ~45°C. Comp roasts can prioritize structural precision over aromatic volume — "quiet hot" is not always under-extraction. Calibration point for evaluating future Picolot comp lots.
...
- SWORKS valve restriction timing principle (confirmed: Picolot Emerald Garrido Mokka Natural). Restricting the valve to Dial 5 during mid-extraction pours (Pours 1–2) can REDUCE fruit and sugar development by starving the bed of fresh water. Extraction depends on the concentration gradient of fresh water meeting saturated grounds, not just contact time — holding a restricted valve through the extraction-critical window slows flow when you want water renewal. Reserve Dial 5 restriction for the final integration pour (the slow phase). For fast/fast/slow roaster structures (e.g. Picolot): Dial 7 Open → Dial 7 Open → Dial 5 Restricted, not Dial 5 throughout. **Exception family — heavy fermentation lots invert this rule** because the extraction ceiling is much higher: (a) heavy co-ferment washed lots (e.g. Moonwake El Eden Tamarind) need Dial 5 through all main pours, valve timing is about WHEN to crack open. (b) Heavy yeast-anaerobic naturals (e.g. Picolot Janson Green-Tip Gesha Natural Anaerobic 1010) invert to slow/slow/open (Dial 5 → 5 → 7) — restricting Pours 1–2 forces contact during the aromatic-extraction window, opening Pour 3 rinses rather than steeps on the finish. Same logic, different process family. Generalized principle: **extraction ceiling determines whether contact time is the bottleneck (high ceiling → restrict early) or water renewal is the bottleneck (low ceiling → open early).** Diagnostic for low-ceiling lots: tea-like body with subtle attack + no sweetness = under-extraction from over-restriction of mid-pours. Fix by opening Pours 1–2 and going finer on grind. Diagnostic for high-ceiling lots: muddled attack with dark tea aftertaste = split extraction from fast early pours moving water past bed before aromatics express. Fix by inverting to slow/slow/open.
