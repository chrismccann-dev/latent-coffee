...
import {
  displayVariety,
  displayCoverProcess,
  displayProducerShort,
} from '@/lib/brew-cover-title'
...
export function BrewCard({ brew }: { brew: Brew }) {
  ...
  // displayVariety abbreviates the messy legacy titles (NAMING session) at the
  // render layer; canonical names stay intact in the taxonomy.
  const variety =
    displayVariety(brew.variety || brew.cultivar?.cultivar_name) || null
  const coverProcess = displayCoverProcess(brew)
  const producer = displayProducerShort(
    brew.producer || brew.green_bean?.producer,
  ) || null
  const region =
  ...
  return (
    <Link href={`/brews/${brew.id}`} className="brew-card">
      <div className="face" style={{ background: coverColor }}>
        ...
        {coverProcess ? <div className="process">{coverProcess}</div> : null}
