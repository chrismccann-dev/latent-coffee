...
- **Design-system priority-stack recount follow-ups** (2026-06-02). The page-by-page priority recount that produced the **priority stack** model (see [docs/design-system.md § Detail-page grammar](../docs/design-system.md)) surfaced a backlog of small IA/visual adjustments. Split into one bundled session + standalone tweaks.
  ...
  - **Standalone tweaks (simpler, do anytime):** collapse `green-bean-info` by default on all `/green` detail views; collapse Anchor Baseline by default on waiting-for-roast; relocate the out-of-place "Reference Recipe Design Intent" drop-rules block on resolved `/green`; recompose the resolved Reference Cup section to pull in the optimized-brew recipe card + tasted flavor/structure + peak expression (mirror `/brews/[id]`); paginate / cap the `/brews` index + per-roaster Coffees list as they grow (89 brews today); add a short "what is this process" description to `/processes` variant pages (nice-to-have, IA-nicer-not-urgent).
