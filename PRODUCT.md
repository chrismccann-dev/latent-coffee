...
**Last reorder:** 2026-04-22 (Variety sprint 1b shipped + Terroir research handed over by Chris, 1d unblocked). Variety side: 1b shipped — CanonicalTextInput enforcement on `/add` purchased review + `/brews/[id]/edit`; edit picker flips from 26-DB-row select to 63-entry canonical registry with lazy find-or-create; aliases resolve to canonical ("Geisha" → "Gesha"); `isResolvable` added to `makeCanonicalLookup`. Variety sprint-1 fully shipped except 1c (Observed corpus, Chris-driven authoring, deferred). Terroir side: Chris completed the same CSV research pass for country / admin_region / macro_terroir / meso + per-macro attributes (mirrors Variety 1a.1 research phase). Sub-sprint 1d is now the top Claude-driven candidate — same 3-phase shape as Variety: 1d.1 structural port, 1d.2 content backfill, 1d.3 enforcement.
...
**Sub-sprint 1c — Variety Observed corpus distillation (downgraded to side-quest, nice-to-have).** Per-cultivar "Observed Across My Corpus" subsections in `varieties.md` for ≥3-brew cultivars. Chris-authored, sample-size-aware voice, empirical layer (distinct from the external reference content that shipped in 1a.1). Not blocking anything; filed under Side-quests below for tracking.

**Sub-sprint 1d — Terroir (Region) port + backfill + enforce (NEXT Claude sprint, unblocked 2026-04-22).** Chris has completed the research phase — CSV covers country / admin_region / macro_terroir / meso + per-macro authored attributes, mirroring what the Variety CSV produced. Mirror the three Variety phases exactly:

- **1d.1 — Structural port** (mirror of 1a.1). Author `docs/taxonomies/regions.md` as the authoritative content source; refactor `lib/terroir-registry.ts` from today's 3 flat `CanonicalLookup`s (country / macro / meso) to a rich-shape registry along the lines of `CULTIVARS: CultivarEntry[]` — likely `REGIONS: RegionEntry[]` with `{ country, admin_region, macro_terroir, meso_terroir, ...attributes }` + per-tier lookup bundles preserved for back-compat. Apply a structural migration for any canonical renames / hierarchy changes surfaced by the research (mirrors migration 021). **Pre-sprint audit `rg TERROIR_REGISTRY` before starting** — Variety retro found 7+ inconsistencies between `lib/brew-import.ts` re-exports and the primary registry; likely the same shape here. `matchTerroir`'s multi-row prefer-admin-region / prefer-meso fallback (`lib/brew-import.ts:214-235`) needs to survive the refactor.
- **1d.2 — Content backfill** (mirror of 1a.2). Schema additions for any attribute columns Chris's research produced that the `terroirs` table doesn't yet have (elevation ranges exist; climate / soil / farming-model / cup-profile etc. likely need new columns). Data migration populates all existing DB rows from regions.md content. regions.md is authoritative; DB columns are materialization.
- **1d.3 — Enforcement** (mirror of 1b). Swap `/brews/[id]/edit` terroir `<select>` to `CanonicalTextInput` + find-or-create on PATCH. `/add` purchased review terroir card already has drift + tri-state confirm UX; replace macro_terroir datalist with `CanonicalTextInput(MACRO_LOOKUP)` + auto-populate admin_region/country from the registry entry when macro resolves. Apply the registry-as-picker pattern per `memory/feedback_canonical_registry_picker.md`. **Natural moment to extract `findOrCreateTerroir(supabase, userId, entry)` helper into `lib/brew-import.ts` alongside a `findOrCreateCultivar` extraction from Variety 1b** — flagged twice by /simplify in 1b; terroir find-or-create would be the third caller that tips the scale.

See [docs/features/reference-taxonomies-region.md](docs/features/reference-taxonomies-region.md) for the baseline feature doc (written 2026-04-21 pre-research). Expect CSV to scope ~2-3× the feature doc estimate per Variety's retro. Split structural vs. content migrations per the 021/022 precedent.

**Variety retro lessons that apply directly to 1d:**
- Plan-mode before editing. Investigate the current surface (`/add` purchased review terroir card + `/brews/[id]/edit` terroir select + `/api/brews/import` terroir resolution) before deciding the swap shape. Expect latent-broken discoveries (parallel to `/add` self-roasted step 8 finding during 1b).
- Pre-sprint audit `rg TERROIR_REGISTRY` to surface dual-registry drift in `lib/brew-import.ts` + anywhere else.
- Structural-only migration first (renames / hierarchy), content-backfill migration second. One PR can ship both if tight, but in the diff they're separate files.
- Edit form architectural shift: pre-1d.3, "pick from existing" means DB rows; post-1d.3, it means the canonical registry with lazy find-or-create. Rewrite the CLAUDE.md edit-brew note at the 1d.3 stage.
- `isResolvable` already landed as part of `CanonicalLookup` during 1b — reuse for the terroir save-gate.
- Memory pointer: read `memory/feedback_canonical_registry_picker.md` at the start of 1d.3.

**Follow-on sub-sprints (fire after Terroir lands):**
- **Sub-sprint 1e — Process research + port** (A2, ~20-25 processes, author from Sami). See [docs/features/reference-taxonomies-process.md](docs/features/reference-taxonomies-process.md). Same 3-phase shape once Chris completes research.
...
**Sizing:** 1a.2 was a half-sprint (content SQL). 1b was one sprint (two form surfaces + PATCH handler). 1d is ~2-3 Claude sprints split across the 3 phases (port / backfill / enforce). 1e-g similar shape to 1d.
...
- **d.** Dog-food against one backlog brew end-to-end. Find real friction, not speculated friction. **Pre-reqs flagged by step (c) dry-run:** (1) ✅ **resolved 2026-04-22 (PR #34)** — Mokka's genetic_family + lineage locked as Bourbon Family / Bourbon (classic) via candidate #1 Variety adoption; (2) decide whether Producer promotes from `warn` to `block`; (3) decide whether to extend `LABEL_ALIASES` first or accept manual stash during the sync.
...
- **`/add` self-roasted step 8 cultivar rebuild (surfaced 2026-04-22 during Variety 1b)** — the generic-textarea step has no parser and no structured fields; `parsedCultivar` is initialized null and never populated, so the save flow's cultivar branch is a no-op. The 4 existing SR brews got their `cultivar_id` from a pre-PR #25 historical path. Rebuild step 8 with structured inputs (Cultivar name via `CanonicalTextInput(CULTIVAR_LOOKUP)` + auto-populated species/family/lineage), wire the save flow to consume them via the same find-or-create logic as `/api/brews/[id]` PATCH. Natural moment to extract `findOrCreateCultivar(supabase, userId, canonicalName, entry)` into `lib/brew-import.ts` — the PATCH handler's ~55-line block duplicates `matchCultivar` + the create branch of `persistBrew`, flagged by /simplify in 1b. Third caller (SR save flow) tips the balance toward the shared helper. 1 sprint.
- **Variety Observed corpus distillation (Reference Taxonomies 1c, nice-to-have)** — per-cultivar "Observed Across My Corpus" subsections in `docs/taxonomies/varieties.md` for ≥3-brew cultivars: Gesha (22 brews, strong voice), 74158 (4 brews, moderate voice), Sidra (3 brews, tentative voice). Distinct from the external reference content shipped in sprint 1a.1 — this is the **empirical layer** distilling Chris's own 55-brew corpus in sample-size-aware voice. Chris-authored; Claude can surface raw material (synthesis outputs + `cultivar_connection` + `what_i_learned` fields) for review, but the writing is Chris's. Fires when Chris has focused authoring time; not blocking anything.
- **Multi-cultivar lineage render polish (flagged PR #36)** — `/cultivars/[id]` uses first-non-null merge ordered alphabetically when multiple canonical cultivars share a lineage. For `Ethiopian landrace-derived selection (non-JARC)` (14 canonicals, 5 DB rows), Pink Bourbon / Java / Rosado / Sudan Rume content is correctly stored per-row but shadowed by Ethiopian landrace population's content at render time. Fix is a per-cultivar subsection render within the lineage detail page, not a merge-strategy change (data is fine). No schema, no migration. 1 sprint.

### How to apply (operational hints for future sessions)

- **Chris's default next task:** hand the Terroir CSV over to Claude to kick off sub-sprint 1d.1. Separately, Variety Observed corpus distillation is a Chris-authored side-quest (nice-to-have, not blocking) — fires when focused authoring time is available.
- **Claude's default next task:** candidate #1 sub-sprint 1d (Terroir port + backfill + enforce). Chris has completed the research phase as of 2026-04-22; 1d.1 is unblocked the moment Chris pastes the CSV / markdown. Second choice: candidate #2 sub-sprint 1, step (d) — V1-brews sync dog-food. Third choice: `/add` self-roasted step 8 rebuild side-quest.
- **Candidate #1 sub-sprint 1d (Terroir port)** — apply Variety's retro lessons directly. Do NOT pre-populate `docs/taxonomies/regions.md` from training data — Chris owns the research, Claude vets + ports. Pre-sprint audit `rg TERROIR_REGISTRY`. Expect CSV to scope ~2-3× the feature doc estimate. Split structural vs content migrations. Read `memory/feedback_canonical_registry_picker.md` at the 1d.3 enforcement phase.
