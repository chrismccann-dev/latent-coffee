...
- **Responsive show/hide: prefer CSS classes over `matchMedia` hooks.** The mobile-polish sprint's filter-bar disclosure uses `hidden md:block` + React `useState` for the mobile toggle — desktop always renders the full filter bar via CSS, mobile toggles via state. The alternative (`useMediaQuery('(min-width: 768px)')` hook branching render paths) causes hydration flicker because the server can't know the viewport. The ~30 extra collapsed DOM nodes on mobile are trivial; the flicker-free SSR is not. When /simplify flags "unnecessary DOM" on a responsive disclosure, the finding is a false positive unless the component cost is real.
- **Run /simplify against the full file, not just the diff.** Mobile polish ended up extracting `<FilterTrigger>` because `/simplify` caught that the new mobile toggle button duplicated the existing `FilterPopover` trigger styling — the two sites were 170 lines apart in the same file and I missed the overlap during plan mode. Lesson: during plan mode, grep the target file for similar utility-class chains before writing new JSX; during /simplify, pass the full file to reviewer agents, not just the hunks.
