...
#### 5. Claude-Design-led redesign — ACTIVE (Sprints 0-5 shipped 2026-05-29: Foundation + Brew Detail + Green Cupping + Green waiting-for-roast + Green seam-closure + aggregation detail re-skin; **all detail surfaces** — `/brews/[id]`, all 5 `/green` views, AND the 4 aggregation detail families (roaster / cultivar / terroir / processes) — now `Ssp*`; `SectionCard`+`Tag` deleted; only the index surfaces remain)
...
- **Next: index surfaces** — the `/green` index, then `/brews` index, then the `/roasters` / `/cultivars` / `/terroirs` / `/processes` indexes — each its own plan-mode sprint on the now-stable token + primitive base. `/producers` + `/experiments` + homepage remain deferred per the scope doc.
