...
**Last reorder:** 2026-04-22 (Variety sprint 1a.1 shipped — PR #34. 63 canonical cultivars in `docs/taxonomies/varieties.md`, rich-shape `lib/cultivar-registry.ts`, migration 021, SYNC V1 step (d) Mokka pre-req resolved as side effect. Sub-sprints 1a.2 (content backfill) + 1b (enforcement) queued next under candidate #1).
...
**Sub-sprint 1a.2 — Variety content backfill (NEXT, Claude).** UPDATE migration against the 26 existing DB cultivar rows that populates the 18 attribute content columns (genetic_background, acidity_style, body_style, aromatics, extraction_sensitivity, roast_tolerance, roast_behavior, resting_behavior, brewing_tendencies, typical_origins, common_processing_methods, typical_flavor_notes, common_pitfalls, market_context, cultivar_notes, altitude_sensitivity, terroir_transparency, limiting_factors) from the CSV content now landed in `varieties.md`. Zero UI change; existing DB-driven render on `/cultivars/[id]` immediately shows the improved content. ~1 half-sprint (content is already mapped, just needs SQL authoring + review).

**Sub-sprint 1b — Variety enforcement (Claude, after 1a.2).** Swap `/add` cultivar select + `/brews/[id]/edit` cultivar input to `CanonicalTextInput` backed by the new `CULTIVAR_LOOKUP` (63 canonicals + 48 aliases). Closes the "strict enforcement bar" loop for the Variety feature doc. 1 sprint.
...
- **Claude's default next task:** candidate #1 sub-sprint 1a.2 (Variety content backfill) — UPDATE migration from varieties.md content into the 26 existing DB cultivar rows. Half-sprint. Followed by 1b (CanonicalTextInput enforcement). Alternative: candidate #2 sub-sprint 1, step (d) — V1-brews sync dog-food. The Mokka pre-req is now resolved; two remaining pre-reqs (Producer block promotion + LABEL_ALIASES extension) are still pending.
