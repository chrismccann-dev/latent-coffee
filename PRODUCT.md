...
#### 5. Claude-Design-led redesign — ACTIVE (Sprint 0 Foundation + Sprint 1 Brew Detail shipped 2026-05-29; per-surface sprints continue)
...
- **Next: companion surface 2** — `/green/[id]` waiting-for-cupping (mobile-primary, container-query dual-subtree — table→slot-cards at 520px), then roasters / cultivars / terroirs / processes, then the `/green` index — each its own plan-mode sprint on the now-stable token + primitive base.
