...
**Last reorder:** 2026-04-24 (**sub-sprint 1h.1 shipped**; 1m Roast level promoted to top Claude priority). **Sequencing:** ~~1i SR canonical rebuild~~ ✅ → ~~1h.1 Roaster structural port~~ ✅ → **1m Roast level** → 1h.2 Roaster enforcement (paired with /add work) → 1k Grinder → 1l Producer → 1f Brewer+filter → 1g Flavor (Chris-parallel) → 2.1 V1-brews dog-food → 2.2-2.6 V2 brainstorm + build.
...
- **Sub-sprint 1h.2 — Roaster enforcement on /add + add-new escape hatch.** Mirror of Variety 1b enforcement. Replace `<input>` on `/add` purchased review step with `CanonicalTextInput(ROASTER_LOOKUP)`; extract `findOrCreateRoaster` helper to `lib/brew-import.ts` for parity with cultivar/terroir helpers. New UX primitive: "add new roaster..." escape hatch that lets Chris save a brew with a non-canonical roaster name (with amber warning), preserving the strict-by-default discipline while honoring his note that adding a new roaster is more likely than adding a new cultivar. Pairs naturally with 1m's `/add` work since both touch the same purchased-review form.
