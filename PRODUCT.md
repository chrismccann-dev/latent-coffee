...
### Workflow rule: upload on resolution (both paths)

**Beans land in the app only after the lot is structurally closed** — a declared best brew (purchased) or a declared reference roast + replication (self-roasted). The iteration itself stays out of the archive.

Why: in-progress data warps aggregation. If a mid-iteration bean shows up on `/brews`, it weights lineage / terroir / process synthesis the same as finished work. The `/brews` count is a claim about *resolved* coffees, not "every bean I've tasted" — keep that boundary clean.

One deliberate exception exists in the history: SRH Washed was imported pre-resolution for the `/green/[id]` audit sprint (real data was needed to exercise the render pipeline) and then deleted afterward. That's the shape of an exception — scoped, explicit, reversed on completion.
...
| **/add** | Built | Self-roasted 9-step wizard end-to-end (Bean / Roast Log / Experiments / Cuppings / Learnings / Terroir / Cultivar / Brew). Purchased 4-step wizard (Bean / Terroir / Cultivar / Brew) via `/api/brews/parse`. |
...
- The roasting spreadsheet has 16 additional experiments tied to 5 CGLE / Forrest / Higuito beans whose green_beans rows have not been imported yet — these land as each lot closes, per the upload-on-resolution workflow rule

**/green/[id] render + data gaps — audited 2026-04-19, not yet remediated**
- Full audit lives at `/tmp/sprint-handoff/green-detail-audit.md` (20 prioritized gaps across P0–P5)
- **P0 display bugs:** `moisture` renders `10.0%%` and `density` renders `809 g/L g/L` — template appends units the schema already stores ([page.tsx:84-85](app/(app)/green/%5Bid%5D/page.tsx#L84))
- **P1 data loss:** `green_beans` schema has no column for `elevation`, `producer_tasting_notes`, or `exporter`; `cuppings` has no `sweetness` or `brew_method`; `roasts` has no `turning_point_temp`. All of these are populated in the source spreadsheet and are referenced in the roasting prose — they just don't reach the database.
- **P2 render gaps:** ROAST LEARNINGS renders 5 of 16 schema fields; `starting_hypothesis` (the output artifact of the whole experiment run) is invisible. ROAST LOG drops `fc_temp` + `drop_temp` (the two numbers the learnings call the primary lever) and every Roest graph URL.
- **P3 modeling:** `best_batch_id` is single-valued but Chris routinely tracks a reference + replication pair. `process = "Hybrid Washed"` falls into the "Other" family (not in `lib/process-families.ts`). Terroir `meso_terroir` is single-valued per macro even though Valle del Cauca has both Trujillo and Caicedonia.
...
- **Self-roasted flow end-to-end audit + `/add` flow hardening (PR #25)** — started as an observe-only audit of `/green/[id]` using SRH Washed (17 roasts / 6 experiments / 27 cuppings / full overall-lessons row) as realistic-shape test data; expanded mid-sprint when the audit surfaced a blocker in the `/add` self-roasted flow. Three commits landed. **020** one-shot SRH Washed import via SQL (exception to the upload-on-resolution rule, scoped to the audit and reversed after) + new `cuppings_user_roast_date_method_unique` constraint for idempotency + Python dedup of 27 copy-paste-duplicate cupping rows in the source spreadsheet. **/add flow parser fixes**: 20+ broken key lookups realigned in `parseGreenBeanSpreadsheet` + `parseRoastLogSpreadsheet` (the header normalizer collapses doubled/trailing underscores, so `row.roest_batch__` never matched `roest_batch`; every roast and every green-bean paste had been silently returning zero rows). Added 6 missing parsers for steps 4-9 (experiments / cuppings / learnings / terroir / cultivar / brew) — each step now has its own Parse button + summary-line feedback. `parseSpreadsheet` gets a positional `col_N` fallback key so the unlabeled "Observed Outcome D" column in the canonical experiments-tab export is readable. **Save-flow find-or-create fix**: terroir lookup had filtered only 3 of the 4 unique-constraint columns, which silently failed with `.single()` when two existing rows shared the 3-col subset; fixed to filter all 4 with `.maybeSingle()`. Normalized empty strings to null at insert time. Added a friendlier duplicate-lot_id error on green_beans insert. Audit deliverable at `/tmp/sprint-handoff/green-detail-audit.md` (20 prioritized gaps across P0-P5) scopes the next 1-2 sprints.
...
- **A feature that has never been exercised end-to-end is not a feature — it's an artifact.** The `/add` self-roasted flow shipped months ago and "worked" in the sense that the UI rendered, the buttons clicked, the save function existed. The SRH Washed audit sprint was the first time anyone pasted real spreadsheet data through all 9 steps and clicked Save & Finish. That single walkthrough revealed: 20+ broken parser key lookups (header normalizer collapsed doubled underscores but the consumers were still asking for `roest_batch__` instead of `roest_batch`), 3 completely missing parsers for steps 4-6 (textareas that saved nothing), a missing parser for step 9 brew, and a find-or-create bug that only fires when a user has 2+ existing rows sharing the lookup's column subset. **Before claiming a code path exists, walk it end-to-end with representative data.** The "it compiled and the UI renders" bar is not the "this works" bar.
- **Audits against real data catch gaps synthetic fixtures don't.** A 3-row test would have proven parsers *can* parse without proving they parse *your* columns. The SRH Washed corpus (17 roasts spanning 5 sessions with 3 different coffee-name spellings, a blank-header column for Observed Outcome D, multi-value best-batch strings like "#133 reference, #148 replication", 27 copy-paste-duplicate cupping rows in the source) stressed every edge case at once. Future audits should default to "import one real, finished bean" rather than "synthesize a fixture" — it's cheaper to delete after than to fake realism.
- **Save-flow find-or-create lookups must match the unique constraint exactly.** Terroir's constraint is `(user_id, country, admin_region, macro_terroir)`. The old lookup filtered only 3 of the 4 columns with `.single()`. When 2+ existing rows shared the 3-col subset (Colombia/Antioquia/Western Andean + Colombia/Valle del Cauca/Western Andean), `.single()` errored on "multiple rows found," the JS destructure swallowed the error into `data: null`, the code assumed no match existed, the insert fired, and Postgres threw the full 4-col constraint name at the user. Lesson: **mirror the unique constraint shape exactly in find-or-create queries, and use `.maybeSingle()` so zero-match returns `null` instead of throwing.** This pattern applies to every entity (green_beans, terroirs, cultivars) — whenever there's a "look up existing or insert new" flow, the lookup has to be ≥ as strict as the constraint.
- **Source spreadsheet data isn't clean — dedupe on import.** The Coffee Roasting Spreadsheet had rows 117-145 copy-pasted again at 195-220, giving 54 cupping rows for 27 unique cuppings. Migration 020's Python generator deduped on `(batch_id, cupping_date, eval_method)` before emitting SQL. **Every spreadsheet import should include a dedup step.** A `SELECT DISTINCT ON` pre-check on the source tuple is cheap; discovering doubled rows after they land is expensive.
- **"Observe + document" sprints have gravitational pull toward "observe + document + remediate."** The SRH audit was scoped observe-only. When Chris hit the `/add` blocker on step 3, we expanded to parser fixes. Then the save-flow bug surfaced on step 9, and we expanded to lookup fixes. Each expansion was legitimate — the blocker was squarely on the audit's critical path — but the PR grew from "one migration + one markdown file" to "one migration + one markdown file + a 400-line edit to `/add/page.tsx`." **Lesson: when an audit finds a blocker the user wants fixed inline, explicitly rescope the sprint before the third commit.** The alternative — a second PR for parser fixes — would have been cleaner, but at the cost of Chris sitting on a half-working flow. Scope expansion was the right call here; just call it out when it happens.
- **Normalizing spreadsheet headers for dict lookup: collapse underscores, strip edges, fall back positional.** The three things a tab-paste header normalizer has to handle: (a) consecutive non-alphanumerics collapse to one underscore (`"Dev %"` → `dev`, not `dev__`); (b) leading/trailing underscores are stripped (`"Reference Roast?"` → `reference_roast`, not `reference_roast_`); (c) empty or duplicate headers get a positional `col_N` fallback (the unlabeled "Observed Outcome D" column). Consumer code then needs to know the normalizer's output shape, not the raw header spelling — the first pass of `/add` parsers got this wrong on every field.
...
| 015-017 | Data-depth backfill: Guatemala terroir reclassification (015), cultivar canonicalization + Gesha consolidation + Garnica move (016), 9-lineage extended-field backfill (017). |
| 018 | Producer canonicalization: collapse drift to 49 canonical "Person, Farm" names; introduce `lib/producer-registry.ts` + `lib/canonical-registry.ts` factory. |
| 019 | Experiments import: backfill 18 experiments across 4 green_beans via `jsonb_to_recordset`; add `(user_id, green_bean_id, experiment_id)` unique constraint for idempotency. |
| 020 | Cuppings idempotency: add `(user_id, roast_id, cupping_date, eval_method)` unique constraint. (Shipped alongside a one-shot SRH Washed import for the `/green/[id]` audit; the bean itself was deleted post-audit per upload-on-resolution.) |
