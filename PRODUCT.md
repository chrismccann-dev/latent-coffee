...
**Sizing:** 1a.2 is a half-sprint (content SQL). 1b is one sprint (two form surfaces). 1d.1 was one sprint (structural port); 1d.2 half-sprint, 1d.3 one sprint. 1e-g similar shape.
...
- **Chris's default next task:** candidate #1 sub-sprint 1c (Variety Observed corpus distillation, deferred) or Region Observed distillation (same pattern, per-macro for ≥3-brew macros). Both are Chris-driven authoring passes with Claude vetting.
- **Claude's default next task:** candidate #2 sub-sprint 1, step (d) — V1-brews sync dog-food. Variety 1b + Region 1d.1 both shipped 2026-04-22. Candidate-#2-step-(d) pre-reqs: Mokka ✅ resolved (1a.1); Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Alternative: candidate #1 sub-sprint 1d.2 (Region content backfill) or 1d.3 (Region enforcement) — both Claude-ready. Side-quest `/add` self-roasted step 8 rebuild is also Claude-ready and scoped.
- **Candidate #1 sub-sprint 1d.2 / 1d.3 (Region content backfill + enforcement)** fires after 1d.1. Content backfill is one-shot SQL (mirror of Variety 1a.2); enforcement is the 3-file pattern (mirror of Variety 1b — cultivar_name flip on edit form, find-or-create on save).
