...
The full design system — token map, palette, type scale, component-primitive reference, and design/UX conventions — lives in **[docs/design-system.md](docs/design-system.md)** (single source of truth, shared with CLAUDE.md § Design / UX conventions). Extracted there 2026-06-02 (root-doc compaction) so both root docs point at one design reference instead of duplicating it. The code-level enforcement checklist (the rules wanted every UI session) is inline in [CLAUDE.md § Design / UX conventions](CLAUDE.md).

## Roadmap

The full forward-looking work surface, structured by readiness + scope. Per-sprint retrospectives live in `memory/project_*.md`. Sprint-by-sprint shipped log is at [docs/sprints/shipped.md](docs/sprints/shipped.md). **Grilling sessions** (the `/grill-with-docs` drains that resolve terminology / framing / queued decisions) are recorded per-session in `docs/sprints/grilling-<date>-*.md`, with the standing concept queue + append-only resolution history in [docs/grilling-queue.md](../grilling-queue.md) — those records live outside this doc to keep it from bloating; this pointer is their home in the roadmap.

**Last reorder:** 2026-06-02 (post-redesign/post-compaction roadmap review — retired the consumed 2026-06-02 working queue; Cluster B set NOW [2 of ~7 members done]; Lot Coordinator graduated to a brainstorm doc + promoted to NEXT/operator-gated; two MCP context items re-tagged as Lot-Coordinator prerequisites). Prior reorder 2026-05-26 (roadmap re-session — closes Sprint R Option 1 sequence; Sprints 3.4 / 3.6 / 3.7 killed; restructured into Writing-path + Read-path surface polish series + Brainstorms-to-schedule + Filter drawdown research closeout; Longer Term Items reordered per Chris audio). Prior reorder 2026-05-25 (Sprint R restructured the queue around Option 1 — audit → architecture → re-session). Prior reorder 2026-05-08 (post-v8.5 sweep).
...
- **Cluster A — cross-domain lot↔brew scaffold. [SHIPPED 2026-06-01]** Item 4 (cross-domain simulated-pour-over workflow + roast↔brew handoff) + MB-7 (attach reference brew) + roasted-variant-of-same-green modeling are ONE scaffold — the lot's web of linked brews (canonical reference brew / my other brews / peer-roasted variants) and keeping the roast↔brew loop intact across the claude.ai project boundary. **Shipped:** A1 schema (migration 075 `green_beans.optimized_brew_id` + embed FK-hints, #339/#340); A4 CONTEXT/ADR-0019 (#338); A2 brewing prompts (`simulated-pourover.md` + `peer-variant-completion.md` + `bundled-brewing-completion.md` optimized-brew carve-out); A3 roasting prompts (`log-cupping.md` SPG thin-packet emission + `eval_method:'Simulated Pourover'` flip + `close-lot.md` optimized-brew LINK path + `start-lot.md` peer-variant pickup) + Peer-Learning Roasting Archivist consumption layer + filed handoffs. Dogfooded live across optimized-brew backfill (9 lots), 3 peer variants (Janson/Wush Wush/Sudan Rume), 3 simulated pourovers (Gesha/Red Plum/Bukure). See shipped.md.
- **Cluster B — system-maintenance / anti-bloat layer.** Chris's insight: the Claude Code + Latent system side has had the least optimization, and nothing actively synthesizes/prunes/curates docs. Members: doc synthesis/pruning mechanism (**#1 scoped 2026-06-03** → [scope doc](../features/doc-pruning-mechanism-brainstorm-2026-06-03.md) + [tripwire registry](../architecture/doc-tripwires.md); protocol locked, systematization deferred until all 5 shapes seen; CONTEXT-roasting prune next), doc-size tripwire review (**CLAUDE.md compaction SHIPPED #352 — 152→38KB; PRODUCT.md 114KB under**; tripwires now consolidated in `docs/architecture/doc-tripwires.md` covering CONTEXT-* + claude.ai prompts too), the **claude.ai grilling review** (3-part: memory/instruction currency + cross-party claude.ai grill + context-window-usage check, roasting-side emphasized — fires at every grilling-session close; see `feedback_claude_ai_grilling_review`), operator review of CCIL/librarians, MCP continuous-feedback drain, codebase refactoring (new CC review/cleanup skills), feedback-handoff formalization. Interleave between builds.

**Ordering** _(superseded 2026-06-02 by the post-roadmap-review working queue in § Active Sprints — Cluster B is now NOW; Lot Coordinator graduated to NEXT/operator-gated; items 2a/2b below remain valid near-term work but now sit behind Cluster B + the Lot Coordinator gate)_**:**
1. ~~**NOW** — Cluster A scaffold~~ **SHIPPED 2026-06-01** (A1 #339/#340 + A4 #338 + A2/A3 workflow prompts + Peer-Learning archivist; see the Cluster A bullet above + shipped.md).
2. **NOW** (promoted from NEXT) — (a) Predicted-vs-Actual roast delta surface (cycle-time leverage; higher value, less defined); (b) Coffee-brief / `roaster_tasting_notes` scoping.
...
4. **Side quests** (completionist, not gated) — surface trio (`/producers` · `/experiments` · homepage; homepage also seeds public-surface v1) · skeleton / taxonomy content fill (Chris authors content): cultivar skeleton backfill + MCP cultivar-content path · **process content fill** (Honey base summary + any unfilled `BaseProcessEntry.summary` / `SignatureEntry.overview` / `brewArchetype` / `ModifierEntry.overview` + a lightweight honey-subprocess description map — only persist the honey process when the roaster actually states it) — prerequisite: extend `list_skeleton_entries` to a process axis so blank process content surfaces in the arbiter queue as a low-priority reminder (derive "blank = overview missing," no explicit flag; resync-grill 2026-05-31 decision) · experiments/cupping display polish on `/green/[id]` · auto-research surfacing on green upload · signature "what I learned" synthesis variant · data nits (#185 `best_batch_id` drift, `end_condition` backfill, green-helper dedup) · WBC champion corpus expansion · MEMORY.md consolidation (overdue).
...
**Ordered working queue (set 2026-06-02, post-roadmap-review):**
1. **Cluster B — system-maintenance / anti-bloat (complete the theme). NOW.** 2 of ~7 members shipped (migration-drift gate [#350](https://github.com/chrismccann-dev/latent-coffee/pull/350) + CLAUDE.md compaction [#352](https://github.com/chrismccann-dev/latent-coffee/pull/352)); remaining: doc-pruning *mechanism* (the meta-process, distinct from the one-off compaction) · operator review of CCIL/librarians · MCP continuous-feedback drain · codebase-refactor skills · feedback-handoff formalization · MEMORY.md consolidation (overdue). Chris-prioritized NOW (2026-06-02) — the most-neglected surface area; finish it before the Lot Coordinator restructure.
2. **Lot Coordinator + V-Set Assistant — NEXT, operator-gated.** Graduated to a brainstorm doc 2026-06-02 ([docs/features/lot-coordinator-brainstorm-2026-06-02.md](docs/features/lot-coordinator-brainstorm-2026-06-02.md)); roasting-only restructure mirroring [ADR-0017](docs/adr/0017-research-assistant-architecture.md). **Trigger:** Chris green-lights when the next *fresh* green-bean lot is ready to start clean on the new process (dogfood + design simultaneously — current lots are mid-cycle, can't be retrofitted). Plan-sprint shape: own ADR + CONTEXT-roasting grill + the three packet/brief templates. The big roasting-side sprint; brainstorm → plan → build, no build until the lot trigger fires.

_(Shipped 2026-06-02: ~~One-shot lot - emit the Optimized Brew Packet + LINK-not-push~~ ([#346](https://github.com/chrismccann-dev/latent-coffee/pull/346)) · ~~Repair the migration-drift gate~~ ([#350](https://github.com/chrismccann-dev/latent-coffee/pull/350)) · ~~CLAUDE.md root-doc compaction~~ ([#352](https://github.com/chrismccann-dev/latent-coffee/pull/352)) · ~~PRODUCT.md roadmap review~~ (this session) → see [shipped.md](docs/sprints/shipped.md).)_
...
#### 5. Claude-Design-led redesign — FIRST ARC COMPLETE (Sprints 0-6 shipped 2026-05-29: Foundation + Brew Detail + Green Cupping + Green waiting-for-roast + Green seam-closure + aggregation detail re-skin + index-surface re-skin; **every existing surface** — all detail pages AND all 6 index pages — now on the v2 chrome (`Ssp*` detail family + the `IndexList` index family); `SectionCard`+`Tag` deleted. `/producers` + `/experiments` + homepage remain deferred per the scope doc.)
...
- **Sprint 1 — Brew Detail re-skin — SHIPPED 2026-05-29.** First per-surface re-skin + first mobile-primary companion surface + first `Ssp*` consumer. `/brews/[id]` swapped from `SectionCard`/`RecipeTable`/`PourStructureList`/`CollapsibleBlock` to the `Ssp*` family inside `.ssp-page`; IA unchanged (already recipe-first T1–T4), mobile = plain single-column stack (no `order-*` needed). Ported the brew-tier + `.status` CSS Sprint 0 left out; added `StatusPill` + optional `tastedAs`/`detail` on `SspModifier`. Per Chris's call, did NOT force `SspFlavorAxis` (Latent's flat flavor structure is the source — flavors render as a green-`Chip` row, structure via `SspStructure`), which dropped the planned flavor→4-axis helper. `RecipeTable` + `PourStructureList` now dead (cleanup follow-up flagged). Verified at 390 + 1024 + a sparse brew. See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Sprint 2 — Green Cupping re-skin — SHIPPED 2026-05-29.** Second mobile-primary companion surface + **first container-query dual-subtree** in the codebase. `/green/[id]` waiting-for-cupping re-skinned to `Ssp*` inside `.ssp-page`: the desktop transposed Cupping Hypothesis + Roast Actuals tables (`SspExpGrid`) can't `order-*` into the mobile per-slot Taste-for cards, so `.s2-desktop`/`.s2-mobile` both render and `@container ssppage` reveals one at the **520px crossover** (content-box; ~552px viewport). Mobile critical path: T1 lavender Taste-for slot cards (lead) → T2 Primary Question + demoted Roast Actuals → T3 collapsed Reference & Detail. Two net-new primitives (`SspExpGrid` transposed grid + `SspProseRows`); ported the cupping CSS Sprint 0 left out (`.ssp-exp`/`.ssp-question`/`.ssp-why`/`.ssp-inset`/`.ssp-prose-rows`/`.ssp-tastefor`/`.ssp-predstack`/`.s2-*` toggle). IA preserved from Bundle B (2-row hypothesis table). Shared components (`GreenBeanInfoCard`/`RoastLogTable`/`CrossBatchNotesBlock`/`ExperimentFrameCard`/`PerRoastReflections`) stay legacy-skinned below the toggle through the migration window. Header tile goes lavender (cup-emphasis) per the artboard. The other 4 `/green` lifecycle shapes are untouched. See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Sprint 3 — Green waiting-for-roast re-skin — SHIPPED 2026-05-29.** Third companion surface (soft mobile-primary). `/green/[id]` waiting-for-next-roast re-skinned to `Ssp*` inside `.ssp-page`, **single tree (no dual-subtree)** — the hypothesis table is design-intent reference (fewer rows than cupping actuals), so the `.ssp-exp` `<520` narrow variant handles mobile without a transposed→slot-card recomposition. 6.3 IA preserved (Primary Question → Roast Hypothesis transposed table w/ variance-gated amber lever rows → Drop Rules → green-bean info → roast log). Header tile amber (`#A88037` roast-emphasis) per artboard `STATE["stage-1"]`. New builders `buildHypothesisData` + `buildDropRulesGrid` feed `SspExpGrid` + an inline amber `.ssp-inset` grid-mode Drop Rules (replacing the foreground `DropRulesCard` on this view); old `HypothesisTable` retired. Ported `.ssp-inset .inset-grid` CSS (Sprint 2 only ported `inset-stack`) + a `state-roast .ssp-shead` `<520` wrap rule so the context line clears the corner badge. Corner is bare `{vLabel}` (dropped the artboard's "· Gold Standard" for the 390 collision, matching the shipped Sprint 2 corner call). **Chris ratified "keep the seam"** in plan-mode — shared components stayed legacy-skinned below pending the dedicated green shared-components sprint (Sprint 4, below). Verified at 390 + 1024 (Fazenda Um - Wush Wush Natural). See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Sprint 4 — Green shared-components + remaining lifecycle shapes (seam closure) — SHIPPED 2026-05-29.** The promised seam-closure sprint: re-skinned the 6 shared `/green` components + the `CollapsibleSection` collapse substrate (→ `.ssp-coll`, re-skinned in place — used only by `/green`) + `InventoryPlaceholder`, **and** the 3 remaining shapes (`ResolvedView` / `UnresolvedView` / inventory), so **all 5 `/green` views land on `Ssp*` at once** and the migration-window seam Sprints 2-3 deliberately kept is closed. Two-PR scope split: PR1 = shared components + inventory + a regression sweep of the 2 already-shipped waiting views (the seam-closure proof — they inherit the re-skinned components); PR2 = the dense `ResolvedView` + `UnresolvedView`, collapsed into one shared `ArchiveLotBody({variant})` (−441 net lines — the two were ~95% duplicate). Extracted the shared `SspInset` primitive (grid/stack, rule-of-3). 6.5 / 4a IA preserved (chrome-only): Design/Achieved kept as two labelled `.ssp-roastspec` strips (contrast is load-bearing); Carry-Forward goes dark `.ssp-learnings` with scope-tag chips when populated. Ported the resolved/archive CSS (`.ssp-log`/`.ssp-roastspec`/`.ssp-insights`/`.ssp-pair-rows`/`.ssp-learnings`/`.ssp-cite` + `state-archive` corner) + extended the `<520px` shead-wrap rule to `state-resolved`/`state-archive` (corner/ct 390 collision, generalized). Deleted dead `RecipeRow`/`CharacterCard`/`LearningRow` + `SectionCard`/`StrategyPill` imports. Verified at 390 + 1024 (resolved CGLE Sudan Rume Hybrid Washed + unresolved Higuito + the cupping/waiting-roast regression sweep). See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Sprint 5 — Aggregation detail re-skin — SHIPPED 2026-05-29.** The last un-migrated detail surfaces: all 4 aggregation detail families (roaster / cultivar / terroir / processes — 8 page files) swapped to `Ssp*`, IA preserved (chrome-only). 4-PR split (PR1 foundation + roasters / PR2 cultivars / PR3 terroirs / PR4 processes). PR1 landed the shared aggregation chrome that amortizes across all 4: new `CoffeesList` (book-cover rows + 10+expander; `metaFor`/`showProcessBadge`) + `ConfidenceCard` (`.ssp-card.dark`, shared `confidenceFor`, `desc` override) + re-skinned `CollapsibleBlock` (`.ssp-coll`) / `FlavorNotesByFamily` + `TagLinkList` (`.ssp-sub` `Chip` blocks) / `SynthesisCard` (`.ssp-card`+`SspShead`, kept the `md:` short/long split) + the `compactRows` helper. PR4 closed the legacy chrome family: **deleted `SectionCard` + `Tag`** (zero consumers app-wide) + the process-only `ProcessCoffeesList`/`ProcessConfidenceCard`/`ProcessBreakdownRow` (subsumed by `CoffeesList`/`ConfidenceCard`/`SspStructure`; process breakdown rows via shared `lib/process-breakdown.ts`). Desktop-primary reference surfaces — mobile must-not-regress, no dual-subtrees. Verified each surface @1024 + 390. See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Sprint 6 — Index-surface re-skin — SHIPPED 2026-05-29.** The 6 index pages re-skinned to the v2 "legacy" index family (a net-new `components/IndexList.tsx` — `IndexCap` / `GrlCap` / `GrlGroupHeader` / `GrlRow` + `barBlocks` + `LotStage` — distinct from the detail-page `Ssp*`), closing the **first redesign arc**. 3-PR split: **PR1** ([#313](https://github.com/chrismccann-dev/latent-coffee/pull/313)) = foundation (`.cap`/`.grl-*`/`.lot-stage` CSS + `IndexList.tsx`) + `/green` index (re-skinned the flat lifecycle list, NOT the v2 lot-card grid — Chris's call); **PR2** ([#314](https://github.com/chrismccann-dev/latent-coffee/pull/314)) = the 4 aggregation indexes (`/roasters` `/terroirs` `/cultivars` `/processes`) on the shared grouped-row-list (`GrlRow` + 5-block count bar; cultivars kept the Species→Family→Lineage nesting NOT the v2 text-tree; processes kept its 3 nav surfaces); **PR3** = `/brews` (the v2 **paper-foot** book-cover card — `components/BrewCard.tsx` — superseding the all-content-on-cover rule; filter bar + server round-trip preserved). Three interpretive source-vs-brief calls resolved with Chris in plan-mode (green=flat-list, brews=paper-foot, cultivars=grouped-list). Chrome-only, IA preserved; desktop-primary, verified @1024 + 390. `/simplify` extracted `GrlCap` + flattened the count/max reduces. See [docs/sprints/shipped.md](../sprints/shipped.md).
...
- ~~**Doc pruning mechanism.**~~ **RAN 2026-06-03** (Cluster B member #1) → [scope doc](../features/doc-pruning-mechanism-brainstorm-2026-06-03.md). From Group D Round 2 audio. Outcome: protocol defined + tripwires placed, systematization **deliberately deferred** until all 5 pruning shapes (extract / split / consolidate / archive / delete) have a lived worked example. Shipped this session: [doc-tripwires.md](../architecture/doc-tripwires.md) registry (tripwires now cover root + CONTEXT-* + claude.ai prompts at a tighter 40KB cap); [pruning-cases/](../sprints/pruning-cases/) structured-handoff home + `001` seed; retroactive worked examples filed (#352, #244); delete = flag-only. **Next:** CONTEXT-roasting manual prune (live fire 115.8/120KB — the deliberate consolidate/delete hunt; kickoff `docs/sprints/context-roasting-prune-kickoff-2026-06-03.md`), then ≥1 more shape before deciding to systematize.
- **Lot Coordinator + V-Set Assistant subagent architecture (roasting-only).** **GRADUATED 2026-06-02** → brainstorm doc [docs/features/lot-coordinator-brainstorm-2026-06-02.md](docs/features/lot-coordinator-brainstorm-2026-06-02.md); promoted to Active Sprints § Ordered working queue #2 (NEXT, operator-gated on next fresh-lot intake; Cluster B completes first). Brainstorm converged: V-set is the assistant container; Coordinator/Assistant boundary = predicted/actual boundary; Cluster A is already the brewing-handoff half; MCP shared state keeps packets thin; lot plan = "Roasting Brief" Coordinator-side; scope locked roasting-only. Original framing retained below for provenance. From Chris audio 2026-06-02 (during the one-shot Optimized Brew Packet dogfood). **The load-bearing context that was never formalized:** the roasting side runs as ONE claude.ai session per green-bean lot - the entire lifecycle (initial recipe design → V-set roast → cupping → iterate → reference-roast call → learnings synthesis) lives in a single thread. This predates Latent (carried from the pre-app spreadsheet era, where a single session held the whole lot) and is a constant on the claude.ai surface Claude Code couldn't see. It's also the likely root cause of cross-domain handoff verbosity (e.g. the rich-vs-thin Optimized Brew Packet, 2026-06-02): the session carries the lot's entire iterative history "in its head" and can't easily compress to a thin packet. Contrast: brewing is already thread-per-brew, short-lived (3-4 iterations, resolve, new thread). **Brainstorm:** restructure roasting as a master-coordinator + spawned-assistant pattern mirroring the **Research Coordinator + Research Assistant** ([ADR-0017](docs/adr/0017-research-assistant-architecture.md)) - a **Lot Coordinator** session holds the lot-level plan; a **V-Set / Experiment Assistant** is spawned per V-set, takes that set from roast → cupping → what-it-learned, STOPS (no implementation), and returns a clean handoff document the coordinator consumes to plan the next set. Mitigates single-session context bloat + keeps handoffs thin (handoff-doc discipline already proven on the research + cross-domain-packet surfaces). **Open scope for the brainstorm:** (a) roasting-only, or also brewing (brewing's short threads may need only a light touch or nothing); (b) how it composes with the existing operational prompts (`start-lot` / `log-roast` / `log-cupping` / `close-lot` + the one-shot pair) and the Master Coordinator; (c) the operator-courier handoff shape between Lot Coordinator and V-Set Assistant; (d) walk the full roasting workflow end-to-end as part of the brainstorm. **Big sprint - brainstorm → plan → roadmap before any build.** Chris explicitly: don't over-design now. Until it lands, packet verbosity is **accepted** (it's a symptom of the single-session model; tightening packet templates fights the symptom, not the cause).
...
- **`read_canonical` name filter (MCP context-efficiency).** `read_canonical` returns the full axis registry (flavors ~20K tokens, ~5% used) — the #1 context-window sink in roasting/brewing claude.ai sessions, surfaced empirically in the 2026-05-31 claude.ai grilling review (Component 3). Both project memories already *document* `read_canonical(axis, name)`, so the fix is to implement the documented behavior: add an optional `name` (consider `names[]`) param that resolves through the axis's canonical + alias lookup and returns just the resolved entry(ies) + aliases; omitted = current full-registry behavior (back-compat). Spawned as a task 2026-05-31. Additive, low-risk; closes the biggest payload cost AND makes the docs true. (The separate axis-count description bug — "12 axes" vs the real 13 incl `hybrid-subforms` — was fixed independently in #335.)
- **`get_bean_pipeline` incremental fetch (`since:`).** By mid-V-set the single pipeline response exceeds all prompt-fetched docs combined (REDPLUM self-audit, 2026-05-31 grilling review); MCP payloads are the dominant context cost on long roasting arcs. A `since:<timestamp>` param returning only newer-than-last-pull rows would cut the largest accumulation category. Pairs with the `read_canonical` name filter as the two highest-leverage roasting context-window wins. **Both re-tagged 2026-06-02 as Lot-Coordinator prerequisites** — they keep Coordinator↔V-Set-Assistant handoff packets thin (a fresh assistant reconstructs its input from a `since:` pipeline pull rather than a fat courier paste); see [docs/features/lot-coordinator-brainstorm-2026-06-02.md § MCP is shared state](docs/features/lot-coordinator-brainstorm-2026-06-02.md).

### Bugs and Issues

- **Missing lifecycle state: "pourover packet handed off, awaiting brew-side completion" on `/green`** (surfaced 2026-06-02 design-system recount — Mountain Harvest one-shot lot; **deferred to the Lot Coordinator sprint per Chris audio 2026-06-03, kept as-is for now**). The Mountain Harvest mis-bucket (a `green_beans.is_one_shot=true` lot, cupped, `roast_learnings` still NULL, rendering under "Waiting for next roast" because `computeLifecycleState` rule (f) routes a fully-cupped experiment back to waiting-for-next-roast) is **one instance of a broader missing-state gap**, not an isolated bug. The lifecycle has `waiting_for_next_roast` and `waiting_for_next_cupping`, but **no state for "I emitted a pourover packet to the brewing side and am waiting for the brew-side task to finish before the next roasting-side step."** Two variants, both real today:
  - **One-shot:** cupping emits the **optimized-pourover packet** → operator runs the brew on the brewing side → result comes back as the signal to run close-out (one-shot-closeout → `roast_learnings`, then resolved/unresolved). Mountain Harvest is here now (will close as **unresolved** — it was a one-shot and didn't nail it). No state for the wait.
  - **V-set:** an iteration emits the **simulated-pourover packet** → operator runs a brew-side runoff → result comes back to either declare a **reference roast** or go forward with another iteration. No state for that wait either.
  - **Decision (Chris, 2026-06-03):** these are **state-shaped, not design-shaped**, and the **[Lot Coordinator + V-Set Assistant sprint](#active-sprints)** (NEXT, operator-gated) will touch all these surfaces — so do **not** patch the lifecycle derivation now (it'll probably change). Keep current behavior. Address both missing-wait states as part of that sprint's lifecycle/state model. Cross-ref the Lot Coordinator brainstorm doc.
...
- **Section-read navigation pattern across CONTEXT zones + prompt surfaces** (flagged 2026-06-03, post pruning case 002). CONTEXT-roasting now carries Reading-order + Agent-term-index scaffolding so agents can `read_doc_section` against the index instead of full-reading the glossary — the section-read win that justifies the 120 KB section-read budget (vs the tighter 40 KB full-read prompt cap). Make the substrate actively route to it: (1) point the claude.ai/projects/roasting instructions at the Reading-order section; (2) **per-prompt preload** — each prompt names the cluster it needs first and pulls only that section (`log-cupping` → Cupping + V-set close, `close-lot`/`one-shot-closeout` → Lot-close synthesis, `log-roast` → Roast diagnostics, `start-lot` → V-set design + Core ontology), proactive without re-bloating the prompt; (3) replicate the Reading-order + term-index scaffolding into CONTEXT-brewing + CONTEXT-shared; (4) a first-reader pointer at the CONTEXT index so an agent unsure of a term hits the index → knows which zone to hunt, anticipating the workflow. **Triggers when:** step 1 (roasting instruction pointer) is validated in a live claude.ai roasting session; then fan out to prompts, then brewing + shared.
...
- **Design-system priority-stack recount follow-ups** (2026-06-02). The page-by-page priority recount that produced the **priority stack** model (see [docs/design-system.md § Detail-page grammar](../docs/design-system.md)) surfaced a backlog of small IA/visual adjustments. Split into one bundled session + standalone tweaks.
  - **Bundled follow-up session — SHIPPED 2026-06-03.** Items 1 + 2 landed; item 3 reframed + deferred. See [docs/sprints/shipped.md](../sprints/shipped.md).
    1. ✅ **Recolor `/brews` book-cover by extraction strategy.** Cover hue now delegates to `lib/extraction-strategy.ts` (`getStrategyCoverColor` → each strategy's `border` signal hue, same one outlining the pill) so cover + pill read as one hue family; no-strategy brews fall back to neutral slate. Extraction-push uses its existing burnt-orange (distinct from full-expression's wine) per the source-of-truth palette. Touched `lib/brew-colors.ts` + `lib/extraction-strategy.ts`; propagates to the aggregation-page `CoffeesList` swatches too. Preview-signed-off @1024 + 390.
    2. ✅ **Reorder `/roasters/[id]` to its priority stack.** Coffees list promoted to primary (leads after hero); roaster-info blocks secondary; synthesis dropped to collapsible tertiary via a new `SynthesisCard collapsible` prop (reuses `CollapsibleBlock`). Touched `app/(app)/roasters/[slug]/page.tsx` + `components/SynthesisCard.tsx` + `docs/architecture/page-ia.md`. Preview-signed-off @1024 + 390.
    3. ⏸ **One-shot lifecycle mis-bucket — reframed + deferred to the Lot Coordinator sprint** (Chris audio 2026-06-03). Not a small fix: it's one instance of a broader missing "awaiting brew-side completion" state gap (one-shot + V-set variants). Code left as-is; documented in § Bugs and Issues + the Lot Coordinator brainstorm open-questions.
  - **Standalone tweaks — RESOLVED 2026-06-03.** Tweaks 1-4 + 6 **SHIPPED** (1+2 / 3+6 / 4 across three PRs); Tweak 5 **split: per-roaster Coffees list already handled** (`CoffeesList` 10-item expander), **`/brews` index deferred** to the ~100-brew tripwire (now codified in § Scaling Watch-Items). No open recount-tweak work remains.
    - **Tweak 1-2 — SHIPPED.** Collapse `green-bean-info` by default on all `/green` views (done in the shared `GreenBeanInfoCard` so it propagates to all 5 lifecycle shapes); collapse Anchor Baseline by default on waiting-for-roast (in-place `<details>` disclosure).
    - **Tweak 3 — SHIPPED.** Relocated the "Reference Recipe Design Intent" drop-rules block on resolved/unresolved `/green` down into the de-emphasized archive-detail cluster (between Per-Roast Reflections and All Cuppings, near the roast log) — it's just that roast's drop rules, so it no longer interrupts the roast→cup reading order.
    - **Tweak 6 — SHIPPED.** Added a "Process Description" card to the `/processes` variant pages that lacked one — base+modifier combo pages + Honey subprocess pages (new `MODIFIER_COMBO_OVERVIEWS` / `HONEY_SUBPROCESS_OVERVIEWS` maps + getters in `lib/process-registry.ts`) and the 3 signature pages (`SignatureEntry.overview` populated for Moonshadow / TyOxidator / XO). Chris authored all text. Honey-generic = base Honey (same description, titled "Honey"). **Flagged follow-up (Actor 2/3, not built):** Chris's convention "a coffee defaults to generic Honey unless the bag/producer/roaster names a specific honey tier" belongs in the claude.ai write-path prompts + `push_brew` honey-subprocess defaulting — verify that path actually defaults to Generic Honey.
    - **Tweak 4 — SHIPPED** (built from Chris's mockup). Recomposed the resolved/unresolved Reference Cup card into one vertical narrative: **Cupping** (table read) → **Optimized Brew** (full `/brews/[id]`-style label/value grid: Recipe / Strategy / Flavor Notes chips / Structure chips / **Learned** = the brew's `what_i_learned`, moved in from the old floating top synthesis block) → **Producer Notes** (was a dashed `.ssp-cite` footer, now its own pane). New `.ssp-optbrew` grid (stacks at the 520px crossover). **Judgment calls (flag if wrong):** (a) followed the mockup and did NOT add a separate peak-expression row even though the original brief mentioned peak — easy to add back; (b) kept the cupping pane's extra fields (sweetness / aromatic+structural behavior) since Chris said "keep the components," so richer lots show more than the mockup's minimal cupping example.
    - **Tweak 5 — RESOLVED (split).** Per-roaster Coffees list: **already done** (`CoffeesList` caps at 10 with a "Show N more" expander). `/brews` index: **deferred** — non-issue at 89 brews (server-rendered, no client list JS); revisit at ~100 brews as its own mini-sprint, likely "show first N + load more" on the unfiltered view only (filters already narrow). Tripwire codified in § Scaling Watch-Items.
  - **Roasting-workflow, not app:** drop-rule slow-condition should carry a hard max cutoff time (~6 min) — Chris currently infers it. Belongs in how claude.ai/roasting *writes* drop rules (roasting prompt / drop-rules protocol), not a design-doc or app change.

- **Standing — needs Chris's design thinking (parked, not yet scoped):** these three came out of the same recount but are bigger than tweaks.
  - **Synthesis surface rethink.** Across every aggregation detail page the recount demoted synthesis below the entity-info block *because it's too long*, not low-value (intended-priority-vs-render-weight mismatch). Fix is shortening/restructuring synthesis so its weight matches its secondary rank, not reordering. Corroborates prior "rethink synthesis" flags (and the three-LLM-call pipeline / short-form work).
  - **Navigation reorg by actual usage.** Chris's lived nav: **brews / roasters / green** are the primary destinations; **terroirs / cultivars / processes** are rarely visited (secondary). Options floated: reorder the desktop nav by usage, or use the hamburger even on desktop. Reinforces roaster's status as a navigation spine, not a reference page.
  - **Brew-card recall mismatch (variety vs bag name).** `/brews` cards show the variety/coffee name; `/roasters/[id]` shows the roaster's bag name; Chris recalls coffees by bag name ("Picolot → Loud Giants"), which is why he still falls back to the roaster page for older coffees. Consider surfacing the bag/coffee name on brew cards — interacts with producer prominence + card density, so it needs thought, not a quick edit.

- **Brew-list/card component unification** (Chris-flagged Sub-sprint 4e, 2026-05-28). **Detail-page half RESOLVED in Redesign Sprint 5 (2026-05-29):** all 4 aggregation detail families now import one shared `components/CoffeesList.tsx` (book-cover rows + meta via a `metaFor` prop + `showProcessBadge`) instead of re-implementing the row markup — exactly Chris's "all of them pull from the same place." **Index-page half RESOLVED in Redesign Sprint 6 (2026-05-29):** the 4 aggregation indexes now share one `GrlRow` primitive (`components/IndexList.tsx`) and `/brews` + `/green` reuse the same family (`BrewCard` / `GrlRow` simple variant). The `/brews` book-cover grid intentionally stays a distinct shape from the aggregation rows (grid of covers vs. list of rows — a genuinely different shape, flagged + kept in plan-mode). **CLOSED** — both halves of the unification done.
...
- **`/brews` index unbounded render (~100-brew tripwire).** The `/brews` index renders every brew as a `BrewCard` with no cap/pagination (`brewList.map(...)` in `app/(app)/brews/page.tsx`). At 89 brews today this is a non-issue — server-rendered, no client list JS, just scroll length — so it was deliberately **deferred** out of the 2026-06-02 priority-stack recount (Tweak 5). **Action when crossing ~100 brews:** spin a small mini-sprint. Likely fix is "show first N + load more" on the *unfiltered* view only (the cross-dimensional filters already narrow the list, so heavy pagination would over-engineer and fight the scroll-to-recall habit). The **per-roaster Coffees list half of Tweak 5 is already handled** — `components/CoffeesList.tsx` caps at `VISIBLE_LIMIT = 10` with a "Show N more" `<details>` expander.
