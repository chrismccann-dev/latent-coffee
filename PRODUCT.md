...
| `roaster` | `lib/roaster-registry.ts` | 🔶 64-canonical CSV authored; adoption pending | **1h** |
...
| `roast_level` | `lib/roast-level-registry.ts` (new) | ❌ free-text; canonical 4-5 bucket scale + Agtron ranges + synonyms not yet captured | **1m** |
...
- **Sub-sprint 1i — `/add` SR canonical rebuild (terroir + cultivar)** — top Claude priority. The self-roasted `/add` flow is the one UI surface still not wired to the canonical registries post-1b + 1d.3. Step 7 (terroir) + step 8 (cultivar) both use generic textareas with no parser, no structured fields, no find-or-create. Chris's workflow with full taxonomies landed is: pick canonical cultivar + pick canonical macro, everything else flows from the registry entry. Rebuild step 7 with `CanonicalTextInput(TERROIR_COUNTRY_LOOKUP)` + `CanonicalTextInput(TERROIR_MACRO_LOOKUP)` + free-text meso; step 8 with `CanonicalTextInput(CULTIVAR_LOOKUP)` + auto-populated species/family/lineage. Wire `handleSaveSelfRoasted` to consume via `findOrCreateCultivar` + `findOrCreateTerroir`. 1 sprint.
- **Sub-sprint 1h — Roaster taxonomy adoption.** Chris completed a roaster reference taxonomy (64 canonicals + richer field set vs the current 21-canonical [lib/roaster-registry.ts](lib/roaster-registry.ts); ~80% coverage of roasters Chris encounters in the wild). Mirror of Variety 1a.1 / Region 1d.1 / Process 1e.1 structural port shape, but against an existing canonical registry rather than a greenfield one. Steps: port authored content → `docs/taxonomies/roasters.md`; refactor `lib/roaster-registry.ts` to rich `RoasterEntry[]`; decide alias handling (Sey / Sey Coffee class); update `/roasters` + `/roasters/[id]` consumers; decide structural vs content migration split. 1 sprint.
- **Sub-sprint 1m — Roast level taxonomy** (new, scoped 2026-04-24). Chris flagged as "straightforward" — canonical 4-5 bucket scale (Light / Medium-Light / Medium / Medium-Dark / Dark) + Agtron color-reading ranges per bucket + alias map covering the common specific-word variants (City+, Full City, Vienna, etc.). Currently `brews.roast_level` is free-text with drift. No research dependency; Chris can hand-author in the sprint itself. New `lib/roast-level-registry.ts` + `docs/taxonomies/roast-levels.md`. Likely 1 sprint including adoption on `/add` + `/brews/[id]/edit`.
...
- **Claude's default next task (recommended):** **sub-sprint 1i — `/add` SR canonical rebuild**. Closes the last `/add` UI surface where the canonical-picker pattern is missing. One sprint, mirror of Variety 1b + Region 1d.3 applied to SR steps 7 + 8. Tips `persistBrew` toward the `findOrCreate*` helpers as a third caller.
