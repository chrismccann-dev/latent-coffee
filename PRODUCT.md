...
**Last reorder:** 2026-04-22 (Region sprint 1d.3 shipped — CanonicalTextInput enforcement on `/add` + `/brews/[id]/edit` macro_terroir inputs backed by `TERROIR_MACRO_LOOKUP` (121 canonicals + 12 aliases) + `TERROIR_COUNTRY_LOOKUP` (38 countries). PATCH resolves `terroir_name` + `country` → `terroir_id` via new `findOrCreateTerroir` helper with `admin_region` auto-populated from `getTerroirEntry()` on create. `findOrCreateCultivar` + `findOrCreateTerroir` extracted into `lib/brew-import.ts`, collapsing ~55 lines of inline PATCH code from Variety 1b. Meso free-text pass-through. Region slice fully shipped (1d.1 + 1d.2 + 1d.3); Variety slice fully shipped earlier today (1a.1 + 1a.2 + 1b). Reference Taxonomies umbrella remaining: Process 1e, Dripper 1f, Flavor 1g. Next Claude candidate: SYNC V1 step (d), or Process 1e if Chris hands over research.
...
- **Claude's default next task (recommended):** **candidate #2 sub-sprint 1 step (d) V1-brews sync dog-food**. Both strict-enforcement gates are now live (Variety 1b + Region 1d.3 — aliases resolve to canonical, unresolvable inputs rejected), so sync inputs are consistent with the canonical surface. Pre-reqs: Mokka ✅ resolved (Variety 1a.1); Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Side-quest `/add` SR step 8 rebuild is also Claude-ready and would tip `persistBrew` toward the new `findOrCreate*` helpers (third caller).
- **Claude's alternative task:** **sub-sprint 1e — Process research + port** (A2, ~20-25 canonical processes, author from Sami). First A2 taxonomy port; will exercise the scripts that A1 ports built. Harder than 1d.3 because it lacks a pre-existing CSV — Chris needs to do the research first.
