...
- **Sub-pages first-principles rethink series (Sprints 2-6)** — scoped 2026-05-09 as a follow-up to the brew detail sprint ([PR #131](https://github.com/chrismccann-dev/products/pull/131)). Same playbook as Sub Pages 1: directed informational rethink per page, stay inside existing chrome (`<SectionCard>` / `<Tag>` / `<TagLinkList>` / `.label`), no new design tokens, surface buried fields, mobile-collapse where helpful. Each sprint is its own session; Chris brings source material (mocks / pain points / written job-to-be-done framing) at kickoff per the [feedback_ask_before_implementing.md](~/.claude/projects/-Users-chrismccann-latent-coffee/memory/feedback_ask_before_implementing.md) rule.
  ...
  - **Sub Pages 6 — Roasting-side rebuild (6-sub-sprint series, scoped 2026-05-13).** Scope expanded mid-kickoff from a single green-bean detail rethink to a roasting-side rebuild covering 4 lifecycle states (in-inventory / waiting-for-next-roast / waiting-for-next-cupping / resolved), a new `roast_recipes` first-class entity, 16 cross-batch fields on `experiments`, MCP-only writes (deprecating `/add` for roasting), and 3 new state-driven page shapes at `/green/[id]`. Series-level scope doc lives at [docs/roasting/redesign.md](docs/roasting/redesign.md) (530 lines, Chris-authored hand-off). Cross-sprint decisions locked via AskUserQuestion at kickoff: sequenced series (not mega-sprint); `roast_learnings.best_batch_id` → typed `best_roast_id` FK; per-batch Hypothesis prose → new `roast_recipes.rationale` field; `/add` deprecation = sub-sprint 6.6.
    ...
    - **Sub Pages 6.2 — Index page (3 lifecycle sections)** ✅ Shipped 2026-05-13. New [lib/lifecycle-state.ts](lib/lifecycle-state.ts) helper computes state on read (`computeLifecycleState(bean)`) returning one of 4 states; resolution order codifies scope-doc § 8 verbatim + two edge cases the doc doesn't explicitly name (pre-framework legacy lots with roasts but no experiments → `waiting_for_next_roast`; synthesis-done-but-V_(n+1)-not-designed → also `waiting_for_next_roast` per the loop framing in § 3). Cupping gate uses scope-doc disjunction — either `cupping_count == 0` OR `winner == null` routes to `waiting_for_next_cupping` (so Red Plum / Fazenda / Bukure surface there even though their experiments are synthesized — the page tells the user "go log the cuppings"). All 13 live lots route correctly (4 next-roast / 3 next-cupping / 6 resolved). [app/(app)/green/page.tsx](app/%28app%29/green/page.tsx) rewritten as flat sectioned list (no card chrome) — section header + right-aligned "Stage"/"Reference" column + tile + name + metadata + stage label per row. Tile color signals lifecycle: sage (`bg-latent-accent-light`) for active, near-black (`bg-latent-fg`) for resolved — hue shift = green-to-roasted, fits hue-not-lightness rule, no new tokens. Resolved rows surface `Batch #${best_batch_id}` with prefix-strip normalization so `"Batch 139"` renders as `Batch #139`. In-inventory lots intentionally hidden (waiting for the eventual inventory page per scope doc § 5.1 punt). Tabler `ti-leaf` icon swap deferred to 6.3 — not load-bearing for the index (no leaf icon in the index mockup), lands with the detail-page hero rewrite. Empty state preserved through 6.6 (still links to `/add?type=self-roasted`). Verified end-to-end via preview server desktop + mobile 375px (truncation clean, no horizontal scroll). `npx tsc --noEmit` + `npm run build` both pass.
    - **Sub Pages 6.3 — Waiting-for-next-roast page shape.** Roasts · V_n card with Primary Question + Roast Hypothesis transposed table (varying-row amber highlight) + Drop Rules amber card + Green Bean Info + Roast Log (current-experiment rows highlighted) + Additional Information collapse. Surfaces the previously-invisible per-roast `what_worked` / `what_didnt` / `what_to_change` (99/92/91% populated today, all invisible) + the 8 invisible experiment fields (`context` / `control_baseline` / `shared_constants` / `levels_tested` / `expected_outcomes` / `failure_boundary` / `observed_outcome_a-d`).
    - **Sub Pages 6.4 — Waiting-for-next-cupping page shape.** Cupping Hypothesis · V_n card (transposed table of Original prediction / Updated prediction / Taste for, per batch) + Reference signals card + Roast Actuals · V_n card (FC / Drop / vs-expected / Dev time / Maillard / Agtron WB per batch with amber `+5s overran` delta callouts) + Green Bean Info + Roast Log + Additional Information.
    - **Sub Pages 6.5 — Resolved view page shape.** Reference Roast card (green-tinted "Why this roast won" verdict + 2-col recipe design vs achieved values) + Reference Cup card (Best cup synthesis + Cupping / Optimized Brew side-by-side + producer notes) + Roasting Learnings: {lot} card (3 character cards + detail rows) + Roasting Learnings: To Carry Forward card (cultivar / general / starting hypothesis) + Green Bean Info + collapsed Roast log / All cuppings / Experiment journey + Additional Information.
    - **Sub Pages 6.6 — /add self-roasted deprecation.** Removes the `/add?type=self-roasted` flow. Header `+ ADD` button surgically narrows to brewing-only. Prompts in `docs/prompts/*.md` (new-bean-intake / design-next-experiment-set / etc.) update to call the new Tool surface end-to-end. Optional cleanups bundle here if Chris opts in: moisture/density bare-numeric rollout to /add (note: dead post-deprecation), `expected_outcomes` / `observed_outcome_*` Phase 3 legacy-field handling on resolved view's Experiment journey, 5-bean backfill to unblock 13 unresolved experiments.
