...
**Last reorder:** 2026-04-23 (Region slice fully shipped — 1d.1 + 1d.2 + 1d.3 all landed 2026-04-22; Claude-driven portion complete. Chris confirmed Process CSV ready — **Process 1e is now Claude's default next candidate** (was SYNC V1 step d). Chris also working in parallel outside sessions on Flavor taxonomy 1g + a Roaster taxonomy refinement (content enrichment of the existing 21-canonical [lib/roaster-registry.ts](lib/roaster-registry.ts)). Remaining umbrella sub-sprints: Process 1e (unblocked), Dripper 1f (still needs research), Flavor 1g (Chris-parallel). Region Observed corpus distillation (mirror of Variety 1c) remains Chris-authored + deferred.
...
**Sub-sprint 1e.2 — Process schema migration + data decomposition (✅ shipped 2026-04-23).** Migration 025 adds 8 structured columns to `brews` (`base_process`, `subprocess`, `fermentation_modifiers`, `drying_modifiers`, `intervention_modifiers`, `experimental_modifiers`, `decaf_modifier`, `signature_method`) and batch-UPDATEs all 55 rows from `LEGACY_DECOMPOSITIONS`. `base_process` locked with NOT NULL + CHECK + index; modifier arrays default `'{}'`; scalar columns NULL when absent. Post-migration base distribution: Washed 31 / Natural 19 / Honey 5 / Wet-hulled 0. Four interpretive cells from 1e.1 resolved via pre-sprint DB audit: Anaerobic Honey → Generic Honey subprocess; Moonshadow Washed → plain Washed (full mis-label correction, not a Moonshadow lot); both Double Anaerobic Thermal Shock + Double Fermentation Thermal Shock → base:Washed with `[Double Anaerobic, Thermal Shock, Yeast Inoculated]` (El Paraiso house protocol under two English labels — they merge in 1e.4). Legacy `brews.process` text column untouched (zero UI change); dropped in 1e.4 when /processes is redesigned. See `memory/project_process_schema_migration.md` for full retro.

**Remaining umbrella sub-sprints:**
- **Sub-sprint 1e.3 — Process enforcement** (next Claude sprint). `/add` + `/brews/[id]/edit` picker redesign: base selector + conditional Honey subprocess + 4 chip inputs (fermentation / drying / intervention / experimental) + decaf + signature text. `CanonicalTextInput` variants per axis. Save-flow writes both structured columns AND `composeProcess(structured)` to `brews.process` so display stays synced. Mirror of Variety 1b + Region 1d.3 but with the composable schema needing a builder UX rather than a single typeahead.
- **Sub-sprint 1e.4 — /processes page redesign** (deferrable). Faceted by base + modifier axis. The existing flat "/processes/[slug]" string-equality page keeps working through 1e.1-1e.3 via `composeProcess()` denormalizing into `brews.process`. 1e.4 fires only if the flat page starts feeling wrong; could be punted indefinitely.
...
**Chris's parallel work outside the umbrella:**
- **Roaster taxonomy refinement** (started 2026-04-23, Chris-authored) — deepens the existing [lib/roaster-registry.ts](lib/roaster-registry.ts) (21 canonicals + BMR strategy + house-style cards, shipped as the Roasters aggregation sprint 2026-04-18). Different shape from the A1/A2 umbrella ports — not a new canonical registry, a content enrichment of an existing one. Claude may help vet / apply later; for now track as Chris-solo.
...
- **Chris's default next task:** candidate #1 sub-sprint 1c (Variety Observed corpus distillation, deferred) or Region Observed distillation (same pattern, per-macro for ≥3-brew macros: Volcán Barú 13, Huila 9, Sidama 4, Guji 3, EC Northern Andean 3). Both are Chris-driven authoring passes with Claude vetting. Chris is also separately authoring the **Flavor taxonomy (1g)** and a **Roaster taxonomy refinement** outside Claude sessions (2026-04-23).
- **Claude's default next task (recommended):** **sub-sprint 1e.2 — Process schema migration + data decomposition**. 1e.1 shipped the registry + doc on 2026-04-23; 1e.2 adds the 8 structured columns to `brews` (migration 025), Chris fills the 4 `TODO(1e.2)` interpretive cells in `LEGACY_DECOMPOSITIONS` (Moonshadow Washed base, Anaerobic Honey subprocess tier, both Thermal Shock bases), and a batch UPDATE materializes the new columns. Legacy `brews.process` stays as denormalized display string. Smaller sprint than 1e.1; could plausibly land same-day as 1e.3 (the picker rework) if scope holds.
- **Claude's alternative task:** **candidate #2 sub-sprint 1 step (d) V1-brews sync dog-food**. Both strict-enforcement gates are now live (Variety 1b + Region 1d.3 — aliases resolve to canonical, unresolvable inputs rejected), so sync inputs are consistent with the canonical surface. Pre-reqs: Mokka ✅ resolved (Variety 1a.1); Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Side-quest `/add` SR step 8 rebuild is also Claude-ready and would tip `persistBrew` toward the new `findOrCreate*` helpers (third caller).
