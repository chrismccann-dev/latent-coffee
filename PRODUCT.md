...
- **Sub-sprint 1e — Process research + port** (A2, ~20-25 processes, CSV ready 2026-04-23). Mirror of Variety 1a.1 / Region 1d.1 shape — Chris has the research CSV in hand. Next Claude candidate (default). See [docs/features/reference-taxonomies-process.md](docs/features/reference-taxonomies-process.md).
...
- **Claude's default next task (recommended):** **candidate #1 sub-sprint 1e — Process research + port** (A2, ~20-25 canonical processes). Chris has the research CSV ready as of 2026-04-23 — same shape as the Variety 1a.1 and Region 1d.1 CSVs. Mechanical mirror of those ports: author `docs/taxonomies/processes.md`, refactor [lib/process-families.ts](lib/process-families.ts) into a rich registry, write structural migration for any DB-row renames, kill any dual-registry drift. First A2 taxonomy port; will exercise the taxonomy-port generator scripts that the A1 ports built (Region + Variety — see side-quest #510).
