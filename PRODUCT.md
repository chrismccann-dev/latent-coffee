...
- **Use `jsonb_to_recordset` for any bulk text import where the data has newlines, smart quotes, or em-dashes.** Migration 019's first attempt used per-row `INSERT ... VALUES (...)` with PostgreSQL dollar-quoted strings (`$exp$...$exp$`). Worked in principle but became fragile when applying through MCP `execute_sql` calls — manually re-typing 27KB chunks dropped columns, mis-shifted values, and produced two failed attempts before switching strategies. JSON encoding via `jsonb_to_recordset` sidesteps every escape concern (newlines, single quotes, smart quotes, em-dashes all just work), and one `INSERT INTO ... SELECT FROM bean, jsonb_to_recordset(...) AS rec(...)` call inserts N rows from a single JSON array. Pattern for any future spreadsheet → DB import: emit JSON, not VALUES tuples.

**Migrations / data import**
- **Idempotent migrations require an explicit unique constraint, not a hope.** Migration 019 added `UNIQUE (user_id, green_bean_id, experiment_id)` as part of the migration so `ON CONFLICT DO NOTHING` had something to land on. The `experiments` table previously only had a UUID `id` PK, which doesn't help dedup on natural keys. When writing a data-import migration, always: (1) audit whether a natural unique key exists, (2) add the constraint as part of the same migration if not, (3) pair it with `ON CONFLICT DO NOTHING` on every INSERT. Skip step 2 and re-running a migration silently doubles the data.
- **MCP migration-apply has two flavors: split DDL from data.** `apply_migration` records the migration in Supabase's tracking table (use for DDL like the unique constraint above). `execute_sql` is for data INSERTs and runs each call as its own transaction. Don't try to span a `BEGIN ... COMMIT` across multiple MCP calls — each call commits independently. If a migration ships both DDL + data, do the DDL via `apply_migration` first, then split the data into per-section `execute_sql` calls. Keep the on-disk `.sql` file as the canonical, single-transaction record (with `BEGIN`/`COMMIT`) for future humans + replay.
- **Import faithfully into matching DB columns even if the source has cell drift.** The roasting spreadsheet's `LIBERTAD-BOURBON-AIR-01` row has the "Failure Boundary" cell populated with what is semantically observed_outcome_a content, with subsequent outcomes shifted left by one column. The migration imported each cell into its named DB column (failure_boundary cell → failure_boundary column). The render label shows "Failure Boundary" with outcome-A prose under it. Could have remapped at import time, but that would make the migration interpret the spreadsheet — a slippery slope where every import needs per-row hand-mapping. Pattern: import is a faithful pipe; data quality fixes happen on the source side OR via a follow-up migration with explicit per-row UPDATEs.
- **Spreadsheet section structure: detect, don't enumerate.** The Experiments tab uses bean-name rows in column 1 (no other cells populated) as section headers, then experiment rows below. The parse logic was: walk row by row, when col-1 is populated but col-2 + col-3 are empty, it's a new section header; otherwise it's an experiment row whose section is the most recent header. This survives row insertions, section reorders, and most cell-edit drift. Also caught: a duplicated header row at row 201 ("Experiment ID" / "Coffee Name" / ...) needed an explicit skip. Pattern: when parsing semi-structured spreadsheets, derive section membership from row shape, not from row index.
- **`/simplify` agent disagreement is a defer signal, not a tiebreaker.** This sprint's /simplify split: Agent 1 said "Field extraction is premature for 5 sites all in one block"; Agent 2 said "extract — combined with the 5 in roast learnings = 10 sites." When the agents disagree about whether to extract, the disagreement itself is the answer: if reasonable readers see it both ways, the abstraction isn't ready. Defer to the next sprint that touches the second site.
...
| 015 | Reclassify two Guatemala terroirs (Huehuetenango, Acatenango) into proper macros instead of foreign-country macros they had drifted into. |
| 016 | Cultivar canonicalization: collapse 5 Gesha rows into one canonical Gesha (re-pointed 22 brews + 2 green_beans), rename JARC blend lineage, move Garnica to Modern Hybrids, drop Timor-derived crosses. |
| 017 | Backfill 5 array fields (typical_origins, limiting_factors, common_processing_methods, typical_flavor_notes, common_pitfalls) on the 9 lineages that rendered blank post-canonicalization. |
| 018 | Producer canonicalization: collapse 2 duplicate clusters (Pepe Jijon ×3, Peterson Family ×2) + 5 verbose-string cleanups. 52 → 49 canonical "Person, Farm" producers. |
| 019 | Experiments import: 18 rows across 4 green_beans via `jsonb_to_recordset` + `(user_id, green_bean_id, experiment_id)` unique constraint. 16 more spreadsheet rows blocked on green_beans backfill. |
