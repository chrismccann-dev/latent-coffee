...
**Last reorder:** 2026-04-24 (**candidate #2 substantially rescoped** from "Claude Code paste-driven V1-brews sync" to "Claude ↔ app sync pipeline" — direct push from external `claude.ai` projects, app-served BREWING.md + ROASTING.md with read/write, all entity types not just brews. Original V1 step (d) dog-food preserved as sub-sprint 2.1 to prove the write stack before V2 work. Roasting absorbed from candidate #7 into sub-sprint 2.6. Chris priority order for Claude's remaining queue: 1i SR canonical rebuild (top) → 1h Roaster adoption → 2.1 V1-brews dog-food → 2.2+ V2 scoping → 1e.4 page redesign (good-to-do) → 1f Dripper held.)

**Prior reorder (2026-04-23):** Process slice Claude-complete (1e.1 + 1e.2 + 1e.3). Roaster taxonomy CSV (64 canonicals) authored — promoted to sub-sprint 1h. Chris-parallel work: Flavor 1g authoring; Variety 1c + Region Observed distillation remain Chris-authored + deferred.
...
**Remaining umbrella sub-sprints** (in Chris's priority order for Claude work):
- **Sub-sprint 1i — `/add` SR canonical rebuild (terroir + cultivar)** — **top Claude priority (promoted 2026-04-23).** The self-roasted `/add` flow is the one UI surface still not wired to the canonical registries post-1b + 1d.3. Step 7 (terroir) + step 8 (cultivar) both use generic textareas with no parser, no structured fields, no find-or-create. Chris's workflow with full taxonomies landed is: pick canonical cultivar + pick canonical macro, everything else flows from the registry entry. Rebuild step 7 with `CanonicalTextInput(TERROIR_COUNTRY_LOOKUP)` + `CanonicalTextInput(TERROIR_MACRO_LOOKUP)` + free-text meso; step 8 with `CanonicalTextInput(CULTIVAR_LOOKUP)` + auto-populated species/family/lineage. Wire `handleSaveSelfRoasted` to consume via `findOrCreateCultivar` + `findOrCreateTerroir` (already in `lib/brew-import.ts` post-1d.3). Tips `persistBrew` toward third-caller dedup (flagged in 1b + 1d.3 /simplify retros). Also includes process-field wire if not already covered by 1e.3's SR save path (it is — 1e.3 calls `seedStructuredProcess(parsedGreenBean.process)` + `composeProcess` at insert; ProcessPicker UI on SR step 9 could be added but is lower-priority polish). 1 sprint.
- **Sub-sprint 1h — Roaster taxonomy adoption** — **promoted 2026-04-23.** Chris completed a roaster reference taxonomy (64 canonicals + richer field set vs the current 21-canonical [lib/roaster-registry.ts](lib/roaster-registry.ts); ~80% coverage of roasters Chris encounters in the wild — roasters are more varied than varieties / regions / processes so absolute coverage is lower by design). Mirror of Variety 1a.1 / Region 1d.1 / Process 1e.1 structural port shape, but against an existing canonical registry rather than a greenfield one. Steps: (a) port authored content into `docs/taxonomies/roasters.md` (new); (b) refactor `lib/roaster-registry.ts` from flat `ROASTER_MAP` + `ROASTER_METADATA` to a rich `RoasterEntry[]` shape parallel to `CultivarEntry` / `TerroirEntry` (name / family / location / url / bmr_strategy + richer fields from the new taxonomy); (c) decide alias handling (Sey / Sey Coffee / Sey Brooklyn class — `SIGNATURE_ALIASES` pattern); (d) update any `/roasters` / `/roasters/[id]` consumers to read the rich fields; (e) if the richer fields include per-roaster attributes that don't currently have columns on a `roasters` DB entity (currently `brews.roaster` is free-text, no FK), decide structural vs content migration split (same Variety 1a.1 vs 1a.2 trade-off — do the structural rename first, content backfill second). Estimate: 1 sprint for structural port + back-compat (mirror Process 1e.1's ~45-min shape). Content backfill + enforcement sub-sprints follow if warranted.
- **Sub-sprint 1e.4 — /processes page redesign** (deferrable; Chris-flagged as "good to do soon, less urgent while data model works"). Faceted by base + modifier axis. The existing flat "/processes/[slug]" string-equality page keeps working through 1e.1-1e.3 via `composeProcess()` denormalizing into `brews.process`. Drops `brews.process` text column when it ships.
- **Sub-sprint 1e-content — Process reference content backfill** (Chris-authored, parallel). Rich per-entity reference content in `processes.md` (extraction framing, fingerprints, pitfalls, when-delivers-vs-off per canonical entity). Mirror of Variety 1c shape — Chris-driven, not Claude-driven.
- **Sub-sprint 1f — Dripper research + port** (A2; **on hold, Chris hasn't started taxonomy research**). ~8-10 drippers, BREWING.md integration, no new app page. See [docs/features/reference-taxonomies-dripper.md](docs/features/reference-taxonomies-dripper.md). Fires when Chris's research CSV is ready.
- **Sub-sprint 1g — Flavor extension** (A2, loose bar, family-level content, extends existing `lib/flavor-registry.ts`). **Chris is authoring this in parallel outside Claude sessions (2026-04-23).** When his draft lands, the Claude sprint collapses to port + enforcement rather than research. See [docs/features/reference-taxonomies-flavor.md](docs/features/reference-taxonomies-flavor.md).
...
- **`/add` SR canonical rebuild 1i (pending).** Until 1i ships, the SR `/add` flow is the one surface NOT canonically enforced. The sync pipeline's write path goes through `persistBrew` + `handleSaveSelfRoasted` — 1i is a pre-req for V2 SR pushes so the external-Claude write doesn't have to reinvent canonical resolution.
...
- 1i (`/add` SR canonical rebuild) — SR path's canonical enforcement matches purchased + edit. Must precede 2.3 for SR pushes to go through canonical find-or-create.
...
**Near-term action:** run 2.1 (V1-brews dog-food paste path) as the concrete next step after 1i + 1h to prove the write stack end-to-end on a real brew. Then scope 2.2 with fresh empirical data.
...
- **Claude's default next task (recommended):** **sub-sprint 1i — `/add` SR canonical rebuild**. Highest-priority remaining work per Chris 2026-04-23. Closes the last UI surface where the canonical-picker pattern is missing. One sprint, straightforward mirror of Variety 1b + Region 1d.3 but on the self-roasted step 7 + 8 surfaces. Tips `persistBrew` toward the `findOrCreate*` helpers as a third caller.
- **Claude's second recommended task:** **sub-sprint 1h — Roaster taxonomy adoption**. Structural port + registry refactor against Chris's new 64-canonical taxonomy. Same ~45-min shape as Process 1e.1. Content backfill + enforcement sub-sprints follow if warranted.
- **Claude's near-term task:** **candidate #2 sub-sprint 2.1 — V1-brews dog-food** (the originally-scoped step (d)). Proves the write stack end-to-end before V2 architecture decisions are made. All strict-enforcement gates now live (Variety 1b + Region 1d.3 + Process 1e.3 — aliases resolve to canonical, unresolvable inputs rejected, structured process columns mandatory at write). Pre-reqs: Mokka ✅ resolved; Producer `warn` → `block` promotion decision still pending. Fires after 1i + 1h. Candidate #2 was substantially rescoped 2026-04-24 (Claude ↔ app bidirectional sync with external `claude.ai` projects + app-served `BREWING.md`/`ROASTING.md`) — 2.1 is the empirical-feedback step before 2.2+ V2 scoping.
