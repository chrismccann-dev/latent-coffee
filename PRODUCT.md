...
- **One-shot lot mis-bucketed as "waiting for next roast" on `/green` index** (surfaced 2026-06-02 design-system recount — Mountain Harvest lot). A one-shot lot (`green_beans.is_one_shot=true`) has no next roast; after cupping it sits on the optimized-pourover step, but `computeLifecycleState` appears to route it into the waiting-for-next-roast section. Verify the lifecycle derivation handles one-shot post-cupping (likely a missing `is_one_shot` branch). Small fix, no visual call. **Bundled into the priority-stack recount follow-up session** (see Longer Term Items § Small / opportunistic).
...
- **Design-system priority-stack recount follow-ups** (2026-06-02). The page-by-page priority recount that produced the **priority stack** model (see [docs/design-system.md § Detail-page grammar](../docs/design-system.md)) surfaced a backlog of small IA/visual adjustments. Split into one bundled session + standalone tweaks.
  - **Bundled follow-up session (3 items — Chris runs as one, two need a preview sign-off):**
    1. **Recolor `/brews` book-cover by extraction strategy, not flavor.** The 6 strategies each already carry a color (`lib/extraction-strategy.ts`); the cover color drifted from its original flavor-keyed intent (when strategy was a single axis) and no longer signifies anything to Chris. Map cover → strategy color (suppression cooler, clarity tea-like, balanced mid, full bolder, hybrid purple; extraction-push is the one to decide). Touches `lib/brew-colors.ts`. *Needs preview sign-off.*
    2. **Reorder `/roasters/[id]`** so the **Coffees list is primary** (roaster is the recall gateway into brews — Chris recalls by roaster + bag style, not coffee name) and **synthesis drops to collapsible tertiary**. The current page leads with Brewing Template and buries Coffees mid-page. *Needs preview sign-off.*
    3. **One-shot lifecycle mis-bucket bug** (see Bugs and Issues). Verify-and-fix, no visual call.
