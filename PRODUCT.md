...
**Last reorder:** 2026-04-22 (Variety sprints 1a.1 + 1a.2 shipped — PRs #34 + #36. 1a.1: 63 canonical cultivars in `docs/taxonomies/varieties.md`, rich-shape `lib/cultivar-registry.ts`, migration 021, SYNC V1 step (d) Mokka pre-req resolved as side effect. 1a.2: migration 022 backfills 18 attribute fields across all 26 existing DB cultivar rows. Sub-sprint 1b (CanonicalTextInput enforcement) queued next under candidate #1).
...
**Sub-sprint 1b — Variety enforcement (NEXT, Claude).** Swap `/add` cultivar select + `/brews/[id]/edit` cultivar input to `CanonicalTextInput` (already in use for flavor / producer / roaster) backed by the new `CULTIVAR_LOOKUP` (63 canonicals + 48 aliases). Closes the "strict enforcement bar" loop for the Variety feature doc. Plan-mode investigation should open the sprint because /add is a 9-step wizard with cultivar in one of the steps — touch points need to be understood before editing. 1 sprint.
...
- **Claude's default next task:** candidate #1 sub-sprint 1b — CanonicalTextInput enforcement on `/add` + `/brews/[id]/edit`. 1 sprint. Plan-mode investigation before editing — the `/add` wizard has cultivar in one of 9 steps; current shape needs reading before the swap. Alternative: candidate #2 sub-sprint 1, step (d) — V1-brews sync dog-food. The Mokka pre-req is now resolved; two remaining pre-reqs (Producer block promotion + LABEL_ALIASES extension) are still pending.
