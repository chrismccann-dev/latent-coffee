...
**Last reorder:** 2026-04-23 (Process slice Claude-complete — 1e.1 + 1e.2 + 1e.3 all landed same day; only 1e.4 page redesign + 1e-content Chris-authored remain. Chris confirmed Roaster taxonomy CSV is ready (64 canonicals + richer field set, ~80% coverage of roasters Chris encounters) — **promoted to sub-sprint 1h**, mirror of 1a.1 / 1d.1 / 1e.1 structural port shape. Chris priority order for Claude's remaining queue: 1i SR canonical rebuild (top) → 1h Roaster adoption → SYNC V1 step (d) (near-term) → 1e.4 page redesign (good-to-do, not urgent) → 1f Dripper held. Chris-parallel work: Flavor 1g authoring; Variety 1c + Region Observed distillation remain Chris-authored + deferred.
...
- **Claude's near-term task:** **candidate #2 sub-sprint 1 step (d) V1-brews sync dog-food**. All strict-enforcement gates now live (Variety 1b + Region 1d.3 + Process 1e.3 — aliases resolve to canonical, unresolvable inputs rejected, structured process columns mandatory at write), so sync inputs are consistent with the canonical surface. Pre-reqs: Mokka ✅ resolved; Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Fires after 1i + 1h.
