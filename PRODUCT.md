...
#### 1. Architecture implementation — CLOSED (Wave 4 PR 4b shipped 2026-05-21)
...
#### 2. Architecture implementation — Wave 4 PR 4b shipped (architecture implementation arc CLOSED)

**Wave 4 PR 4b shipped 2026-05-21** — closes the architecture implementation arc. BREWING.md rewritten 124KB → ~3KB redirect-stub pointer table; ROASTING.md rewritten 72KB → ~6KB redirect-stub pointer table. ~196KB residual operational content migrated to sub-skill clusters: new `coordinator/operator-guide.md` (cross-domain canonical lookups + MCP server + Session Debrief paste template); expanded `coordinator/catalog.md § brewing-domain-principles + § roasting-domain-principles` (Two-Axis framing + Roasting Philosophy + What Good Looks Like + What I Am Not Trying To Do); new `brewing-assistant/cluster/operational-guide.md` (BREW PROMPT Steps 1-4); new `brewing-equipment-expert/cluster/operational-reference.md` (Location Constraints + Equipment + Valve + Filter + Examples); new `roasting-assistant/cluster/onboarding-protocol.md` (New Coffee Onboarding Steps 1-4 + Naming Conventions + Parallel Experiment); 2 new `roasting-historian/cluster/patterns/by-process/{natural,honey}.md` (Naturals + Honey roast-direction frameworks per Wave 3 PR 1 Chris-lock cleanup); 6 new `roasting-historian/cluster/active-lots/<lot>.md` + README (Active Lots from Lot Knowledge); `roasting-historian/cluster/one-shot-calibrations/README.md` (empty in-process state); 2 new `roest-knowledge/cluster/protocols/{between-batch-protocol,green-storage}.md`; Process/Variety Signal Table appended to `brewing-historian/cluster/patterns/cross-coffee-insights.md`. CLAUDE.md sub-skills section extracted to new `docs/architecture/sub-skills-status.md` per PR 4a scope decision 4 (CLAUDE.md 125,750 → ~115KB, back under the 120KB tripwire). ARBITER.md target_doc allow-list marks `brewing.md` / `roasting.md` as deprecated (redirect-stub-only); proposals now route to `skills/{path}.md` cluster paths. 14 new MCP Resources registered in `lib/mcp/docs.ts` (listDocs 100 → 114). All 4 scope decisions defaulted to recommended (brewing-assistant cluster destination; per-lot directories for Lot Knowledge; keep Session Debrief in prompts + fold into operator-guide; single PR single commit).

**Triggers when:** Wave 4 PR 4a closes (shipped 2026-05-21). **Shipped 2026-05-21.**

#### 4. Roadmap re-session — ACTIVE (next sprint)
