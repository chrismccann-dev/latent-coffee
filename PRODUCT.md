...
**Follow-on sub-sprints (fire after Variety fully lands):**
- **Sub-sprint 1d — Region research + port** (mirror of Variety, ~21 macros; enriches `lib/terroir-registry.ts` with per-macro authored content + reconciles the 14-vs-19-vs-21 drift between doc / registry / DB). See [docs/features/reference-taxonomies-region.md](docs/features/reference-taxonomies-region.md). Apply Variety's retro: pre-sprint audit for duplicate registries (`rg TERROIR_REGISTRY`); expect CSV to scope deeper than the feature doc estimate; split structural vs content migrations.
...
**Sizing:** 1a.2 is a half-sprint (content SQL). 1b is one sprint (two form surfaces). 1d research is Chris-driven, unbounded; 1d port + render + enforce is ~2 Claude sprints after handover. 1e-g similar shape to 1d.
...
- **Chris's default next task:** candidate #1 sub-sprint 1c (Variety Observed corpus distillation, deferred) or sub-sprint 1d research (Region taxonomy — mirror of Variety for the 21 macros). Both are Chris-driven authoring passes with Claude vetting.
- **Claude's default next task:** candidate #2 sub-sprint 1, step (d) — V1-brews sync dog-food. Variety 1b (CanonicalTextInput enforcement) shipped 2026-04-22. All 3 candidate-#2-step-(d) pre-reqs status: Mokka pre-req ✅ resolved (1a.1); Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Alternative: candidate #1 sub-sprint 1d (Region port) if Chris has handed over research. Side-quest `/add` self-roasted step 8 rebuild is also Claude-ready and scoped — fires when the user picks it up.
- **Candidate #1 sub-sprint 1d (Region port)** fires after Variety fully lands (1a.2 + 1b). Do NOT attempt to pre-populate `docs/taxonomies/regions.md` from training data — Chris owns the research, Claude vets + ports. Apply Variety's retro lessons: pre-sprint audit for duplicate registries, expect CSV to scope ~3× the feature doc estimate, split structural vs content migrations.
