...
- **Flavor-notes canonicalization + brew edit flow** — `brews.flavor_notes` has visible drift: case inconsistencies (Mango / mango, Pear / pear), near-duplicates ("Bergamot" vs "hints of bergamot", "Stone fruit" vs "Stone fruit sweetness"), and whole sentences crammed into single tags ("prune and lemon notes blend with a generous sweetness reminiscent of caramel"). Root cause: `/add` creates, but there's no UI to edit an existing brew — so drift festers until someone writes SQL. Sprint should bundle (1) one normalization SQL pass to collapse obvious duplicates and lift sentences into `what_i_learned`, (2) a canonical flavor-notes registry with a small approval loop for new entries, (3) a brew-edit UI so future normalization is ergonomic rather than migration-only. Sequence after the design polish sprint so the detail-page layout is stable before new editing surfaces land.
