...
- **But don't extract at 3 when the shape varies.** Follow-up from cross-link backfill sprint (PR #19): /simplify flagged that 3 pages now build a `terroirMap` with the same `macro_terroir || admin_region || country` key + first-wins pattern. Resisted extracting because each page's label format differs (`country / name` vs plain name vs `lineage / name`) and the loops are 5-6 lines inline. A generalized `buildTagItems()` helper with 4-5 params (keyFn, valueFn, labelFn, hrefFn, firstWinsFlag) would read worse than the inline loops. The "extract on 2nd duplication" rule is for *identical* shapes, not *similar* shapes. When the call-sites would all need config params, inline wins until it doesn't. Trigger for actual extraction: when a 5th page appears with the same shape, or when a call-site starts deviating in ways that would feed back into the shared loop.
- **Audit-first validates (or invalidates) the brief in <10 min.** Cross-link backfill sprint's entire execution was 3 edits × 3 files — the audit at the top (read the 3 files, enumerate which sections are plain `<Tag>` vs already `<TagLinkList>`, confirm the data-fetch layer already carries the IDs needed) answered "is this as mechanical as the brief claims?" in under ten minutes. Matched exactly, went direct to execution, skipped plan mode per brief instructions. Pattern worth keeping: for a bounded mechanical sprint, the audit is the plan. Only escalate to plan mode if the audit surfaces a structural surprise.
- **PostgREST embedded selects extend cheaply.** Adding `id` to `terroir:terroirs(...)` / `cultivar:cultivars(...)` in a Supabase `.select()` doesn't add a query or a join — the embed already joined that table, `id` is one more column on the rows being fetched. Useful default for "I need to link to X's detail page from this rendered tag": extend the existing embed, don't round-trip. Payload cost: ~36 bytes × row count, negligible.
