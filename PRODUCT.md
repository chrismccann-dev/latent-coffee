...
**Last reorder:** 2026-04-22 (Region sprint 1d.2 shipped — migration 024 overwrites migration 009's terroir content columns with the canonical `docs/taxonomies/regions.md` macro-level content across all 22 DB rows. `climate_stress` flips from one-word enum values to free-text Climate Regime sentences; `typical_processing` Title Case mapped via `lib/process-families.ts`. Zero schema / code / FK change. Next Claude candidate: sub-sprint 1d.3 (Region enforcement via CanonicalTextInput on `/add` + `/brews/[id]/edit` macro_terroir, mirror of Variety 1b), or candidate #2 step (d) SYNC V1 dog-food. `findOrCreateCultivar` + new `findOrCreateTerroir` extraction into `lib/brew-import.ts` is a natural side-effect of 1d.3 — third caller tips the balance per /simplify's Variety 1b flag.
...
**Follow-on sub-sprints:**
- **Sub-sprint 1d.3 — Region enforcement** (mirror of Variety 1b). `CanonicalTextInput` bound to `TERROIR_MACRO_LOOKUP` on `/add` + `/brews/[id]/edit`. PATCH resolves `macro_terroir` → canonical + find-or-create with country + admin_region auto-populate from `getTerroirEntry()`. Natural moment to extract `findOrCreateCultivar` + new `findOrCreateTerroir` into `lib/brew-import.ts` (third caller tips the balance per /simplify's Variety 1b flag).
...
- **Chris's default next task:** candidate #1 sub-sprint 1c (Variety Observed corpus distillation, deferred) or Region Observed distillation (same pattern, per-macro for ≥3-brew macros: Volcán Barú 13, Huila 9, Sidama 4, Guji 3, EC Northern Andean 3). Both are Chris-driven authoring passes with Claude vetting.
- **Claude's default next task (recommended):** **sub-sprint 1d.2 — Region content backfill**. Half-sprint, mechanical, highest payoff — migration 024 UPDATEs the 22 existing `terroirs` rows with the 14 attribute content fields from `docs/taxonomies/regions.md`. Terroir detail pages immediately render the macro-level reference content. Mirror of Variety migration 022 (PR #36). No schema / code change.
- **Claude's follow-on task:** **sub-sprint 1d.3 — Region enforcement**. One sprint. `CanonicalTextInput` on `/add` + `/brews/[id]/edit` bound to `TERROIR_MACRO_LOOKUP`. PATCH `/api/brews/[id]` resolves `macro_terroir` → canonical + find-or-create with `country` + `admin_region` auto-populate from `getTerroirEntry()`. Mirror of Variety 1b (PR #38). Also: extract `findOrCreateCultivar` (flagged during Variety 1b /simplify) alongside a new `findOrCreateTerroir` into `lib/brew-import.ts` — third caller (SR save flow side-quest) tips the balance toward shared helpers.
- **Alternative Claude task:** candidate #2 sub-sprint 1 step (d) V1-brews sync dog-food. Pre-reqs: Mokka ✅ resolved (Variety 1a.1); Producer `warn` → `block` promotion decision + `LABEL_ALIASES` extension still pending. Side-quest `/add` SR step 8 rebuild is also Claude-ready.
