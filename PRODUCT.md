...
- **Sub-sprint 1e.2 — Process schema migration + data decomposition** (next Claude sprint). Migration 025 adds 8 structured columns to `brews` (base_process / subprocess / fermentation_modifiers / drying_modifiers / intervention_modifiers / experimental_modifiers / decaf_modifier / signature_method). Chris fills the 4 interpretive cells in the `LEGACY_DECOMPOSITIONS` table (Moonshadow Washed base correction; Anaerobic Honey subprocess tier; both Double Anaerobic Thermal Shock + Double Fermentation Thermal Shock bases); batch UPDATE populates new columns from the decomposition map. Legacy `brews.process` column kept as denormalized display string for back-compat through 1e.4.
- **Sub-sprint 1e.3 — Process enforcement** (after 1e.2). `/add` + `/brews/[id]/edit` picker redesign: base selector + conditional Honey subprocess + 4 chip inputs (fermentation / drying / intervention / experimental) + decaf + signature text. `CanonicalTextInput` variants per axis. Mirror of Variety 1b + Region 1d.3 but with the composable schema needing a builder UX rather than a single typeahead.
