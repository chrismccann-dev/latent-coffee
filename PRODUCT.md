...
- **Design-system priority-stack recount follow-ups** (2026-06-02). The page-by-page priority recount that produced the **priority stack** model (see [docs/design-system.md § Detail-page grammar](../docs/design-system.md)) surfaced a backlog of small IA/visual adjustments. Split into one bundled session + standalone tweaks.
  ...
  - **Standalone tweaks (simpler, do anytime).** Tweaks 1-2 **SHIPPED 2026-06-03** (collapse `green-bean-info` by default on all `/green` views — done in the shared `GreenBeanInfoCard` so it propagates to all 5 lifecycle shapes; collapse Anchor Baseline by default on waiting-for-roast — in-place `<details>` disclosure). Remaining:
    - **Tweak 3 — relocate the out-of-place "Reference Recipe Design Intent" drop-rules block on resolved `/green`.** Decision locked 2026-06-03: **move it into the Additional Information collapse** (down near the roast log / experiment journey), de-emphasizing the drop-rules section rather than cutting it.
    - **Tweak 4 — recompose the resolved Reference Cup section** to pull in the optimized-brew recipe card + tasted flavor/structure + peak expression (mirror `/brews/[id]`). Heaviest of the set — give it its own mini-sprint with a plan.
    - **Tweak 5 — paginate / cap the `/brews` index + per-roaster Coffees list** as they grow (89 brews today). Lowest urgency.
    - **Tweak 6 — add a short "what this process is" description to `/processes` variant pages** (nice-to-have, IA-nicer-not-urgent). Source decision 2026-06-03: **Chris authors the descriptions**; needs the list of variant pages lacking one (handed off in the session brief).
