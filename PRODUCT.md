...
- **Bidirectional cross-links across the 4 aggregation detail pages** — `/roasters/[slug]` links out to all 3 other aggregation pages, and the 3 older detail pages now link to roasters. But the older CULTIVARS / TERROIRS / PROCESSES tag sections on each other still render as plain text. Wrapping those in `<TagLinkList>` (already extracted) and joining IDs into the existing aggregation maps would close the cross-link cycle in ~3 small edits per page.
