...
- **Sub-pages first-principles rethink series (Sprints 2-6)** — scoped 2026-05-09 as a follow-up to the brew detail sprint ([PR #131](https://github.com/chrismccann-dev/products/pull/131)). Same playbook as Sub Pages 1: directed informational rethink per page, stay inside existing chrome (`<SectionCard>` / `<Tag>` / `<TagLinkList>` / `.label`), no new design tokens, surface buried fields, mobile-collapse where helpful. Each sprint is its own session; Chris brings source material (mocks / pain points / written job-to-be-done framing) at kickoff per the [feedback_ask_before_implementing.md](~/.claude/projects/-Users-chrismccann-latent-coffee/memory/feedback_ask_before_implementing.md) rule.
  ...
  - **Sub Pages 2 — Terroir detail page.** Code surface: [app/(app)/terroirs/[id]/page.tsx](app/(app)/terroirs/%5Bid%5D/page.tsx) + [TerroirSynthesis](components/) adapter. Aggregates all terroirs sharing macro+country, surfaces synthesis + flavor + cultivars + processes + coffee list. Most likely shape: similar reorder + buried-field surfacing, no major restructure.
  - **Sub Pages 3 — Cultivar detail page.** Code surface: [app/(app)/cultivars/[id]/page.tsx](app/(app)/cultivars/%5Bid%5D/page.tsx). Same shape as Terroir.
  - **Sub Pages 4 — Processes detail page.** Code surface: [app/(app)/processes/[name]/page.tsx](app/(app)/processes/%5Bname%5D/page.tsx). Same shape; deferred subprocess/modifier-axis facet redesign lives separately under "/processes page redesign (sub-sprint 1e.4)".
  - **Sub Pages 5 — Roasters detail page.** Code surface: [app/(app)/roasters/[name]/page.tsx](app/(app)/roasters/%5Bname%5D/page.tsx). Sibling of the existing "Per-roaster archive page enhancement" entry in Longer Term Items — Chris will decide at sprint time whether to bundle the directed-prompt enhancement here or keep separate.
  - **Sub Pages 6 — Green bean detail page.** Code surface: [app/(app)/green/[id]/page.tsx](app/(app)/green/%5Bid%5D/page.tsx). Structural outlier — not an aggregation page like the other 4; it's an entity page with children (roasts / experiments / cuppings / roast_learnings) plus active write surface during iterative roasting. Job-to-be-done may split ("track an active lot" vs "reference a closed lot"). May want its own scope doc upfront like pour-structure got, rather than a single-session sprint. Bundles compound with Longer Term § Green-bean surface polish + Experiments/Cupping rework — Chris will decide at sprint time how much to fold in.
