...
**Last reorder:** 2026-04-22 (Variety sprint 1b shipped — CanonicalTextInput enforcement on `/add` purchased review + `/brews/[id]/edit`. Edit picker flips from 26-DB-row select to 63-entry canonical registry with lazy find-or-create on save; aliases resolve to canonical ("Geisha" → "Gesha"); auto-populate species/family/lineage from `getCultivarEntry()` on match; `isResolvable` added to `makeCanonicalLookup`. Variety sprint-1 fully shipped except 1c (Observed corpus, Chris-driven, deferred). Next Claude candidate: sub-sprint 1d Region port once Chris hands over research, or candidate #2 step (d) SYNC V1 dog-food.
...
**Follow-on sub-sprints (fire after Region fully lands):**
- **Sub-sprint 1d.2 — Region content backfill** (mirror of Variety 1a.2). Migration 024 populates the 11 attribute content columns on existing terroirs rows from `regions.md`. One-shot SQL.
- **Sub-sprint 1d.3 — Region enforcement** (mirror of Variety 1b). `CanonicalTextInput` bound to `TERROIR_MACRO_LOOKUP` on `/add` + `/brews/[id]/edit`. PATCH resolves `macro_terroir` → canonical + find-or-create with country + admin_region auto-populate from `getTerroirEntry()`.
