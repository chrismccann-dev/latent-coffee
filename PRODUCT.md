...
- **Next: companion surface 3 (soft)** — `/green/[id]` waiting-for-roast (usually desktop, so a strong mobile pass not mobile-first), then the remaining `/green` lifecycle shapes (resolved / unresolved / inventory) which share the now-legacy components, then roasters / cultivars / terroirs / processes, then the `/green` index — each its own plan-mode sprint on the now-stable token + primitive base.
