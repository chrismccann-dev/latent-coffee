...
#### 5. Claude-Design-led redesign — ACTIVE (Sprint 0 Foundation + Sprint 1 Brew Detail + Sprint 2 Green Cupping shipped 2026-05-29; per-surface sprints continue)
...
- **Sprint 3 — Green waiting-for-roast re-skin — SHIPPED 2026-05-29.** Third companion surface (soft mobile-primary). `/green/[id]` waiting-for-next-roast re-skinned to `Ssp*` inside `.ssp-page`, **single tree (no dual-subtree)** — the hypothesis table is design-intent reference (fewer rows than cupping actuals), so the `.ssp-exp` `<520` narrow variant handles mobile without a transposed→slot-card recomposition. 6.3 IA preserved (Primary Question → Roast Hypothesis transposed table w/ variance-gated amber lever rows → Drop Rules → green-bean info → roast log). Header tile amber (`#A88037` roast-emphasis) per artboard `STATE["stage-1"]`. New builders `buildHypothesisData` + `buildDropRulesGrid` feed `SspExpGrid` + an inline amber `.ssp-inset` grid-mode Drop Rules (replacing the foreground `DropRulesCard` on this view); old `HypothesisTable` retired. Ported `.ssp-inset .inset-grid` CSS (Sprint 2 only ported `inset-stack`) + a `state-roast .ssp-shead` `<520` wrap rule so the context line clears the corner badge. Corner is bare `{vLabel}` (dropped the artboard's "· Gold Standard" for the 390 collision, matching the shipped Sprint 2 corner call). **Chris ratified "keep the seam"** in plan-mode — shared components (`GreenBeanInfoCard`/`RoastLogTable`/`PerRoastReflections`/`ExperimentFrameCard`/`DropRulesCard`) stay legacy-skinned below; they re-skin on a dedicated green shared-components sprint covering the remaining 4 shapes at once. Verified at 390 + 1024 (Fazenda Um - Wush Wush Natural). See [docs/sprints/shipped.md](../sprints/shipped.md).
- **Next: remaining `/green` lifecycle shapes** — resolved / unresolved / inventory (which share the now-legacy components — the natural home for the shared-components re-skin), then roasters / cultivars / terroirs / processes, then the `/green` index — each its own plan-mode sprint on the now-stable token + primitive base.
