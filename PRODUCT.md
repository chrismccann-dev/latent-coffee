...
Sprint 3.1 brainstorm shipped 2026-05-12. Sprint 3.2 Cleanup-A shipped 2026-05-13. The post-grilling-sequencing era (Sprints 0-14 + M + F) closed 2026-05-24 — Sprint F's 6-actor matrix walk confirmed clean substrate. **Sprint R restructured this queue 2026-05-25** around Chris-locked Option 1 sequence: audit cluster → architecture brainstorm → architecture implementation → roadmap re-session. **Audit cluster shipped 2026-05-26** (PR #195 — claude.ai memory refactor + 3 prompt patches + POD-1 scoping draft). **Architecture brainstorm cluster shipped 2026-05-26** — 3 ADRs (0011 / 0012 / 0013) + 18 sub-skill decomposition spec stubs + master-doc transition plan + Wave 1 implementation kickoff brief. **Wave 1 shipped 2026-05-26** — Master Coordinator catalog + Brewing Equipment Expert cluster (4 equipment taxonomies migrated from `docs/taxonomies/` to `docs/skills/brewing-equipment-expert/cluster/` with redirect stubs at the old paths). **Wave 2 PR 1 shipped 2026-05-26** — WBC Brewing + WBC Roasting Archivists paired ship (4 WBC docs migrated from `docs/brewing/wbc-*.md` + `docs/roasting/wbc-*.md` into the two archivist clusters with redirect stubs at the old paths; ARBITER.md routing + propose_doc_changes target_doc allow-list extended with `skills/{path}.md` shape; 16 new MCP Resources + sourcing kept tentatively merged with WBC Roasting per ADR-0011). **Wave 2 PR 2 shipped 2026-05-26** — Brewing Historian (BREWING.md § Cross-Coffee Insight Layer extracted into 21 cluster files: 1 cross-coffee-insights.md + 6 by-strategy + 7 by-cultivar stubs + 6 by-coffee-family stubs; BREWING.md shrunk 213KB → 124KB ~88KB shrink, within the 60-80KB target band; h1 + pointer block retained at #cross-coffee-insight-layer for anchor back-compat; 21 new MCP Resources). **Wave 2 PR 3 shipped 2026-05-26 — Wave 2 closed** — Roasting Historian (ROASTING.md § Cross-Coffee Insight Layer + § Open Questions extracted into 12 cluster files: cross-coffee-insights.md + open-questions.md + general.md + FK-seeded by-cultivar/gesha.md + by-process/washed.md + 7 per-lot learnings/ pointer files; ROASTING.md shrunk 147KB → 103KB ~43KB shrink, slightly under the 60-80KB target band because Roast-to-Brew Translation deliberately kept in ROASTING.md until Wave 4; h1 + pointer blocks retained at #cross-coffee-insight-layer + #open-questions for anchor back-compat; 12 new MCP Resources). Wave 2 cumulative master-doc shrink: ~131KB (BREWING + ROASTING). **Wave 3 PR 1 shipped 2026-05-26 — operator-stub Knowledge pair** — Peer-Learning Roasting Archivist (3 cluster files: per-peer/dongzhe.md merging the Yunnan-Hatchi livestream extraction + ROASTING.md § Reference Roast Target #249 + § Peer Insights; cross-peer/patterns.md N<3 stub; source-index.md provenance log) + Roest Knowledge (10 cluster files: machine/l200-ultra.md + machine/counterflow-observations.md + protocols/{evaluation,fan-strategy,fc-marking}.md + api/{read-surface,write-surface,quirks}.md + firmware/README.md + observed-quirks.md). Also extracts Roasting Historian's cluster/patterns/roast-to-brew-translation.md from ROASTING.md (override of the prior roasting-historian/SKILL.md:51 Chris-lock that kept R-to-B in ROASTING.md until Wave 4 — re-locked to Roasting Historian because content shape is cross-coffee pattern aggregation, not Roest-machine-specific; see [feedback_recommend_prior_lock_as_default.md](~/.claude/projects/-Users-chrismccann-latent-coffee/memory/feedback_recommend_prior_lock_as_default.md) for the procedural rule the session surfaced). ROASTING.md shrunk 103KB → 72KB (~31KB shrink, within ~25-40KB target after Chris-lock kept Naturals + Honey + 3 ambiguous sections in ROASTING.md for Wave 3 cleanup pass). 29 new MCP Resources (16 Wave 3 PR 1 + 13 Wave 2 PR 3 listDocs entries that were missed at the time — repaired in same PR). Wave 2+3 cumulative master-doc shrink: ~162KB. **Wave 3 PR 2 shipped 2026-05-26** — 4 Workflow Planning sub-skills flipped PLACEHOLDER → ACTIVE; reads-only composition over Wave 1+2 Knowledge clusters. **Wave 3 PR 3 shipped 2026-05-26 — Wave 3 closed** — 5 Workflow Executing sub-skills flipped PLACEHOLDER → ACTIVE; POD-1 absorbed into Cupping Specialist at SKILL.md level + bookmarked at `cluster/pod-1-routing.md` pending 4 lived-practice trigger conditions; Chains 1 + 3 + 4 promoted PARTIAL → ACTIVE, Chain 2 to ACTIVE-pending-Learning-Knowledge, Chain 5 already ACTIVE, Chain 6 placeholder. **Wave 4 PR 4a shipped 2026-05-21** — CCIL flipped PLACEHOLDER → ACTIVE; skeleton + Sudan Rume seed pattern at `cluster/coffee/sudan-rume/across-roasting-and-brewing.md` (N=3 across both domains) + Pattern F audit trail starter at `cluster/decomposition-log.md`; Chain 6 promoted PLACEHOLDER → ACTIVE-pending-lived-practice; 3 new MCP Resources (listDocs 97 → 100). **Wave 4 PR 4b pending** — closes the architecture implementation arc by rewriting BREWING.md + ROASTING.md as ~500-byte redirect stubs, folding in the Naturals + Honey + 3 ambiguous ROASTING.md sections cleanup pass per Wave 3 PR 1 Chris-lock, and compacting the CLAUDE.md sub-skills section via extraction to `docs/architecture/sub-skills-status.md`. Per Chris 2026-05-25, 3.3-3.7 + deferred candidates are demoted to **Queued post-architecture** below — they will be re-prioritized in the roadmap re-session that closes the architecture implementation work. Continuous-feedback channel for non-architectural friction continues at `memory/feedback_mcp_continuous_log.md`.

#### 1. Architecture implementation — Wave 3 closed (Wave 4 PR 4a shipped; PR 4b = next active sprint)

**Wave 3 PR 1 shipped 2026-05-26** — Peer-Learning Roasting Archivist + Roest Knowledge operator-stub Knowledge clusters (3 + 10 cluster files; ~31KB ROASTING.md shrink). **Wave 3 PR 2 shipped 2026-05-26** — 4 Workflow Planning tier sub-skills (Roasting Assistant + Brewing Assistant + Learning Assistant + Sourcing Workflow Planner) flipped PLACEHOLDER → ACTIVE. **Wave 3 PR 3 shipped 2026-05-26 — Wave 3 closed** — 5 Workflow Executing tier sub-skills flipped PLACEHOLDER → ACTIVE: Roast Recorder (`push_roast` + `push_roast_recipe` + `patch_roast` + `patch_roast_recipe`) + Brew Recorder (`push_brew` + `patch_brew` + downstream `propose_doc_changes`; per-coffee terminal write per ADR-0011 § iteration-depth asymmetry) + Cupping Specialist (`push_cupping` + `patch_cupping` + `patch_experiment` + `patch_roast` is_reference_candidate flag + V-set Path A/B/C-1/C-2 routing; **POD-1 absorbed at SKILL.md level + bookmarked at `cluster/pod-1-routing.md` pending 4 lived-practice trigger conditions** — today's Path C-1/C-2 substrate kept intact, future simulated-pourover-gate rewrite deferred per the DRAFT scoping doc's own "Decision deferred" lock) + Roest API Worker (`push_roast_profile` + `patch_roast_recipe` linkage; symmetry-split from Roest Knowledge) + Close-Lot Specialist (`push_roast_learnings` + `patch_roast_learnings` + `patch_roast` is_reference + worth_repeating + `patch_inventory` archive + cross-link verification; handles both V-set + one-shot close-out incl. 7 lever-attribution fields schema-rejected on one-shots per migration 054 + Outcome A/B routing). **4 scope decisions Chris-locked at session start via AskUserQuestion, all recommended defaults accepted**: (1) POD-1 integration depth = summary inline + author `cluster/pod-1-routing.md` reference doc — respects the scoping draft's own "Decision deferred. More cross-thread observation needed" lock; the rewrite waits for lived practice on 2-3 V-set Path A lots + at least one one-shot close-out + Stefano Um / Bukure / Higuito decisions + C-2 disambiguation cases; (2) `lib/agents/` helper authoring = skip per ADR-0011 + ADR-0012 defer-code-until-needed pattern; future authoring gated on lived-practice friction; (3) Chain status = promote Chains 1 + 3 + 4 from PARTIAL → ACTIVE at PR 3 ship time per "all hops ACTIVE → chain ACTIVE" convention; Chain 2 to ACTIVE-pending-Learning-Knowledge (archival hop deferred per ADR-0011 trigger of ≥2 completed research tracks); Chain 5 already ACTIVE single-step; Chain 6 placeholder for Wave 4 CCIL; (4) Tool descriptions = append single-line "Owned by <sub-skill> per ADR-0011" pointer to each affected push/patch Tool (13 Tools touched; co-ownership pointers on multi-owner cases like `patch_roast` co-owned by Roast Recorder + Cupping Specialist + Close-Lot Specialist). Cupping Specialist is the only Wave 3 PR 3 sub-skill with a `cluster/` — `cluster/pod-1-routing.md` carries forward the POD-1 scoping draft + 4 trigger conditions. Other 4 ship SKILL.md only per PR 2/3 precedent. Master Coordinator: catalog flipped 5 entries placeholder → ACTIVE + wave-status updated; dispatch-rules added 10 new intent → executor mappings; handoff-rules promoted 4 chains. 6 new MCP Resources registered in `lib/mcp/docs.ts` (5 SKILL.md + 1 cluster doc). **POD-1 status moved from "parked / absorbed" → "absorbed into Cupping Specialist; full Path-C rewrite + simulated-pourover schema scoping gated on lived-practice trigger conditions per `docs/skills/cupping-specialist/cluster/pod-1-routing.md`."** **Cleanup pass deferred** — Naturals - Roasting Framework + Honey Process - Roast Direction Fork stayed in ROASTING.md per Chris-lock during PR 1; both flagged as needing a future cleanup-pass reclassification (Roasting Historian by-process/ vs stay-in-master). Fold into Wave 4 redirect-stub work OR run as a small follow-up sprint.

**Triggers when:** Wave 2 closes (closed 2026-05-26). **PR 1 shipped 2026-05-26. PR 2 shipped 2026-05-26.**

#### 2. Architecture implementation — Wave 4 PR 4b (master-doc residual migration + redirect stubs + CLAUDE.md compaction + cleanup pass)

**Wave 4 PR 4a shipped 2026-05-21** — CCIL flipped PLACEHOLDER → ACTIVE; skeleton + Sudan Rume seed pattern (`cluster/coffee/sudan-rume/across-roasting-and-brewing.md` N=3 across both domains; CGLE Sudan Rume Hybrid Washed closed + CGLE Sudan Rume Natural V5 active + externally-roasted CGLE Las Margaritas Sudan Rume Natural brewing-only); Pattern F audit trail starter at `cluster/decomposition-log.md`; Chain 6 promoted PLACEHOLDER → ACTIVE-pending-lived-practice; 3 new MCP Resources registered in `lib/mcp/docs.ts` (listDocs 97 → 100); Master Coordinator catalog flipped CCIL entry → ACTIVE + Learning Assistant entry updated to reference CCIL as live; CLAUDE.md sub-skills section extended with the Wave 4 PR 4a marker (compaction held for PR 4b per scope decision 4). 4 scope decisions Chris-locked at session start via AskUserQuestion, all recommended defaults accepted: (1) skeleton + Sudan Rume seed (matches CCIL placeholder line 56 lock); (2) split into PR 4a + PR 4b (residual master-doc content size made single-PR sizing unrealistic); (3) fold Naturals + Honey + 3 ambiguous cleanup into PR 4b; (4) CLAUDE.md compaction in PR 4b via extraction to `docs/architecture/sub-skills-status.md`.

**Wave 4 PR 4b** closes the architecture implementation arc. Scope: rewrite BREWING.md (124KB → ~500-byte redirect stub) + ROASTING.md (72KB → ~500-byte redirect stub) per `docs/architecture/master-doc-transition-plan.md` § Wave 4. Migrate residual operational content (BREW PROMPT Steps 1-4, Two-Axis Framework, Location Constraints, Equipment Reference, Valve Position, Filter System, Example Outputs from BREWING.md; Roasting Philosophy, Standard Workflow, Naturals, Honey, Data Capture, New Coffee Onboarding Protocol, Lot Knowledge, Session Debrief Template, Storage Protocol from ROASTING.md) to appropriate sub-skill clusters or `coordinator/catalog.md § domain-principles`. Fold in the Naturals + Honey + 3 ambiguous ROASTING.md sections cleanup pass per Wave 3 PR 1 Chris-lock. Compact CLAUDE.md sub-skills section via extraction to a new `docs/architecture/sub-skills-status.md` doc (drops CLAUDE.md back under the 120KB tripwire).

**Triggers when:** Wave 4 PR 4a closes (shipped 2026-05-21).
...
**Triggers when:** architecture implementation sprints close. **Sizing:** S (single session, similar shape to Sprint R itself).

### Queued post-architecture

Every entry here waits behind the architecture brainstorm + implementation. Priorities re-decided in the roadmap re-session (#4 above). Content preserved verbatim from pre-Sprint-R state unless flagged otherwise.
...
- **120KB threshold on any single root-level living doc.** When crossed, plan a split sprint per the CLAUDE.md sprint cadence rule. Current (2026-05-26 post-Wave-3-PR-1): PRODUCT.md 108KB (climbing toward tripwire), CLAUDE.md 113KB (climbing toward tripwire), **BREWING.md 124KB (103% of tripwire — just over post-Wave-2-PR-2; shrunk from 213KB via Brewing Historian extraction)**, **ROASTING.md 72KB (60% of tripwire — UNDER, post-Wave-3-PR-1; shrunk from 147KB cumulative via Roasting Historian Wave 2 + Roest Knowledge / Peer-Learning Wave 3 PR 1; Roast-to-Brew Translation extracted to Roasting Historian during Wave 3 PR 1 override)**. **Wave 2 complete; Wave 3 + 4 are the residual architecture work.** **The tripwire response is in flight via [Architecture implementation Waves 1-4](#active-sprints)** — the architecture brainstorm cluster (shipped 2026-05-26) locked the decomposition into 18 sub-skills under `docs/skills/` with master-doc transition plan per [docs/architecture/master-doc-transition-plan.md](docs/architecture/master-doc-transition-plan.md). Wave 2 was the biggest shrink event (cumulative ~131KB across BREWING + ROASTING; PR 2 88KB / PR 3 43KB). Pre-architecture candidate carriers (per-process subdocs lifting lot-specific learnings out of ROASTING.md proper; pulling Cross-Coffee Insight Layer + Open Questions + Roast-to-Brew Translation into a sibling `docs/roasting/insight-layer.md`) are subsumed by the architecture work — those sections become per-sub-skill cluster docs. **Architecture-level tripwires added per [ADR-0013](docs/adr/0013-self-improvement-primitives.md)**: catalog > 30KB → propose domain-shard re-introduction; sub-skill cluster > 60KB → propose self-decomposition; total `docs/skills/` > 300KB → propose Wave 5 architectural retro.
