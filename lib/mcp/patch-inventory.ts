...
export function registerPatchInventoryTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'patch_inventory',
    {
      ...
      description:
        'Patch / update / modify / archive / adjust a single Roest inventory by id via api.roestcoffee.com PATCH /inventories/{id}/. Field-level updates: only keys present in the input are sent. Common uses: mark is_archived=true after final roast on a lot, update current_weight after a roast, fix typos on producer / notes / cultivar. Weight units: raw grams (same convention as the inventory INSERT path of the same domain). Moisture: percentage value (8.7), not fraction. bean_process: Roest 5-bucket enum (1=Natural, 2=Washed, 3=Honey, 4=Co-fermented/XO, 5=Anaerobic). No DB-side effects — the green_beans.roest_inventory_id link (if any) is established at inventory-INSERT time; to bind an existing Roest inventory to an existing green_beans row, use the green_beans field-level mutation path. Returns { ok: true, updated_fields: [...], canonical_values: { ... } } — updated_fields echoes which keys were sent; canonical_values echoes the actual values of small-vocab fields (bean_process, is_archived) so the caller can confirm without a follow-up read. Owned by Close-Lot Specialist per ADR-0011 (is_archived archival at lot close-out; the inventory INSERT / field-level mutation ownership for active-roasting inventory updates remains with the operator-driven Roest sync flow).',
