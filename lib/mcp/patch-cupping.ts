...
export function registerPatchCuppingTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'patch_cupping',
    {
      title: 'Patch Cupping',
      description:
        'Update / save / record / push field-level changes to an existing cupping evaluation by composite key (roast_id + cupping_date + eval_method + recipe_variant — the migration 041 NULLS NOT DISTINCT key). The companion INSERT path of the same domain returns `created:false` without overwriting fields on composite-key conflict, so this Tool closes the prose-typo / numeric-correction path (R29 from the MCP feedback log) for cupping rows that already exist. Use this for fixes to the 10 prose fields (aroma / flavor / acidity / sweetness / body / finish / overall / temperature_behavior / aromatic_behavior / structural_behavior — last two relocated from roast_learnings in Sprint 11 / migration 062 / ADR-0008) or numeric ground_agtron / rest_days. wb_agtron override is available for the rare post-hoc Agtron re-measurement case (normally auto-snapshot at cupping-insert time). Field-level mutation: only fields you EXPLICITLY supply are updated; omitted fields are untouched. NULL matches NULL on the composite key (NULLS NOT DISTINCT) — single-cupping rows look up cleanly with recipe_variant: null. To find cupping rows: call get_bean_pipeline (returns cuppings[] with the composite-key fields). Returns { cupping_id, updated_fields: [...] } — updated_fields echoes which columns landed in the patch. Owned by Cupping Specialist per ADR-0011.',
