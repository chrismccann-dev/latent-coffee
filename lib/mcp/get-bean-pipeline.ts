...
export function registerGetBeanPipelineTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'get_bean_pipeline',
    {
      ...
      description:
        'Look up / fetch / get / list / read all roasts + cuppings + experiments + roast_learnings + brews for a green coffee bean lot in one call. Returns { green_bean, roasts[], cuppings[], experiments[], roast_learnings, brews[] }. Use this AFTER a crash / cross-session retry to recover roast_ids (for the push_cupping FK chain), experiment_pks (for push_experiment UPSERT key matching), or just to verify pipeline state across sessions. brews[] is lightweight summaries (id + coffee_name + source + roast_id + extraction_strategy + what_i_learned + created_at, sorted latest-first) so you can DETECT prior brews for this bean before pushing a new one — surfaces orphan brews where roast_id is null (those need backfill via the future patch_brew Tool). Most common workflow: get_green_bean({lot_id}) recovers green_bean_id, then get_bean_pipeline({green_bean_id}) recovers all downstream IDs the FK chain needs. Mirrors the roasts://by-bean/{green_bean_id} Resource exactly.',
