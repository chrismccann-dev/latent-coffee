...
export function registerPushBrewTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'push_brew',
    {
      title: 'Push Brew',
      description:
        'Log / submit / save / archive / record a brew to the app — the primary write path for finished brews. Inserts a resolved brew row into the brews table; validates every field against canonical registries (cultivars / terroirs / processes / roasters / producers / brewers / filters / flavors / structure_tags / extraction_strategy / etc.); FK-resolves terroir + cultivar via lazy find-or-create. ALL validation errors return aggregated in one response (no fail-fast), so a payload with multiple problems gets the full list back in a single retry round. Returns brew_id + per-field resolution metadata (which FKs were newly created, which canonicals matched). For canonical lookup before drafting the payload, call list_canonicals + read_canonical. Owned by Brew Recorder per ADR-0011.',
