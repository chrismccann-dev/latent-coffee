...
export function registerPushBrewTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'push_brew',
    {
      title: 'Push Brew',
      description:
        'INSERT / CREATE / log / submit / archive / record a NEW brew row into the brews table — the primary write path for finished brews completing the brewing pipeline (start-brew session → in-thread iteration → bundled-brewing-completion finalize). Use to write a brew that does not yet exist in the database; field-level edits on a brew that already exists run through the brew-id-keyed field-level mutation companion of the same domain. Validates every field against canonical registries (cultivars / terroirs / processes / roasters / producers / brewers / filters / flavors / structure_tags / extraction_strategy / etc.). FK resolution behavior is asymmetric per axis: terroir lazy find-or-creates a row for new (admin_region, macro_terroir, meso_terroir) combinations whose macro_terroir is ALREADY in the canonical registry — but macro_terroir itself is strict-canonical with no terroir_override path (net-new macros require `propose_canonical_addition(axis: "terroir")` + a separate registry edit before they resolve); cultivar lazy find-or-creates similarly with cultivar_name strict-canonical. For text-only canonical axes (roaster / producer / brewer / filter / grinder), `*_override: true` accepts net-new entities and queues them via `taxonomy_overrides_queue`. ALL validation errors return aggregated in one response (no fail-fast), so a payload with multiple problems gets the full list back in a single retry round. Returns brew_id + per-field resolution metadata (which FKs were newly created, which canonicals matched). For canonical lookup before drafting the payload, use the canonicals catalog + per-axis canonical reads. Owned by Brew Recorder per ADR-0011.',
