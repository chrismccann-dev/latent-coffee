...
export function registerPatchBrewTool(server: McpServer, auth: McpAuthContext) {
  server.registerTool(
    'patch_brew',
    {
      title: 'Patch Brew',
      description:
        'Edit / amend / fix / mutate field-level data on an existing brew row by brew_id — use only when the brew row already exists. Use when you need to fix a typo on an already-archived brew, backfill a missing field on an existing brew (orphan-brew with NULL roast_id, FK linkage correction), or otherwise mutate a single brew without re-archiving the whole payload. Field-level mutation: only fields you EXPLICITLY supply in the body are updated; omitted fields are untouched. Re-validates every canonical-validated field (roaster / producer / brewer / filter / grinder / roast_level / flavors / structure_tags / extraction_strategy / process axes) — including the `*_override` flags for legitimately net-new entities. Validation errors aggregate (multi-field problems collapse to one round-trip). To find brew_ids: call get_bean_pipeline (returns brews[] for self-roasted beans). Returns { brew_id, updated_fields: [...] } — updated_fields echoes simple-column keys the caller supplied (canonical re-resolutions like roaster / producer / cultivar / terroir / process axes are NOT echoed because they touch multiple columns; check a follow-up read if you need to confirm those landed). Owned by Brew Recorder per ADR-0011.',
